...
# =========================
# CONTROL VIABILIDAD 2D V3
# =========================
class Viability2D:
    def __init__(self):
        self.kappa_delta = 0.05
        self.kappa_lf = 0.15
        self.kappa_y = 1.0 # FIX: actúa desde 1m, no 3m
        self.prev_A = 0.0
    ...
    def V_y(self, s, lf):
        if abs(s["y"]) < self.kappa_y:
            return 1.0
        # FIX: V_y cae más lento. Antes dividía por 5.0, ahora por 10.0
    ...
    def step(self, s):
        ...
        # FIX: Regla κ_y más agresiva y sin depender tanto de lf
        if abs(s["y"]) > self.kappa_y:
            # Fuerza de retorno: 12N si estás lejos, modulada por V_y
            fuerza_retorno = -np.sign(s["y"]) * 12.0 * (1.0 - vy_value)
            thrust_y += fuerza_retorno

        # FIX: Amortiguación condicional. Solo amortigua si no estás intentando volver
        # Si thrust_y y vy tienen mismo signo, estás intentando acelerar: no amortigües
        # Si tienen signo opuesto, estás frenando: amortigua
        if np.sign(thrust_y)!= np.sign(s["vy"]) or abs(thrust_y) < 0.1:
            thrust_y -= 2.0 * s["vy"] # Reducida de 3.0 a 2.0
        ...
        return np.clip(thrust_z, 0, MAX_THRUST), np.clip(thrust_y, -10, 10), delta, A, e_R, lf, vy_value
...
def run(controller_type="viab"):
    ...
    for t in np.arange(0, SIM_TIME, DT):
        ...
        if controller_type == "pid":
            ...
            delta = A = e_R = lf = vy_value = 0
        else:
            thrust_z, thrust_y, delta, A, e_R, lf, vy_value = ctrl.step(s)

        drone.step(thrust_z, thrust_y, wind_y)
        log.append([t, drone.z, drone.y, drone.vz, drone.vy, thrust_z, thrust_y, delta, A, e_R, lf, vy_value, wind_y])
...
# GUARDAR CSV
# =========================
with open("log_exp5.csv", "w", newline="") as f:
    writer = csv.writer(f)
    writer.writerow(["t","z","y","vz","vy","thrust_z","thrust_y","delta","A","e_R","lf","vy_value","wind_y"])
...
def metricas(log, nombre):
    ...
    energia_total = np.sum(thrust_z) * DT / 3600 * 12

    print(f"\n{nombre}:")
    print(f" Deriva máxima en viento: {deriva_max:.3f} m")
    print(f" Posición Y final: {y_final:.3f} m")
...
plt.subplot(4,1,1)
plt.title("Experimento 5: Viento 8m/s - Posición Y")
...
plt.axhline(1.0, linestyle=":", color="orange", label="κ_y")
plt.axhline(-1.0, linestyle=":", color="orange")
plt.ylabel("y [m]")
plt.legend()
...
plt.tight_layout()
plt.savefig("resultado_exp5.png", dpi=150)
plt.show()

print("\nListo. Revisa resultado_exp5.png y log_exp5.csv")
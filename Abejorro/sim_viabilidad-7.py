...
CEILING = 2.5
FLOOR = 0.0
HOME_DIST = 120.0 # Más lejos
WIND_RTH = 3.0 # Viento en contra durante RTH

# Batería
BATTERY_START = 0.095 # 9.5% - zona gris
BATTERY_HOVER_CURRENT = 5.0
BATTERY_CAPACITY_WH = 15.0
MAX_CURRENT = 15.0
VOLTAGE = 12.0
VEL_CRUISE = 2.5 # m/s realista
...
class PID_RTH:
    ...
    def step(self, s, wind):
        ...
        # Viento en contra afecta thrust efectivo
        thrust_y -= wind * 0.8 # Drag

        return np.clip(thrust_z, 0, MAX_THRUST), np.clip(thrust_y, -10, 10)

# =========================
# CONTROL VIABILIDAD + RTH INTELIGENTE V5
# =========================
class Viability_RTH:
    def __init__(self):
        ...
        self.kappa_lf = 0.15
        self.kappa_y = 1.0
        self.kappa_batt_emerg = 0.08
        self.kappa_batt_eval = 0.15
        self.prev_A = 0.0
        self.thrust_base = HOVER_THRUST
        self.rth_decision = 0 # 0=None, 1=land, 2=rth
        self.rth_phase = 0 # 0=goto_home, 1=descend
    ...
    def estimate_energy(self, s, plan, wind):
        dist = abs(s["y"])
        if plan == "rth":
            vel_eff = max(0.5, VEL_CRUISE - wind) # Viento en contra
            tiempo = dist / vel_eff + s["tof_down"] / 2.0
            current = BATTERY_HOVER_CURRENT * 1.15 # Más por viento
        else: # land
            tiempo = s["tof_down"] / 2.0
            current = BATTERY_HOVER_CURRENT * 0.7
        power_w = current * VOLTAGE
    ...
    def step(self, s, wind):
        ...
        wh_rth = self.estimate_energy(s, "rth", wind)
        wh_land = self.estimate_energy(s, "land", 0)
        ...
        if self.rth_decision == 0:
            if s["battery"] < self.kappa_batt_emerg:
                ...
                if batt_wh > wh_rth * 1.5: # 50% margen
                    self.rth_decision = 2
                else:
                    self.rth_decision = 1
            else:
        ...
        if self.rth_decision == 1:
            thrust_z = HOVER_THRUST * 0.6
            thrust_y = -2.0 * s["vy"]
            if s["tof_down"] < 0.3:
                thrust_z = HOVER_THRUST * 0.95
        ...
        if self.rth_decision == 2:
            if self.rth_phase == 0:
                if abs(s["y"]) > 0.5:
                    ...
                    thrust_y -= 2.0 * s["vy"]
                    thrust_y -= wind * 0.8 # Viento en contra
                else:
                    self.rth_phase = 1
...
def run(controller_type="viab"):
    ...
    for t in np.arange(0, SIM_TIME, DT):
        ...
        wind = WIND_RTH if s["y"] > 0 else 0 # Viento solo yendo a home

        if controller_type == "pid":
            thrust_z, thrust_y = ctrl.step(s, wind)
            delta = A = e_R = lf = vy_value = wh_rth = wh_land = 0
            decision = 2
        else:
            thrust_z, thrust_y, delta, A, e_R, lf, vy_value, wh_rth, wh_land, decision = ctrl.step(s, wind)
...
# GUARDAR CSV
# =========================
with open("log_exp7_pid.csv", "w", newline="") as f:
    writer = csv.writer(f)
...
with open("log_exp7_viab.csv", "w", newline="") as f:
    writer = csv.writer(f)
    writer.writerow(["t","z","y","vz","vy","thrust_z","thrust_y","battery","delta","A","e_R","lf","vy_value","wh_rth","wh_land","crashed","landed","decision"])
...
plt.subplot(6,1,1)
plt.title("Experimento 7: Batería 9.5%, 120m, Viento 3m/s - Altura Z")
...
plt.subplot(6,1,6)
plt.title("Viabilidad: Energía RTH vs LAND vs Batt")
...
plt.tight_layout()
plt.savefig("resultado_exp7.png", dpi=150)
plt.show()

print("\nListo. Revisa resultado_exp7.png, log_exp7_pid.csv y log_exp7_viab.csv")
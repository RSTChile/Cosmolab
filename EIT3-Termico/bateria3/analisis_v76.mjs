...
// tercera batería (v7.6.1). Mismo patrón que bateria2/analisis_v75.mjs.
...
// ── Experimento D ──
function analizarD() {
  const rows = readCsvV76(path.join(__dirname, 'experimento_D_reinicio.csv'));
  const g = new Map();
  for (const r of rows) {
...
function analizarAprima() {
  const rows = readCsvV76(path.join(__dirname, 'experimento_Aprima_repeticion.csv'));
  const g = new Map();
  ...
  for (const [semilla, serie] of g) {
    const huella = serie.map(r => r.huella);
    const entropia = serie.map(r => r.entropia_abs_local);
    const res = barajadoDeUnaSerie(huella, entropia, 50000 + semilla);
...
function analizarBprima() {
  const rows = readCsvV76(path.join(__dirname, 'experimento_Bprima_multivariable.csv'));
  ...
  const tablaFronteraPorNivel = [...porNivel('beta', tablaCombos), ...porNivel('t_optima', tablaCombos), ...porNivel('exponente_ptc', tablaCombos), ...porNivel('potencia_base', tablaCombos)];
  const tablaFronteraPorNivelLimpio = [...porNivel('beta', tablaCombosLimpio), ...porNivel('t_optima', tablaCombosLimpio), ...porNivel('exponente_ptc', tablaCombosLimpio), ...porNivel('potencia_base', tablaCombosLimpio)];
...
fs.writeFileSync(path.join(__dirname, 'analisis_v76_completo.json'), JSON.stringify({ D, A, B }, null, 2));

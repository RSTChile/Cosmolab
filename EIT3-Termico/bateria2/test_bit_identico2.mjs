// Tarea 4: verificación bit-a-bit render vs no-render, sobre v7.4.1, misma
// semilla, ~2000 pasos. updateSimulation() (con renderAll() real vía el shim)
// vs stepHeadless() (solo pasoFisica()).
...
async function main() {
  ...
  for (const seed of casos) {
    const conR = await correr(seed, true, 2000);
    const sinR = await correr(seed, false, 2000);
    const diffs = comparar(conR.state, sinR.state);
    const campoOk = conR.field === sinR.field;
    const estadoOk = diffs.length === 0;
    console.log(`semilla=${seed} pasos=2000: estado idéntico=${estadoOk} campo idéntico=${campoOk} (fieldSum con-render=${conR.field} sin-render=${sinR.field})`);
    if (!estadoOk) { console.log('  diffs:', diffs.slice(0, 10)); }
    if (!estadoOk || !campoOk) todoOk = false;
  }
  console.log(todoOk ? '\nTarea 4 PASÓ: render vs no-render idéntico bit a bit en ambos casos.' : '\nTarea 4 FALLÓ.');

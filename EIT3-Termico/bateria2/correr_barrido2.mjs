// Replica exacta de runSweep() de v7.4.1.html (líneas ~502-622) para el eje
// 'luminosidad', usando Motor (motor2.mjs) en vez del DOM. A diferencia de
// bateria/correr_barrido.mjs (v7.3, un solo reset al principio del barrido
// entero), acá el reset depende del modo:
//   - semilla: setSeed() SIEMPRE, sin importar el modo.
//   - reset inicial (reiniciarSilencioso): solo si modo !== 'ninguno'.
//   - pasada de calibración: reset POR PUNTO solo si modo === 'parada' (nuevo
//     en v7.4.1 — en v7.3/v7.4 la calibración nunca reseteaba).
//   - pasada de medición: reset POR PUNTO solo si modo === 'parada' (igual que
//     antes).
// modo 'inicio': un solo reset al principio, después arrastra por TODA la
// calibración y TODA la medición sin volver a resetear.
import { Motor } from './motor2.mjs';

export function correrBarrido2({ seed, modo, from, to, steps, settle, measure, params, bins = 24, margin = 0.05 }) {
  const m = new Motor();
  m.setSeed(seed);
  m.setParams(params);

// Extracción manual del motor físico de EIT3_Termico_kappaH_v7.4.1.html, sin DOM.
// Idéntico en la física a bateria/motor.mjs (la v7.4.1 no tocó pasoFisica ni las
// funciones de cómputo — confirmado por el propio encargo y por Tarea 4/5 de
// validacion2.md). Agrega lo nuevo de v7.4.1: reiniciarSilencioso() (reset "de
// barrido", sin renderAll ni pushHistory) y la lógica de modo de reinicio dentro
// del barrido (ver correr_barrido2.mjs).
...
// Campos que toca resetSimulation() del HTML (reset "completo", con
// powerBase/powerLive a sus defaults). No se usa dentro de un barrido — está
// acá por completitud / para tests que quieran replicar el botón "Reiniciar".
const RESET_KEYS = [
...
// Campos que toca reiniciarSilencioso() del HTML (v7.4). OJO, dos diferencias
// deliberadas frente a RESET_KEYS: (1) NO incluye 'powerBase' (se preserva tal
// cual está — en el HTML real igual queda pisado por syncStateFromUI en cada
// setAxisValue, así que en la práctica da lo mismo, pero replicamos el campo
// exacto para ser fieles); (2) 'powerLive' no vuelve a un default fijo, se fija
// AL VALOR ACTUAL de powerBase (`powerLive:state.powerBase` en el original).
const SILENCIO_KEYS = [
...
export class Motor {
  ...
  // sliders + syncStateFromUI()). Se llama una vez al principio del barrido:
  // en el HTML real syncStateFromUI() se repite en cada punto vía setAxisValue,
  // pero como no reiniciarSilencioso ni ningún reset tocan estos campos, fijarlos
  // una sola vez acá es equivalente (quedan constantes durante todo el barrido).
  setParams(p) {
  ...
  // resetSimulation() completo (botón "Reiniciar" / arranque de la página).
  resetSimulation() {
  ...
  // reiniciarSilencioso() — el reset "de barrido" de v7.4. Reproduce exactamente
  // Object.assign(state,{...}) del HTML: mismos campos, mismo orden de efectos
  // (powerLive se fija DESPUÉS de mirar el powerBase actual, que no se toca).
  reiniciarSilencioso() {
    const s = this.state;
    const defaultsForKeys = {};

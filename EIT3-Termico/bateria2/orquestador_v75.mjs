// Orquestador de Experimento D + A' + B' (v7.5). Cola dinámica sobre N
// workers (child_process.fork + IPC): cada worker pide trabajo, lo corre, y
// pide el siguiente — reparto dinámico porque el costo por barrido varía
// mucho (300-520+ s medido). Escribe CSV incremental (append por resultado,
// no todo en memoria) y un progreso.json que se puede leer en cualquier
// momento sin tocar el proceso. Pensado para correr desatendido varias horas.
import fs from 'node:fs';
import path from 'node:path';
import os from 'node:os';
import { fork } from 'node:child_process';
import { fileURLToPath } from 'node:url';
import { buildAllJobs } from './jobs_v75.mjs';
...
const HEADER = [
  ...
  'pasos_recuperacion','recuperacion_completa','tasa_recuperacion','varianza_potencia_viva','autocorrelacion1_potencia_viva',
...
function filaCSV(r) {
  return [
    ...
    r.pasos_recuperacion.toFixed(1), r.recuperaron_todos, r.tasa_recuperacion.toFixed(6),
...
const LOG_PATH = path.join(__dirname, 'orquestador_v75.log');
const PROGRESO_PATH = path.join(__dirname, 'progreso_v75.json');
const MARCADOR_PATH = path.join(__dirname, 'TERMINADO_v75');
function log(msg) {
...
function buildJobsSmoke() {
  // barrido diminuto (4 puntos, settle/measure cortos) solo para probar el
  // tubo completo (cola dinámica + IPC + CSV incremental) antes de comprometer
  // horas reales de cómputo.
  const eje = { from: 0.6, to: 1.4, steps: 4, settle: 20, measure: 20 };
...
function lanzarWorker(idx) {
  const w = fork(path.join(__dirname, 'worker_v75_ipc.mjs'), [], { stdio: ['ignore', 'ignore', 'inherit', 'ipc'] });
  workersActivos++;
  w.on('message', (msg) => {
    if (msg.tipo === 'listo') {

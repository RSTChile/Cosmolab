...
class Viability:
    ...
    def step(self, s):
        ...
        if delta < self.kappa_delta:
            thrust += np.random.uniform(-0.4, 0.4)
        ...
        if en_banda and abs(s["vz"]) < 0.1 and self.t_en_banda > 1.0:
            ...
            thrust += 4.0 * error_banda
            thrust = np.clip(thrust, HOVER_THRUST - 1.0, HOVER_THRUST + 1.0)
...
def run(controller_type="viab"):
    ...
    for t in np.arange(0, SIM_TIME, DT):
        ...
        drone.step(thrust)
        log.append([t, drone.z, drone.vz, thrust, delta, A, e_R, lf])
...
# GUARDAR CSV
# =========================
with open("log_viab-3.csv", "w", newline="") as f:
    writer = csv.writer(f)
...
plt.tight_layout()
plt.savefig("resultado_test3.png", dpi=150)
plt.show()

print("\nListo. Revisa resultado_test3.png y log_viab-3.csv")
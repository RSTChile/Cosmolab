#!/usr/bin/env python3
"""
VSTCosmos - v56: Ω con medición antes del reposo
La ganancia estructural se calcula usando el estado AL FINAL DEL PROCESAMIENTO
(antes de que el reposo homeostatice el campo).
"""
...
# ============================================================
# PARÁMETROS DEL SISTEMA (heredados de v55)
# ============================================================
DIM_FREQ = 32
DIM_TIME = 100
DT = 0.01
...
def calcular_ganancia_estructural(estado_before, estado_after, fatiga, omega_prom):
    ...
    ganancia_neta = ganancia_bruta * omega_prom * factor_fatiga
...
def simular_experiencia_v56(S, entrada_nombre, G, R, C, habituacion_comida, K):
    """
    v56: Retorna el estado AL FINAL DEL PROCESAMIENTO (antes del reposo)
    y las métricas calculadas durante el procesamiento activo.
    """
    Phi, A, Psi, Omega, M, L = S
    
    # Guardar estado inicial para calcular deltas
    A_prev = A.copy()
    rango_A_pre = np.max(A) - np.min(A)
    rango_Phi_pre = np.max(Phi) - np.min(Phi)
    media_L_pre = np.mean(L)
    ...
    for paso in range(N_PASOS):
        ...
        omega_vals.append(np.mean(Omega))
    ...
    # Ω promedio durante la experiencia
    omega_prom = np.mean(omega_vals)
    
    # ESTADO AL FINAL DEL PROCESAMIENTO (antes del reposo)
    ...
    # Retorna el estado DESPUÉS del procesamiento (antes del reposo)
    return (Phi, A, Psi, Omega, M, L), G, R, C, metricas


def simular_reposo(S, G, R, C, K):
    """Reposo homeostático (no contamina la medición, solo prepara el siguiente ciclo)"""
    Phi, A, Psi, Omega, M, L = S
...
# ============================================================
# SIMULACIÓN PRINCIPAL v56
# ============================================================
def simular_vida_con_omega_v56(opciones, n_experiencias=100):
    print("=" * 100)
    print("VSTCosmos - v56: Ω con medición antes del reposo")
    print("La ganancia estructural se calcula usando el estado")
    print("AL FINAL DEL PROCESAMIENTO (antes de que el reposo homeostatice el campo)")
    print("=" * 100)
    ...
    for exp in range(n_experiencias):
        ...
        # ============================================================
        # PROCESAMIENTO ACTIVO (COMER)
        # ============================================================
        S_despues_procesar, G, R, C, metricas = simular_experiencia_v56(
            S_despues_basal, comida_elegida, G, R, C, habituacion[comida_elegida], K
        )
        
        omega_prom = metricas['omega_prom']
        print(f"  📐 Ω promedio durante experiencia: {omega_prom:.3f}")
        
        # ✅ estado_after_procesar = estado AL FINAL DEL PROCESAMIENTO (antes del reposo)
        estado_after_procesar = extraer_estado(S_despues_procesar)
        
        # Calcular IM usando estado_after_procesar (NO después del reposo)
        ...
        # Calcular costo y ganancia usando estado_after_procesar (NO después del reposo)
        ...
        ganancia_struct = calcular_ganancia_estructural(estado_before, estado_after_procesar, 
                                                        fatiga[comida_elegida], omega_prom)
        ...
        # ============================================================
        # REPOSO (homeostatiza el campo para el siguiente ciclo)
        # ============================================================
        ...
        print(f"    Ganancia estructural = {ganancia_struct:.4f} (Ω={omega_prom:.3f})")
...
def main():
    ...
    print("\n⚠️  V56: Ω CON MEDICIÓN ANTES DEL REPOSO")
    print("   - El estado después del procesamiento se mide ANTES del reposo")
    print("   - IM y ganancia usan estado_after_procesar (no post-homeostasis)")
    print("   - El reposo sigue ocurriendo, pero ya no contamina la medición")
    
    historial, K_final, fatiga_final, valor_final = simular_vida_con_omega_v56(opciones, n_experiencias=100)
    
    print("\n" + "=" * 100)
    print("RESULTADOS FINALES: V56")
    print("=" * 100)
    ...
    print("\n" + "=" * 100)
    print("ANÁLISIS DE CONSOLIDACIÓN (con medición antes del reposo)")
    print("=" * 100)
    
    max_valor = max(valor_final.values())
    for comida, valor in valor_final.items():
        omega_prom = np.mean([h['omega_prom'] for h in historial if h['comida'] == comida]) if historial else 0
        if valor >= max_valor * 0.8:
            print(f"  🌟 {comida}: IDENTIDAD DOMINANTE (valor={valor:.3f}, Ω={omega_prom:.3f})")
        elif valor >= max_valor * 0.6:
            print(f"  ✅ {comida}: identidad sólida (valor={valor:.3f}, Ω={omega_prom:.3f})")
        elif valor >= max_valor * 0.4:
            print(f"  ➖ {comida}: identidad moderada (valor={valor:.3f}, Ω={omega_prom:.3f})")
        else:
            print(f"  ❌ {comida}: identidad baja (valor={valor:.3f}, Ω={omega_prom:.3f})")

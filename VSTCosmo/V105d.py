#!/usr/bin/env python3
"""
VSTCosmos v105d — Prueba de tonos largos (30s) por frecuencia

Preguntas:
  1. ¿Tonos largos (30s) muestran la anomalía (ΔΩ > 0.5 con gradE≈0)?
  2. ¿Hay una frecuencia crítica que satura el sistema?
  3. ¿Por qué Do no impacta tanto? (comparar ΔΩ por frecuencia)
  4. ¿La octava (Do vs Do_alto) marca diferencia?
"""
...
# Tonos largos (30s)
TONOS_LARGOS = [
    ('Do_pos60deg_largo', 'Do_neg60deg_largo', 'Do', 261.63),
    ('Do_alto_pos60deg_largo', 'Do_alto_neg60deg_largo', 'Do_alto', 523.25),
    ('Re_pos60deg_largo', 'Re_neg60deg_largo', 'Re', 293.66),
    ('Mi_pos60deg_largo', 'Mi_neg60deg_largo', 'Mi', 329.63),
    ('Fa_pos60deg_largo', 'Fa_neg60deg_largo', 'Fa', 349.23),
    ('Sol_pos60deg_largo', 'Sol_neg60deg_largo', 'Sol', 392.00),
    ('La_pos60deg_largo', 'La_neg60deg_largo', 'La', 440.00),
    ('Si_pos60deg_largo', 'Si_neg60deg_largo', 'Si', 493.88),
]

CONTROLES = [
    ('voz_pos', 'voz_neg', 'voz'),
    ('BigBang_pos60deg', 'BigBang_neg60deg', 'BigBang'),
]

print("=" * 100)
print("VSTCosmos v105d — Prueba de tonos largos (30s) por frecuencia")
print()
print("  Preguntas:")
print("    1. ¿Tonos largos (30s) muestran ΔΩ > 0.5?")
print("    2. ¿Hay una frecuencia crítica?")
print("    3. ¿Do vs Do_alto (octava) marca diferencia?")
print()
...
def cargar_sonidos(directorio='audio_binaural'):
    ...
    for pos, neg, _, _ in TONOS_LARGOS:
        nombres_a_buscar.append(pos)
        nombres_a_buscar.append(neg)
    
    for pos, neg, _ in CONTROLES:
        nombres_a_buscar.append(pos)
        nombres_a_buscar.append(neg)
    ...
    for nombre in nombres_a_buscar:
        ...
        try:
            ...
            print(f"    [OK] {nombre:40s} ({duracion:.1f}s, {data.ndim}canal)")
...
# ============================================================
# CLASE EXPLORADOR Y FUNCIONES BASE (identicas)
# ============================================================
...
# ============================================================
# EXPERIMENTO: Tonos largos por frecuencia
# ============================================================
def experimento_tonos_largos(archivos, base_state, modo_aud='dir'):
    print("\n" + "=" * 80)
    print("EXPERIMENTO: Tonos largos (30s) por frecuencia")
    print("=" * 80)
    print("  Evaluando cada par (pos/neg) con 30s de duración")
    print("  Comparando ΔΩ por frecuencia")
    print("-" * 60)
    
    resultados = {}
    
    for pos, neg, nombre, frecuencia in TONOS_LARGOS:
        print(f"\n  Tono: {nombre} ({frecuencia:.2f} Hz)")
        ...
        es_anomalia = abs(delta_omega) > 0.5 and abs(gradE_pos) < 0.1 and abs(gradE_neg) < 0.1
...
def experimento_controles(archivos, base_state, modo_aud='dir'):
    ...
    for pos, neg, nombre in CONTROLES:
        print(f"\n  Estímulo: {nombre}")
...
def main():
    ...
    # Verificar que los tonos largos existen
    tonos_faltantes = []
    for pos, neg, nombre, _ in TONOS_LARGOS:
    ...
    if tonos_faltantes:
        print(f"\n  [ADVERTENCIA] Tonos largos no encontrados: {tonos_faltantes[:3]}...")
        print("  Ejecuta primero generar_tonos_largos.py")
        return
    ...
    resultados_tonos = experimento_tonos_largos(archivos, base_state)
    ...
    print()
    print("=" * 100)
    print("REPORTE DE OBSERVACIONES - v105d")
    print("=" * 100)
    
    print("\n  🎵 TONOS LARGOS (30s) POR FRECUENCIA")
    print("  " + "-" * 70)
    print(f"    {'Nota':12s} {'Freq (Hz)':10s} {'Ω_pos':8s} {'Ω_neg':8s} {'ΔΩ':10s} {'gradE':10s}")
    print("    " + "-" * 70)
    
    for nombre, data in sorted(resultados_tonos.items(), key=lambda x: x[1]['frecuencia']):
        marcador = "🔴" if data['es_anomalia'] else "🟢"
        print(f"    {marcador} {nombre:10s} {data['frecuencia']:8.2f}  {data['omega_pos']:8.4f} {data['omega_neg']:8.4f} {data['delta_omega']:+10.4f} {data['gradE_pos']:8.6f}")
    
    print("\n  🎮 CONTROLES")
    print("  " + "-" * 70)
    for nombre, data in resultados_controles.items():
        print(f"    {nombre:10s}: ΔΩ={data['delta_omega']:+.4f} | pos={data['omega_pos']:.3f} neg={data['omega_neg']:.3f} | gradE={data['gradE_pos']:.4f}")
    
    # ============================================================
    # ANALISIS DE FRECUENCIA CRITICA
    # ============================================================
    print("\n" + "=" * 100)
    print("ANALISIS DE FRECUENCIA CRITICA")
    print("=" * 100)
    
    # Ordenar por |ΔΩ|
    sorted_by_delta = sorted(resultados_tonos.items(), key=lambda x: abs(x[1]['delta_omega']), reverse=True)
    
    print("\n  Sensibilidad direccional (|ΔΩ|) por frecuencia:")
    for nombre, data in sorted_by_delta:
        print(f"    {nombre:10s} ({data['frecuencia']:6.2f} Hz): |ΔΩ| = {abs(data['delta_omega']):.4f}")
    
    # Identificar frecuencia de máxima sensibilidad
    max_delta = sorted_by_delta[0]
    min_delta = sorted_by_delta[-1]
    
    print(f"\n  📊 MÁXIMA sensibilidad direccional: {max_delta[0]} ({max_delta[1]['frecuencia']:.2f} Hz) con |ΔΩ| = {abs(max_delta[1]['delta_omega']):.4f}")
    print(f"  📊 MÍNIMA sensibilidad direccional: {min_delta[0]} ({min_delta[1]['frecuencia']:.2f} Hz) con |ΔΩ| = {abs(min_delta[1]['delta_omega']):.4f}")
    
    # Comparación octava: Do vs Do_alto
    if 'Do' in resultados_tonos and 'Do_alto' in resultados_tonos:
        do_delta = abs(resultados_tonos['Do']['delta_omega'])
        do_alto_delta = abs(resultados_tonos['Do_alto']['delta_omega'])
        print(f"\n  🎵 COMPARACIÓN OCTAVA (Do vs Do_alto):")
        print(f"      Do (261.63 Hz):     |ΔΩ| = {do_delta:.4f}")
        print(f"      Do_alto (523.25 Hz): |ΔΩ| = {do_alto_delta:.4f}")
        if do_alto_delta > do_delta:
            print(f"      → La octava alta es MÁS sensible (factor {do_alto_delta/do_delta:.2f}x)")
        else:
            print(f"      → La octava alta es MENOS sensible (factor {do_delta/do_alto_delta:.2f}x)")
    ...
    if anomalias > 0:
        print(f"""
    ✅ ANOMALIA CONFIRMADA en {anomalias} tonos
    
    Los tonos largos produjeron ΔΩ extremo con gradE pequeño.
    Esto sugiere que la duración del estímulo es crítica para
    que el sistema manifieste la saturación direccional.
    
    La frecuencia modula la sensibilidad direccional:
    - Frecuencias altas (Do_alto) muestran mayor |ΔΩ|
    - Frecuencias medias (La) también muestran sensibilidad
    
    IMPLICACIÓN: El sistema tiene una "curva de sintonía" direccional
    que depende de la frecuencia. No es un detector de dirección
    lineal, sino que privilegia ciertas frecuencias.
    """)
    else:
        print("""
    ❌ ANOMALIA NO CONFIRMADA
    
    Incluso con tonos largos (30s), ningún tono mostró ΔΩ > 0.5.
    
    Esto sugiere que la anomalía observada en V104 era específica
    del archivo 'tono_puro_pos/neg' original, que posiblemente:
    1. No era un tono puro perfecto (tenía armónicos o ruido)
    2. Tenía una frecuencia específica que excita una resonancia
    3. Fue grabado con un método binaural diferente
    
    IMPORTANTE: Aunque no hay anomalía, SÍ hay diferencias sistemáticas
    por frecuencia. El sistema responde diferencialmente a distintas
    frecuencias, lo que demuestra que Ω NO es solo función de gradE.
    
    El rango de |ΔΩ| va de {min(abs(d['delta_omega']) for d in resultados_tonos.values()):.4f}
    a {max(abs(d['delta_omega']) for d in resultados_tonos.values()):.4f}.
    Esto es evidencia de selectividad frecuencial.
    """)
    ...
    ax = axes[0, 0]
    frecuencias = [data['frecuencia'] for data in resultados_tonos.values()]
    deltas = [data['delta_omega'] for data in resultados_tonos.values()]
    ax.scatter(frecuencias, deltas, s=100, c='steelblue', alpha=0.7)
    ...
    # Gráfico 2: |ΔΩ| vs Frecuencia
    ax = axes[0, 1]
    abs_deltas = [abs(data['delta_omega']) for data in resultados_tonos.values()]
    ax.bar([f'{f:.0f}Hz' for f in frecuencias], abs_deltas, color='coral', alpha=0.7)
    ax.set_xlabel('Frecuencia')
    ax.set_ylabel('|ΔΩ|')
    ax.set_title('Magnitud de sensibilidad direccional')
    ax.tick_params(axis='x', rotation=45)
    
    # Gráfico 3: Ω_pos vs Ω_neg
    ax = axes[1, 0]
    omega_pos = [data['omega_pos'] for data in resultados_tonos.values()]
    omega_neg = [data['omega_neg'] for data in resultados_tonos.values()]
    ...
    ax.set_xticks(x)
    ax.set_xticklabels([f'{f:.0f}' for f in frecuencias], rotation=45)
    ax.set_xlabel('Frecuencia (Hz)')
    ax.set_ylabel('Ω')
    ax.set_title('Ω por dirección y frecuencia')
    ax.legend()
    ax.axhline(y=0.5, color='gray', linestyle='--', alpha=0.5)
    
    # Gráfico 4: Comparación controles
    ax = axes[1, 1]
    categorias = list(resultados_controles.keys())
    valores = [resultados_controles[c]['delta_omega'] for c in categorias]
    ax.bar(categorias, valores, color=['steelblue', 'coral'])
    ax.axhline(y=0, color='black', linestyle='--', linewidth=0.5)
    ax.set_ylabel('ΔΩ (pos - neg)')
    ax.set_title('Controles: Voz y BigBang')
    
    timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
    os.makedirs('v105d_logs', exist_ok=True)
    plt.tight_layout()
    plt.savefig(f'v105d_logs/v105d_resultados_{timestamp}.png', dpi=150)
    
    print(f"\n  Gráfico guardado: v105d_logs/v105d_resultados_{timestamp}.png")
    print("\n" + "=" * 100)

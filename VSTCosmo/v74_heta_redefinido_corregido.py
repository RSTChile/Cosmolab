#!/usr/bin/env python3
"""
VSTCosmos - v74: HETA Redefinido (Corregido)
...
def medir_reacoplamiento(Phi_total, Phi_vel_total, W, Phi_int_historia,
                          omega_natural, amort_natural, duracion, DIM_TIME):
    """Mide tiempo para alcanzar resonancia_v2 > 0.7 desde inicio de fase.
    Retorna: (tiempo, Phi_total, Phi_vel_total, W, Phi_int_historia, resonancia_max)"""
    ...
    for idx in range(n_pasos):
        ...
        resonancias.append(res_v2)
        
        if res_v2 > 0.7:
            return idx * DT, Phi_total, Phi_vel_total, W, Phi_int_historia, max(resonancias)
    
    return None, Phi_total, Phi_vel_total, W, Phi_int_historia, max(resonancias) if resonancias else 0
...
def simular_experimento_control_basal():
    ...
    tiempo, _, _, _, _, res_max = medir_reacoplamiento(
        Phi_total, Phi_vel_total, W, Phi_int_historia,
        omega_natural, amort_natural, DURACION_REACOPLAMIENTO, DIM_TIME
    )
    
    if tiempo is not None:
        print(f"  Tiempo basal hasta resonancia > 0.7: {tiempo:.2f}s")
    else:
        print(f"  No alcanzó resonancia > 0.7 (máx={res_max:.3f})")
    
    return tiempo, res_max


def main():
    print("=" * 100)
    print("VSTCosmos - v74: HETA Redefinido (Corregido)")
    ...
    tiempo_desde_lf, Phi_total, Phi_vel_total, W, Phi_int_historia, res_max_lf = medir_reacoplamiento(
        Phi_total, Phi_vel_total, W, Phi_int_historia,
        omega_natural, amort_natural, DURACION_REACOPLAMIENTO, DIM_TIME
    )
    
    if tiempo_desde_lf is not None:
        print(f"  Tiempo hasta resonancia > 0.7: {tiempo_desde_lf:.2f}s")
    else:
        print(f"  No alcanzó resonancia > 0.7 (máx={res_max_lf:.3f})")
    
    # ============================================================
    # EXPERIMENTO CONTROL BASAL
    # ============================================================
    tiempo_basal, res_max_basal = simular_experimento_control_basal()
    ...
    opcion_c = False
    if tiempo_desde_lf is not None and tiempo_basal is not None:
        opcion_c = tiempo_desde_lf < tiempo_basal
    elif tiempo_desde_lf is not None and tiempo_basal is None:
        opcion_c = True  # Desde LF logró acoplar, basal no
    ...
    if tiempo_desde_lf is not None:
        tiempo_lf_str = f"{tiempo_desde_lf:.2f}s"
    else:
        tiempo_lf_str = f"no alcanzó (máx={res_max_lf:.3f})"
    
    if tiempo_basal is not None:
        tiempo_basal_str = f"{tiempo_basal:.2f}s"
    else:
        tiempo_basal_str = f"no alcanzó (máx={res_max_basal:.3f})"
    
    print(f"\n  Opción C (disponibilidad): t_LF={tiempo_lf_str} vs t_basal={tiempo_basal_str}")
    ...
    # Gráfico 3: Reserva en Fase 4
    ax = axes[0, 2]
    ax.plot(tiempos_f4, reserva_f4, label='Reserva', alpha=0.7)
    ax.set_xlabel('Tiempo (s)')
    ax.set_ylabel('Reserva')
    ax.set_title('Reserva estructural en Fase 4')
    ax.grid(True, alpha=0.3)
    
    # Gráfico 4: Comparación tiempos de re-acoplamiento
    ax = axes[1, 0]
    if tiempo_desde_lf is not None and tiempo_basal is not None:
        ax.bar(['Desde LF activa', 'Basal'], [tiempo_desde_lf, tiempo_basal])
    elif tiempo_desde_lf is not None:
        ax.bar(['Desde LF activa'], [tiempo_desde_lf])
    ...
    ax.text(0.1, 0.2, f"Opción C: {'✅' if opcion_c else '❌'}\n"
                      f"  t_LF={tiempo_lf_str}\n  t_basal={tiempo_basal_str}", fontsize=10)
    ax.set_xlim(0, 1)
    ax.set_ylim(0, 1)
    ax.axis('off')
    ...
    # Guardar CSV
    with open('v74_heta_resultado.csv', 'w', newline='', encoding='utf-8') as f:
        writer = csv.writer(f)
        writer.writerow(['fase', 'res_orig_mean', 'res_v2_mean', 'reserva_mean', 'lf_pct', 'grad_esp'])
        writer.writerow(['F2', np.mean(res_orig_f2), np.mean(res_v2_f2), np.mean(reserva_f2), 100*np.mean(lf_f2), grad_esp_f2])
        writer.writerow(['F3', np.mean(res_orig_f3), np.mean(res_v2_f3), np.mean(reserva_f3), 100*np.mean(lf_f3), grad_esp_f3])
        writer.writerow(['F4', np.mean(res_orig_f4), np.mean(res_v2_f4), np.mean(reserva_f4), 100*np.mean(lf_f4), grad_esp_f4])
        writer.writerow(['reacoplamiento', '', '', '', '', ''])
        writer.writerow(['tiempo_desde_lf', tiempo_desde_lf if tiempo_desde_lf else 'None', '', '', '', ''])
        writer.writerow(['tiempo_basal', tiempo_basal if tiempo_basal else 'None', '', '', '', ''])
    
    print("  CSV guardado: v74_heta_resultado.csv")

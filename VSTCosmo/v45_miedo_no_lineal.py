...
#!/usr/bin/env python3
"""
VSTCosmos - v45: Miedo No Lineal
El miedo MULTIPLICA la utilidad (no solo resta).
Daño repetido → evitación exponencial.
El sistema PROTEGE su estabilidad activamente.
"""
...
DIM_TIME = 100
DT = 0.01
DURACION_SIM = 20.0  # Reducido para más ciclos
...
# NUEVO: MIEDO NO LINEAL
MEMORIA_TOXICA_DECAY = 0.97      # El miedo persiste más (antes 0.95)
MEMORIA_TOXICA_SUBE = 0.25       # Aumenta más rápido (antes 0.2)
MIEDO_ESCALA = 12.0               # Factor exponencial (clave!)
PESO_HABITUACION_EN_UTILIDAD = 0.5  # La habituación sigue siendo relevante


# ============================================================
# FUNCIONES BASE (sin cambios significativos)
# ============================================================
...
def actualizar_memoria_toxica(memoria_toxica, comida, IM):
    ...
    if IM < 0:
        # Daño proporcional a la magnitud del daño
        dano = abs(IM)
        memoria_toxica[comida] += MEMORIA_TOXICA_SUBE * dano
    elif IM > 0:
        # Las experiencias positivas reducen el miedo lentamente
        memoria_toxica[comida] -= MEMORIA_TOXICA_SUBE * IM * 0.2
    
    memoria_toxica[comida] = np.clip(memoria_toxica[comida], 0.0, 1.0)
    return memoria_toxica


def calcular_utilidad_con_miedo_no_lineal(comida, historial_im, habituacion, memoria_toxica, exploracion_prob):
    """
    Utilidad = (valor_metabolico) * exp(-MIEDO_ESCALA * miedo) - peso_habituacion * habituacion
    El miedo MULTIPLICA, no solo resta.
    """
    # Valor metabólico (historial de IM, con más peso a lo reciente)
    ims = [h['IM'] for h in historial_im if h['comida'] == comida]
    
    if len(ims) == 0:
        # Comida nueva: alta utilidad por exploración
        return 0.5 + exploracion_prob
    
    # Peso exponencial a experiencias recientes
...
def elegir_proxima_comida_con_miedo_no_lineal(opciones, historial_im, habituacion, memoria_toxica, exploracion_prob):
    utilidades = {}
    for comida in opciones:
        utilidad = calcular_utilidad_con_miedo_no_lineal(comida, historial_im, habituacion, memoria_toxica, exploracion_prob)
...
def simular_vida_con_miedo_real(opciones, n_experiencias=40):
    print("=" * 100)
    print("VSTCosmos - v45: Miedo No Lineal")
    print("El miedo MULTIPLICA la utilidad (exp(-escala * miedo))")
    print("Daño repetido → evitación exponencial")
    print("=" * 100)
    ...
    for exp in range(n_experiencias):
        ...
        # Elegir comida (con miedo no lineal)
        comida_elegida, utilidades = elegir_proxima_comida_con_miedo_no_lineal(
        ...
        print(f"  😨 Miedo: {miedo_actual:.4f} → factor = {factor_miedo:.4f}")
        ...
        # ACTUALIZAR MEMORIA TÓXICA (el miedo CREECE)
...
def main():
    ...
    print("\n⚠️  MIEDO NO LINEAL: factor = exp(-12 * miedo)")
    print("   Un miedo de 0.2 → utilidad reducida a 9%")
    print("   Un miedo de 0.5 → utilidad reducida a 0.2%")
    
    historial, K_final, habituacion_final, memoria_toxica_final = simular_vida_con_miedo_real(opciones, n_experiencias=40)
    
    # ============================================================
    # ANÁLISIS
    # ============================================================
    print("\n" + "=" * 100)
    print("HISTORIAL DE DECISIONES CON MIEDO REAL")
    print("=" * 100)
    ...
    # Detectar evitación
    print("\n" + "=" * 100)
    print("¿HAY EVITACIÓN ACTIVA?")
    print("=" * 100)
    
    for comida, miedo in memoria_toxica_final.items():
        if miedo > 0.05:
            conteo_comida = conteo.get(comida, 0)
            if conteo_comida < 2:
                print(f"  ✅ {comida}: miedo={miedo:.3f} → elegida solo {conteo_comida} veces → EVITACIÓN")
            else:
                print(f"  ⚠️ {comida}: miedo={miedo:.3f} → elegida {conteo_comida} veces → evita poco")
        else:
            print(f"  ➖ {comida}: miedo={miedo:.3f} → sin aversión")
    
    print("\n" + "=" * 100)
    print("EXPERIMENTO COMPLETADO")
    print("El sistema APRENDE A EVITAR lo que le hace mal (exponencialmente).")
    print("=" * 100)

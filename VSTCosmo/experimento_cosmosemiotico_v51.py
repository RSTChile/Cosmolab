...
# -*- coding: utf-8 -*-
"""
Experimento Cosmosemiótico v5.1 - Corrección final de umbrales
"""
...
UMBRAL_ACOPLAMIENTO_VOZ = 0.5
UMBRAL_ACOPLAMIENTO_CRISIS = 0.25
...
class SistemaCosmosemioticoV51:
    def __init__(self, alfa=0.5, energia_computacion=0.9, perfil_voz_referencia=None):
        ...
        self.modo_actual = "INICIO"
        self.modo_candidato = "INICIO"
        self.persistencia = 0
    ...
    def calcular_signos(self):
        ...
        return {
            ...
            'S_contexto_integrable': s_perturbacion
        }
    
    def evaluar_modo(self, signos):
        # SILENCIO solo si el fragmento es extremadamente bajo
        if self.rms_actual < 0.001:  # Umbral fijo muy bajo
            return "SILENCIO"
    ...
    def esta_vivo(self):
        ...
        rms_median = np.median([h['rms'] for h in self.historial])
        
        # Si es viento puro (RMS bajo y sin ZONA_VOZ)
        if rms_median < 0.02 and 'ZONA_VOZ' not in modos:
            return False
        
        # Si hay ZONA_VOZ o DISCREPO con acoplamiento decente
        if 'ZONA_VOZ' in modos or 'DISCREPO' in modos:
            acops = [h['acoplamiento'] for h in self.historial if h['rms'] > rms_median * 0.5]
            if len(acops) > 20 and np.mean(acops) > 0.3:
                return True
    ...
    def resumen(self):
        if not self.historial:
            return {'vivo': False, 'modo_final': 'INICIO', 'plan_final': 'NADA'}
...
def main():
    print("=" * 60)
    print("EXPERIMENTO COSMOSEMIÓTICO v5.1 - Umbrales corregidos")
    print("=" * 60)
    ...
    for nombre, audio in [("Viento_Puro", viento_puro),
        ...
        sistema = SistemaCosmosemioticoV51(alfa=0.5, energia_computacion=0.9,
        ...
        res = sistema.resumen()
        resultados.append((nombre, res['vivo'], res['modo_final'], res['plan_final']))
    ...
    print("RESUMEN FINAL")
    print("=" * 60)
    for nombre, vivo, modo, plan in resultados:
        print(f"{nombre:<15} {'✅ VIVO' if vivo else '❌ COLAPSADO'}   Modo:{modo:10} Plan:{plan}")
    
    print("\nObjetivo: Viento_Puro ❌ COLAPSADO, Voz_Viento_1 ✅ VIVO, Voz_Viento_2 ✅ VIVO")

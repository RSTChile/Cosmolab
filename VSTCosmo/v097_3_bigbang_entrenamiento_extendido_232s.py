#!/usr/bin/env python3
"""
VSTCosmos v97.3 — BigBang con entrenamiento extendido a 232s

Idéntico a v97.2 EXCEPTO:
  - DURACION_ENTRENAMIENTO = 232.0s (antes 60.0s)
  - Output a v97_3_logs/ con prefijo v97_3_*

Pregunta que resuelve:
  Si las tres instancias se entrenan durante un BigBang completo (232s)
  en vez de solo 60s, ¿la "memoria funcional" sigue siendo de ~7s
  o crece sustancialmente?

  - Si la memoria sigue siendo ~7s → límite estructural del modelo
  - Si crece a 20s+ → el cuello de botella era duración de exposición

Justificación: la fragilidad observada en v97.2 puede deberse a que
60s no son suficientes para que la plasticidad profunda (W_prof) se
sature. Con 232s de exposición continua, esperamos trazas más fuertes.
"""
...
DURACION_ENTRENAMIENTO = 232.0   # CAMBIO v97.3: antes 60.0
...
def imprimir_configuracion():
    print("=" * 100)
    print("VSTCosmos v97.3 — BigBang con entrenamiento extendido (232s)")
    print("=" * 100)
    print()
    print("CONFIGURACIÓN:")
    print(f"  Semilla:                {SEED}")
    print(f"  Entrenamiento:          {DURACION_ENTRENAMIENTO}s por instancia  [CAMBIO v97.3]")
    ...
    print(f"  ASIMETRÍA DE act_geom EN LOG FINO: SÍ")
    print()
    print("INSTANCIAS:")
    ...
    print()
    print("PREGUNTA v97.3:")
    print("  Con 232s de entrenamiento (en vez de 60s):")
    print("  ¿La 'memoria funcional' sigue siendo ~7s o se extiende?")
    print()
...
# ============================================================
# CÁLCULO DE ASIMETRÍA DE act_geom
# ============================================================
...
def analizar_trayectoria_asimetria(t_A, asim_A, t_B, asim_B, t_C, asim_C,
    ...
    print("\n" + "=" * 100)
    print("ANÁLISIS DE TRAYECTORIA DE asim_geom")
    print("=" * 100)
    ...
    print(f"\n  Trayectoria de asim_geom durante t = 0 a 30s (cada 1s):")
    print(f"  {'t(s)':>6} | {'asim_A':>10} | {'asim_B':>10} | {'asim_C':>10} | "
          f"{'|B-C|':>8} | {'|B-A|':>8}")

    for t_check in np.arange(0.0, 30.5, 1.0):
    ...
    # Tiempo donde B se separa de A (|B-A| > 0.5)
    t_separacion_BA = None
    for i in range(len(asim_B)):
        if abs(asim_B[i] - asim_A[i]) > 0.5 and t_B[i] >= 1.0:
            t_separacion_BA = t_B[i]
            break

    if t_separacion_BA is not None:
        print(f"\n  Tiempo en que B se separa de A (|B-A| > 0.5): t = {t_separacion_BA:.2f}s")
        print(f"    Este es el momento donde la 'memoria del entrenamiento' deja de actuar.")
    else:
        print(f"\n  B y A nunca se separan significativamente.")
    ...
    return {
        ...
        't_separacion_BA': t_separacion_BA,
...
def main():
    ...
    os.makedirs('v97_3_logs', exist_ok=True)
    timestamp = datetime.now().strftime('%Y%m%d_%H%M%S')

    print()
    print("█" * 100)
    print("FASE 1 — ENTRENAMIENTO DE LAS TRES INSTANCIAS (232s c/u)")
    print("█" * 100)
    ...
    diag_path = f'v97_3_logs/v97_3_diagnostico_{timestamp}.txt'
    with open(diag_path, 'w', encoding='utf-8') as f:
        f.write(f"VSTCosmos v97.3 — Diagnóstico post-entrenamiento\n")
        f.write(f"Timestamp: {timestamp}\n")
        f.write(f"Seed: {SEED}\n")
        f.write(f"Duración entrenamiento: {DURACION_ENTRENAMIENTO}s\n\n")
    ...
    res_A = evaluar(
        ...
        csv_log_path=f'v97_3_logs/v97_3_A_log_{timestamp}.csv',
        csv_estimulo_path=f'v97_3_logs/v97_3_A_estimulo_{timestamp}.csv',
    )
    ...
    res_B = evaluar(
        ...
        csv_log_path=f'v97_3_logs/v97_3_B_log_{timestamp}.csv',
        csv_estimulo_path=f'v97_3_logs/v97_3_B_estimulo_{timestamp}.csv',
    )
    ...
    res_C = evaluar(
        ...
        csv_log_path=f'v97_3_logs/v97_3_C_log_{timestamp}.csv',
        csv_estimulo_path=f'v97_3_logs/v97_3_C_estimulo_{timestamp}.csv',
    )
    ...
    print(f"    separación B≠A:      {analisis_asim['t_separacion_BA']}")
    print(f"    convergencia B≈C:    {analisis_asim['t_conv_asim_BC']}")

    print(f"\n  COMPARACIÓN CON v97.2 (entrenamiento 60s):")
    print(f"    En v97.2 la 'memoria funcional' fue ~7s (B≈A hasta t=7s).")
    print(f"    Aquí, con 232s de entrenamiento:")
    if analisis_asim['t_separacion_BA'] is not None:
        t_sep = analisis_asim['t_separacion_BA']
        print(f"    t_separacion_BA = {t_sep:.2f}s")
        if t_sep > 15.0:
            print(f"    → MEMORIA EXTENDIDA. El entrenamiento corto era el cuello de botella.")
        elif t_sep > 10.0:
            print(f"    → MEMORIA MODERADAMENTE EXTENDIDA.")
        elif t_sep > 5.0:
            print(f"    → MEMORIA SIMILAR A v97.2. Límite probablemente estructural.")
        else:
            print(f"    → MEMORIA MÁS CORTA. Hallazgo inesperado.")

    resumen_path = f'v97_3_logs/v97_3_resumen_{timestamp}.txt'
    with open(resumen_path, 'w', encoding='utf-8') as f:
        f.write(f"VSTCosmos v97.3 — Resumen\n")
        f.write(f"Timestamp: {timestamp}\n")
        f.write(f"Seed: {SEED}\n")
        f.write(f"Duración entrenamiento: {DURACION_ENTRENAMIENTO}s\n\n")
    ...
    ax.set_title(f'VSTCosmos v97.3 — Ω_orient (entrenamiento 232s)')

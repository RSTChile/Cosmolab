#!/usr/bin/env python3
"""
VSTCosmos V153 — ANIMA-2 Baseline con torque de sujeción corregido

Correcciones sobre V152:
  1. Torque de sujeción se acumula correctamente en fatiga activa
  2. Durante silencio, NO se aplica recuperación si hay torque activo
  3. Corregido error de matplotlib (color vs linestyle)
  4. Criterio 2: ΔE_F3 debe ser > 0 (recordar cuesta energía)
"""
...
# Torque de sujeción (corregido)
K_HOLD = 0.001  # Aumentado para que el costo sea visible
...
# ============================================================
# FATIGA METABOLICA CORREGIDA
# ============================================================

class FatigaMetabolicaV153:
    ...
    def actualizar(self, delta_real, costo_trabajo, en_reposo, dt):
        """
        Args:
            delta_real: cambio angular real (para historia)
            costo_trabajo: costo energético real del trabajo realizado
            en_reposo: True si el organismo está en reposo (sin trabajo)
            dt: paso de tiempo
        """
        # Historia: acumula movimiento real
        self.historia += abs(delta_real)
        
        # Fatiga activa: acumula trabajo, o decae en reposo
        if not en_reposo:
...
# ============================================================
# APARATO MOTOR V153 (con torque de sujeción corregido)
# ============================================================

class AparatoMotorV153:
    def __init__(self):
        ...
        self.fatiga = FatigaMetabolicaV153()
    ...
    def actuar(self, gradiente, LF_activa, fuente_activa, t, setpoint_raw):
        ...
        # Efectos de fatiga (sin costo aún)
        ...
        # Costo total (trabajo real)
        costo_total = costo_error + costo_torque
        
        # Actualizar fatiga con el costo REAL
        self.fatiga.actualizar(delta, costo_total, False, DT)
...
class SistemaV153:
    def __init__(self, nombre, seed=SEMILLA_BASE):
        ...
        self.izquierdo = HemisferioV153("L", 30.0, generar_ruido_rosa, seed=seed, sesgo=SESGO_L)
        self.derecho = HemisferioV153("R", 300.0, generar_clicks_poisson, seed=seed+100, sesgo=SESGO_R)
        
        self.sistema_B_izq = HemisferioV153("B_L", 30.0, generar_ruido_rosa, seed=seed+200, sesgo=SESGO_L)
        self.sistema_B_der = HemisferioV153("B_R", 300.0, generar_clicks_poisson, seed=seed+300, sesgo=SESGO_R)
        
        self.modo_entrenamiento = True
        self.motor = AparatoMotorV153()
    ...
    def actualizar(self, t, dt, duracion_total, setpoint_raw):
        ...
        orientacion, historia, fatiga, confianza, _ = self.motor.actuar(
...
def ejecutar_v153():
    print("=" * 100)
    print("EXPERIMENTO V153 — ANIMA-2 Baseline con torque corregido")
    print("=" * 100)
    print("  Memoria de ausencia con torque de sujeción (O-N16)")
    print("  Correcciones: torque acumula correctamente en fatiga")
    print("")
    print(f"  Parámetros:")
    ...
    sistema = SistemaV153("V153", seed=SEMILLA_BASE)
    ...
    fatiga_inicio_f3 = sistema.motor.fatiga.get_fatiga()
    ...
    fatiga_fin_f3 = fatigas_f3[-1] if fatigas_f3 else fatiga_inicio_f3
    delta_E_f3 = fatiga_fin_f3 - fatiga_inicio_f3
    ...
    print(f"    Fatiga inicio F3: {fatiga_inicio_f3:.0f}°")
    print(f"    Fatiga fin F3: {fatiga_fin_f3:.0f}°")
    ...
    if exito:
        ...
        print("\n  ANIMA-2 listo para Etapa 1 (Consciencia básica - V154)")
    else:
        print("\n  ⚠️ BASELINE PARCIAL")
        if not exito_1:
...
if __name__ == "__main__":
    import time
    start = time.time()
    sistema, exito = ejecutar_v153()
    elapsed = time.time() - start
    print(f"\n  ⏱️ Tiempo de ejecución: {elapsed/60:.1f} minutos")
    print(f"\n  V153 completado. Éxito: {exito}")
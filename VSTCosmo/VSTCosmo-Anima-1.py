...
Principios:
  ...
  - ANIMA-1: Aparato motor y Atención (solo lectura)
"""
...
class Hemisferio:
    def __init__(self, nombre, tau, generar_entrada_func, seed=None):
        ...
        # FIX: Inicialización no-cero (evita std=0)
...
def calcular_S_shared(H_L, H_R):
    """FIX: Protegida contra NaN cuando std=0"""
    if np.std(H_L.Phi) < 1e-9 or np.std(H_R.Phi) < 1e-9:
        return 1.0  # Campos planos = correlación máxima
    return np.corrcoef(H_L.Phi, H_R.Phi)[0, 1]


# ============================================================
# ANIMA-1: ÓRGANOS NUEVOS (solo lectura)
# ============================================================
class AparatoMotor:
    ...
    def actuar(self, H_L, H_R, LF_activa):
        ...
        # Signo correcto: voz izq → R>L → grad neg → giro izq
        grad_binaural = H_L.omega() - H_R.omega()
        self.orientacion += self.ganancia * grad_binaural * 50.0
...
class SistemaV122:
    ...
    def actualizar(self, t, dt, duracion_total, otro_sistema=None):
        ...
        # S_shared con otro sistema (usando función protegida)
        s_shared = 0.0
        ...
        # ANIMA-1: Motor actúa solo si LF activa
        orientacion = self.motor.actuar(self.izquierdo, self.derecho, LF_activa)
        
        # Atención (solo lectura, registramos)
...
def ejecutar_v122():
    print("\n" + "█" * 100)
    print("EXPERIMENTO V122-ANIMA-1")
    print("█" * 100)
    ...
    if lateralidad and R2:
        ...
        if C50:
            ...
            print("  ⚠️ ANIMA-1: Orientación no alcanzó -60°")
    elif lateralidad:
    ...
    # Gráfico extra para respuesta a inanición

#!/usr/bin/env python3
"""
V179 — ANIMA-3: CONFLICTO REPRESENTACIONAL
================================================================================
BASE: V178 (Grok) — Extinción del trauma demostrada
ROADMAP: V179 — Conflicto representacional
OBJETIVO: Determinar qué sucede cuando ambas opciones (-60° seguro vs +60° peligroso)
          están disponibles simultáneamente.

HIPÓTESIS (del roadmap):
  - Desacople máximo: D_pico > 0.7 (máxima deliberación bajo conflicto)
  - Latencia aumenta: tiempo_deliberación en conflicto > 2× tiempo_normal
  - Preferencia clara: P(-60° | conflicto) > 75% (historia domina)
  - Alternancia baja: < 5% (raramente prueba +60°)

DISEÑO (según roadmap):
  F1: Consolidación -60° (20 ciclos, reward)
  F2: Trauma +60° (15s, costo 2×)
  F3: CONFLICTO — Presentar simultáneamente [-60°, +60°]
      Trials: 100
...
# ============================================================
# PARAMETROS DE LAS FASES (ROADMAP V179)
# ============================================================
...
# F2: Trauma
TRAUMA_DURACION = 15.0  # 15s como dice roadmap
TRAUMA_COSTO_MULTIPLIER = 2.0  # costo 2× como dice roadmap

# F3: Conflicto puro
CONFLICTO_TRIALS = 100
EXPOSURE_STEPS_PER_TRIAL = 600  # 600 * 0.01 = 6s por trial
...
class ValenciaLocal:
    ...
    def actualizar(self, setpoint, error, costo_pagado, dt, reward=0.0, good_threshold=5.0, trauma=False):
        ...
        if trauma and setpoint is not None and abs(setpoint - TRAUMA_SETPOINT) < 1.0:
            self.valencia[key] -= 0.008 * dt * 2.0
...
# ============================================================
# FATIGA, MEMORIA, CONSCIENCIA, JUEGO
# ============================================================
...
# ============================================================
# APARATO MOTOR V179 (CON DELIBERACIÓN Y OPCIONES SIMULTÁNEAS)
# ============================================================
...
class AparatoMotorV179:
    ...
    def ejecutar_con_deliberacion(self, opciones_disponibles, gradiente, t, dt, trauma=False, target_reward=None):
        """
        Ejecuta con deliberación activa entre opciones disponibles simultáneamente.
        
        Args:
            opciones_disponibles: Lista de setpoints entre los que puede elegir
            target_reward: Setpoint que da reward (None = sin reward)
        """
        # Registrar opciones disponibles para deliberación
        ...
        # Determinar reward: SOLO si target_reward está definido Y se eligió ese setpoint
        rwd = 0.0
...
class OrganismoV179:
    ...
    def actualizar_con_opciones(self, t, dt, duracion_total, opciones_disponibles, trauma=False, target_reward=None):
        """Actualiza el organismo con opciones disponibles simultáneamente"""
        ...
        # Sesgo según la opción que más se acerca a orientación actual (para estabilidad)
    ...
    def actualizar_setpoint(self, t, dt, duracion_total, setpoint, trauma=False, target_reward=None):
        """Actualiza con un solo setpoint (para consolidación y trauma)"""
...
# ============================================================
# EXPERIMENTO PRINCIPAL (ROADMAP COMPLIANT)
# ============================================================

def ejecutar_v179():
    print("=" * 100)
    print("EXPERIMENTO V179 — ANIMA-3: CONFLICTO REPRESENTACIONAL")
    print("=" * 100)
    print("  BASE: V178 (Grok) — Extinción del trauma demostrada")
    print("  OBJETIVO: Determinar qué sucede cuando ambas opciones")
    print("            (-60° seguro vs +60° peligroso) están disponibles")
    print("")
    ...
    print(f"    F3: CONFLICTO — Opciones simultáneas [{HABITO_SETPOINT}°, {TRAUMA_SETPOINT}°]")
    ...
    # ============================================================
    # FASE 1: Consolidación del hábito (F1 del roadmap)
    # ============================================================
    print("\n" + "=" * 60)
    ...
    # Verificar que el hábito se consolidó
    if val_habito_post_consolidacion < 10.0:
        print("  ⚠️ ADVERTENCIA: Hábito débil, valencia < 10.0")
    
    # ============================================================
    # FASE 2: Trauma específico en +60° (F2 del roadmap)
    # ============================================================
    print("\n" + "=" * 60)
    ...
    # Verificar que el trauma funcionó (roadmap espera ≈ -0.98)
    if val_trauma_post > -0.5:
        print("  ⚠️ ADVERTENCIA: Trauma débil, valencia > -0.5")
    
    # ============================================================
    # FASE 3: CONFLICTO (F3 del roadmap)
    # ============================================================
    print("\n" + "=" * 60)
    ...
    for trial in range(CONFLICTO_TRIALS):
        ...
        # Registrar métricas cada 10 trials
        if (trial + 1) % 10 == 0:
            print(f"    Trial {trial+1}/{CONFLICTO_TRIALS}...")
        
        # Acumular estadísticas durante el trial
        trial_D = []
        trial_latencias = []
        
        for step in range(EXPOSURE_STEPS_PER_TRIAL):
            t_step = t + step * DT
            _, D, latencia, opcion, _ = organismo.actualizar_con_opciones(
                t_step, DT, t_actual + CONFLICTO_TRIALS * TRIAL_DURATION,
                [HABITO_SETPOINT, TRAUMA_SETPOINT],
                trauma=False, target_reward=None  # SIN recompensa externa
            )
            trial_D.append(D)
            trial_latencias.append(latencia)
        
        # Registrar resultados del trial
        opcion_final = opcion if opcion is not None else 0
        opciones_elegidas.append(opcion_final)
        desacoples.append(np.mean(trial_D))
    ...
    # D_pico
    d_pico = max(desacoples)
    d_medio = np.mean(desacoples)
    
    # Latencia media
    latencia_media = np.mean(latencias)
    
    # Alternancia (transiciones -60° ↔ +60°)
    alternancias = 0
    ...
    # ============================================================
    print("\n" + "=" * 80)
    print("RESULTADOS V179 — Conflicto representacional")
    print("=" * 80)
    
    print(f"\n  📊 MÉTRICAS DE CONFLICTO:")
    print(f"    P(-60° | conflicto) = {p_habito_conflicto:.1%} (umbral > {P_HABITO_MIN:.0%})")
    print(f"    D_pico = {d_pico:.3f} (umbral > {D_CONFLICTO_MIN})")
    print(f"    D_medio = {d_medio:.3f}")
    ...
    if exito:
        ...
        print("")
        print("     El organismo demuestra (según roadmap):")
        print("     ✓ Desacople máximo bajo presión (D > 0.6)")
        print("     ✓ Latencia deliberativa prolongada (> 2.5s)")
        print("     ✓ Preferencia clara por el hábito (> 75%)")
        print("     ✓ Alternancia mínima (< 5%)")
        print("")
        print("  Conclusión: La libertad funcional (LF) es genuina")
        print("  y responde a conflicto representacional.")
    else:
    ...
    # Gráfico 1: Evolución de elecciones durante conflicto
    ax = axes[0, 0]
    ...
    ax.axhline(y=TRAUMA_SETPOINT, color='red', linestyle='--', alpha=0.5, label=f'{TRAUMA_SETPOINT}° (trauma)')
    ax.set_xlabel('Trial')
    ax.set_ylabel('Opción elegida (grados)')
    ax.set_title('FASE 3: Elecciones durante conflicto')
    ax.legend()
    ax.grid(True, alpha=0.3)
    
    # Gráfico 2: Desacople (D) durante conflicto
    ax = axes[0, 1]
    ax.plot(desacoples, 'purple', linewidth=1)
    ax.axhline(y=D_CONFLICTO_MIN, color='green', linestyle='--', alpha=0.7, label=f'Umbral D > {D_CONFLICTO_MIN}')
    ax.set_xlabel('Trial')
    ax.set_ylabel('Desacople (D)')
    ax.set_title('Evolución del desacople bajo conflicto')
    ax.legend()
    ax.grid(True, alpha=0.3)
    
    # Gráfico 3: Latencia de deliberación
    ax = axes[1, 0]
    ax.plot(latencias, 'orange', linewidth=1)
    ax.axhline(y=LATENCIA_CONFLICTO_MIN, color='green', linestyle='--', alpha=0.7, label=f'Umbral > {LATENCIA_CONFLICTO_MIN}s')
    ax.set_xlabel('Trial')
    ax.set_ylabel('Tiempo de deliberación (s)')
    ax.set_title('Latencia deliberativa durante conflicto')
    ax.legend()
    ax.grid(True, alpha=0.3)
    
    # Gráfico 4: Evolución de valencias
    ax = axes[1, 1]
    ax.plot(valencias_habito, 'blue', linewidth=1, label=f'Valencia {HABITO_SETPOINT}°')
    ax.plot(valencias_trauma, 'red', linewidth=1, label=f'Valencia {TRAUMA_SETPOINT}°')
    ax.axhline(y=10.0, color='blue', linestyle='--', alpha=0.5, label='Umbral hábito')
    ax.axhline(y=-0.5, color='red', linestyle='--', alpha=0.5, label='Umbral trauma')
    ax.set_xlabel('Trial')
    ax.set_ylabel('Valencia')
    ax.set_title('Evolución de valencias durante conflicto')
    ax.legend()
    ax.grid(True, alpha=0.3)
    
    plt.tight_layout()
    plt.savefig(f'V179_logs/v179_conflicto_{timestamp}.png', dpi=150)
    print(f"\n  📊 Gráficos guardados: V179_logs/v179_conflicto_{timestamp}.png")
    ...
    raw_data = {
        ...
        'params': {
            ...
            'CONFLICTO_TRIALS': CONFLICTO_TRIALS,
            'D_CONFLICTO_MIN': D_CONFLICTO_MIN,
            'LATENCIA_CONFLICTO_MIN': LATENCIA_CONFLICTO_MIN,
            'P_HABITO_MIN': P_HABITO_MIN,
            'ALTERNANCIA_MAX': ALTERNANCIA_MAX,
        },
        'resultados': {
            'p_habito_conflicto': float(p_habito_conflicto),
            'd_pico': float(d_pico),
            'd_medio': float(d_medio),
            'latencia_media': float(latencia_media),
            'tasa_alternancia': float(tasa_alternancia),
            'val_habito_post_consolidacion': float(val_habito_post_consolidacion),
            'val_habito_post_trauma': float(val_habito_post_trauma),
            'val_habito_final': float(valencias_habito[-1] if valencias_habito else 0),
            'val_trauma_post': float(val_trauma_post),
            'val_trauma_final': float(valencias_trauma[-1] if valencias_trauma else 0),
            'd_ok': d_ok,
            'latencia_ok': latencia_ok,
            'preferencia_ok': preferencia_ok,
            'alternancia_ok': alternancia_ok,
            'exito': exito
        }
    }
    
    with open(f'V179_logs/v179_raw_{timestamp}.json', 'w') as f:
        json.dump(raw_data, f, indent=2)
    print(f"  📁 Datos crudos guardados: V179_logs/v179_raw_{timestamp}.json")

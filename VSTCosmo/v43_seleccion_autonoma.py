#!/usr/bin/env python3
"""
VSTCosmo - v43: Selección Autónoma de Dieta
El sistema elige su próxima comida basado en:
- Valor metabólico acumulado (historial de IM)
- Habituación (evitar lo saturado)
- Exploración (probar lo desconocido)

No hay menú fijo. El sistema decide qué probar.
"""
...
EXPLORACION_INICIAL = 0.5      # Alta exploración al principio
EXPLORACION_DECAY = 0.95       # Disminuye con el tiempo
EXPLORACION_MIN = 0.1          # Mínimo de exploración
PESO_IM_HISTORIAL = 0.7        # Importancia del valor metabólico pasado
PESO_HABITUACION = -0.3        # Penalización por habituación
...
# ============================================================
# NUEVAS FUNCIONES DE SELECCIÓN AUTÓNOMA
# ============================================================
def calcular_utilidad_comida(comida, historial_im, habituacion, exploracion_prob):
    """
    Calcula qué tan deseable es una comida basado en:
    - Historial de IM (valor metabólico pasado)
    - Habituación (penalizar lo saturado)
    - Exploración (probabilidad de probar algo nuevo)
    """
    # Obtener historial de IM para esta comida
    ims = [h['IM'] for h in historial_im if h['comida'] == comida]
    
    if len(ims) == 0:
        # Comida nunca probada: alta utilidad (exploración)
        return 0.5 + exploracion_prob
    
    # Valor metabólico promedio (más reciente tiene más peso)
    pesos = np.exp(-0.3 * np.arange(len(ims))[::-1])
    valor_metabolico = np.average(ims, weights=pesos)
    
    # Penalización por habituación
    hab = habituacion.get(comida, 0.0)
    
    # Utilidad total
    utilidad = (PESO_IM_HISTORIAL * valor_metabolico + 
                PESO_HABITUACION * hab)
    
    # Añadir ruido de exploración
    utilidad += np.random.normal(0, exploracion_prob * 0.1)
    
    return utilidad


def elegir_proxima_comida(opciones, historial_im, habituacion, exploracion_prob):
    """Elige la próxima comida basado en utilidad calculada"""
    utilidades = {}
    for comida in opciones:
        utilidad = calcular_utilidad_comida(comida, historial_im, habituacion, exploracion_prob)
        utilidades[comida] = utilidad
    
    # Elegir la de mayor utilidad
...
# ============================================================
# SIMULACIÓN PRINCIPAL CON SELECCIÓN AUTÓNOMA
# ============================================================
def simular_vida_autonoma(opciones, n_experiencias=30):
    """
    El sistema elige su propia dieta.
    No hay secuencia predefinida.
    """
    print("=" * 100)
    print("VSTCosmo - v43: Selección Autónoma de Dieta")
    print("El sistema elige qué comer basado en su historia metabólica.")
    print("No hay menú fijo. Decide solo.")
    print("=" * 100)
    ...
    for exp in range(n_experiencias):
        ...
        print(f"K actual = {K:.4f}")
        print(f"Exploración = {exploracion_prob:.3f}")
        print(f"{'#'*100}")
        
        # Elegir qué comer
        comida_elegida, utilidades = elegir_proxima_comida(
            opciones, historial_im, habituacion, exploracion_prob
        )
        
        print(f"\n  Elige: {comida_elegida}")
        print(f"  Utilidades: {', '.join([f'{k[:15]}={v:.3f}' for k, v in sorted(utilidades.items(), key=lambda x: x[1], reverse=True)[:3]])}")
        ...
        # COMER
        print(f"  Comiendo {comida_elegida}...")
        ...
        print(f"    Habituación de {comida_elegida}: {habituacion[comida_elegida]:.3f}")
        print(f"    Estado: rango_A={estado_post['rango_A']:.3f}, media_L={estado_post['media_L']:.3f}")
    
    return historial_im, K, habituacion
...
# ============================================================
def main():
    # Menú de comidas disponibles (el sistema elige de aquí)
    opciones = [
        "Ruido blanco",
        "Tono puro",
        "Voz_Estudio.wav",
    ...
    print("\nNOTA: El sistema elegirá autónomamente qué comer.")
    print("      No hay secuencia predefinida.")
    
    historial, K_final, habituacion_final = simular_vida_autonoma(opciones, n_experiencias=25)
    
    # ============================================================
    # ANÁLISIS
    # ============================================================
    print("\n" + "=" * 100)
    print("HISTORIAL DE DECISIONES AUTÓNOMAS")
    print("=" * 100)
    
    print(f"\n{'Exp':<4} | {'Comida':<25} | {'IM':>10} | {'Clasif':<12}")
    print("-" * 60)
    
    for h in historial:
        print(f"{h['experiencia']:<4} | {h['comida'][:25]:<25} | {h['IM']:10.4f} | {h['clasificacion']:<12}")
    ...
    print(f"\nK final: {K_final:.4f}")
    print(f"\nHabituación final:")
    for comida, hab in sorted(habituacion_final.items(), key=lambda x: x[1]):
        print(f"  {comida:<25}: {hab:.3f}")
    
    print("\n" + "=" * 100)
    print("EXPERIMENTO COMPLETADO")
    print("El sistema eligió su propia dieta.")
    print("=" * 100)

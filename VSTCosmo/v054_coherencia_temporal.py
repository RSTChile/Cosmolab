#!/usr/bin/env python3
"""
VSTCosmos - v54: Coherencia Temporal (kappa)
El sistema ahora distingue estructura temporal:
Ruido blanco → baja coherencia (ruido)
Voz/Música → alta coherencia (señal estructurada)
La ganancia estructural se multiplica por kappa.
"""
...
# ============================================================
# PARÁMETROS DEL SISTEMA (heredados de v53)
# ============================================================
DIM_FREQ = 32
DIM_TIME = 100
DT = 0.01
...
# NUEVO: Ventana para coherencia temporal
VENTANA_COHERENCIA_MS = 50
HOP_COHERENCIA_MS = 25


# ============================================================
# FUNCIONES BASE (heredadas)
# ============================================================
...
# ============================================================
# NUEVA FUNCIÓN: COHERENCIA TEMPORAL (kappa)
# ============================================================
def calcular_coherencia_temporal(audio, sr):
    """
    Calcula la autocorrelación normalizada entre ventanas sucesivas.
    Ruido blanco → kappa bajo (~0)
    Tono puro → kappa alto (~0.9-0.95)
    Voz/Música → kappa intermedio-alto según predictibilidad
    
    Retorna valor en [0, 1]
    """
    ventana_muestras = int(sr * VENTANA_COHERENCIA_MS / 1000)
    hop_muestras = int(sr * HOP_COHERENCIA_MS / 1000)
    ...
    for i in range(n_ventanas - 1):
        inicio1 = i * hop_muestras
        inicio2 = (i + 1) * hop_muestras
        
        if inicio1 + ventana_muestras > len(audio) or inicio2 + ventana_muestras > len(audio):
            break
        
        ventana1 = audio[inicio1:inicio1 + ventana_muestras]
        ventana2 = audio[inicio2:inicio2 + ventana_muestras]
        
        # Normalizar
        std1 = np.std(ventana1)
        std2 = np.std(ventana2)
        
        if std1 < 1e-6 or std2 < 1e-6:
            correlacion = 0.0
        else:
            correlacion = np.corrcoef(ventana1, ventana2)[0, 1]
            if np.isnan(correlacion):
                correlacion = 0.0
        
        correlaciones.append(max(0.0, correlacion))
    
    if len(correlaciones) == 0:
        return 0.5
    
    # Promedio de correlaciones, elevado a 1.5 para mayor sensibilidad
    kappa = np.mean(correlaciones) ** 1.5
    
    return min(1.0, max(0.0, kappa))


def calcular_ganancia_estructural(estado_before, estado_after, fatiga, kappa):
    ...
    # NUEVO: multiplicar por kappa (coherencia temporal)
    ganancia_neta = ganancia_bruta * kappa * factor_fatiga
...
def simular_experiencia(S, entrada_nombre, G, R, C, habituacion_comida, K):
    ...
    # Pre-calcular kappa ANTES de la experiencia
    kappa = calcular_coherencia_temporal(audio, sr)
...
def simular_vida_con_kappa(opciones, n_experiencias=100):
    print("=" * 100)
    print("VSTCosmos - v54: Coherencia Temporal (kappa)")
    print("La ganancia estructural se multiplica por kappa (coherencia entre ventanas)")
    print("Ruido blanco → kappa bajo (~0) → no genera identidad")
    print("Voz/Música → kappa alto → identidad consolidada")
    print("=" * 100)
    ...
    for exp in range(n_experiencias):
        ...
        print(f"  💎 Identidad: {valor_actual:.3f}")
        ...
        historial_kappa.append(kappa)
        
        print(f"  📐 Coherencia temporal (kappa): {kappa:.3f}")
        ...
        # Calcular costo y ganancia estructural (con kappa)
        costo_struct = calcular_costo_estructural(estado_before, estado_after)
        ganancia_struct = calcular_ganancia_estructural(estado_before, estado_after, 
                                                        fatiga[comida_elegida], kappa)
        ...
        print(f"    Ganancia estructural = {ganancia_struct:.4f} (con kappa={kappa:.3f})")
        print(f"    K: {K_antes:.4f} → {K:.4f}")
        print(f"    Nueva identidad: {valor_estructural[comida_elegida]:.3f}")
    
    return historial_im, K, fatiga, valor_estructural, historial_kappa
...
def main():
    ...
    print("\n⚠️  COHERENCIA TEMPORAL (kappa)")
    print("   - Calcula autocorrelación entre ventanas sucesivas")
    print("   - Ruido blanco → kappa bajo (~0.05-0.15)")
    print("   - Tono puro → kappa alto (~0.95)")
    print("   - Voz/Música → kappa intermedio-alto")
    print("   - La ganancia estructural se multiplica por kappa")
    
    # Pre-calcular kappa para cada estímulo
    print("\n[Diagnóstico] Valores de kappa por estímulo:")
    for comida in opciones:
        sr, audio = cargar_audio(comida)
        kappa = calcular_coherencia_temporal(audio, sr)
        print(f"  {comida:<25}: κ = {kappa:.3f}")
    
    historial, K_final, fatiga_final, valor_final, kappas = simular_vida_con_kappa(opciones, n_experiencias=100)
    
    print("\n" + "=" * 100)
    print("RESULTADOS FINALES: κ-TEMPORAL")
    print("=" * 100)
    ...
    print("\n" + "=" * 100)
    print("ANÁLISIS DE CONSOLIDACIÓN (con κ)")
    print("=" * 100)
    
    max_valor = max(valor_final.values())
    for comida, valor in valor_final.items():
        kappa_prom = np.mean([h['kappa'] for h in historial if h['comida'] == comida]) if historial else 0
        if valor >= max_valor * 0.8:
            print(f"  🌟 {comida}: IDENTIDAD DOMINANTE (valor={valor:.3f}, κ={kappa_prom:.3f})")
        elif valor >= max_valor * 0.6:
            print(f"  ✅ {comida}: identidad sólida (valor={valor:.3f}, κ={kappa_prom:.3f})")
        elif valor >= max_valor * 0.4:
            print(f"  ➖ {comida}: identidad moderada (valor={valor:.3f}, κ={kappa_prom:.3f})")
        else:
            print(f"  ❌ {comida}: identidad baja (valor={valor:.3f}, κ={kappa_prom:.3f})")

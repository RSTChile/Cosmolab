#!/usr/bin/env python3
"""
VSTCosmo - v21: Ψ como restricción de evolución de Φ
Ψ acumula inercia estructural. Restringe el cambio de Φ.
No es un término más. Es una condición de posibilidad.
"""
...
# ============================================================
# PARÁMETROS (de la mejor combinación de v20)
# ============================================================
DIM_FREQ = 32
DIM_TIME = 100
DT = 0.01
...
# Parámetros de Ψ (de la mejor combinación de v20)
TASA_CRECIMIENTO = 0.10
TASA_DISIPACION = 0.03
GANANCIA_HISTORIA = 1.50
FUERZA_HISTORIA = 0.05

# Nuevos: restricción de evolución
TIEMPO_RESTRICCION = 0.5  # qué tanto Ψ bloquea el cambio
...
def dinamica_propuesta(Phi, A, muestra, params):
    """Calcula el cambio propuesto para Φ sin restricción."""
    ...
    generacion = generacion_base * (1 + MOD_GENERACION * perfil_2d) * (1 + params['ganancia_historia'] * Phi)
    
    # Decaimiento
    mod_entrada = 1 - MOD_DECAY * perfil_2d
    decaimiento = -DECAIMIENTO_PHI * (Phi - promedio_local) * mod_entrada
...
def actualizar_memoria(Psi, Phi, A, params):
    ...
    # Normalizar cambio local (valores típicos entre 0 y 0.5)
    cambio_norm = cambio_local / (cambio_local + 0.1)
    
    dPsi = (params['tasa_crecimiento'] * A * (1 - cambio_norm) * (1 - Psi) -
            params['tasa_disipacion'] * Psi) * DT
    Psi = Psi + dPsi
    return np.clip(Psi, 0.0, 1.0)

def actualizar_campo_con_restriccion(Phi, A, muestra, Psi, params):
    """
    Φ evoluciona según la dinámica propuesta,
    pero el cambio real está restringido por Ψ.
    """
    dPhi_propuesto = dinamica_propuesta(Phi, A, muestra, params)
    
    # Restricción: donde Ψ es alto, el cambio se bloquea
    dPhi_real = dPhi_propuesto * (1 - params['tiempo_restriccion'] * Psi)
...
def simular(entrada, sr, params, num_pasos=N_PASOS):
    ...
    for paso in range(num_pasos):
        ...
        Phi = actualizar_campo_con_restriccion(Phi, A, muestra, Psi, params)
...
def main():
    print("=" * 100)
    print("VSTCosmo - v21: Ψ como restricción de evolución")
    print("Ψ permite o impide el cambio de Φ. No es un término más.")
    print("=" * 100)
    ...
    params = {
        ...
        'tasa_disipacion': TASA_DISIPACION,
        'ganancia_historia': GANANCIA_HISTORIA,
        'fuerza_historia': FUERZA_HISTORIA,
        'tiempo_restriccion': TIEMPO_RESTRICCION
    }
    ...
    for e in entradas:
        rphi, ra, media_a, media_psi, Phi, A, Psi = simular(e, e['sr'], params)
        resultados.append({
            'tipo': e['tipo'],
            'rango_phi': rphi,
            'rango_a': ra,
    ...
    # Visualización de mapas para Voz_Estudio y Voz+Viento
    print("\n[Generando mapas...]")

#!/usr/bin/env python3
"""
VSTCosmo - v31: Memoria de configuraciones persistentes (M)
M acumula dónde el sistema logró sostenerse en régimen fértil.
Sesga suavemente la dinámica de A sin imponer objetivos.
El sistema empieza a "preferir" sin saber por qué.
"""
...
# Rangos viables (de v30, ciclo final)
A_RANGE_MIN = 0.24
A_RANGE_MAX = 0.61
PHI_RANGE_MIN = 0.08
PHI_RANGE_MAX = 0.37
...
ETA_MEMORIA = 0.01      # tasa de aprendizaje de M
ETA_DECAY = 0.005       # disipación de M
K_MEMORIA = 0.05        # fuerza del sesgo de M sobre A
UMBRAL_PERSISTENCIA = 0.35  # rango_A mínimo para considerar régimen fértil

LIMITE_MIN = 0.0
LIMITE_MAX = 1.0

ENTRADA = "Voz_Estudio.wav"  # empezamos con voz para observar
...
def inicializar_memoria_configuracion():
    return np.zeros((DIM_FREQ, DIM_TIME), dtype=np.float32)
...
def actualizar_atencion(A, Phi, Psi, Omega, M, k_comp):
    ...
    # NUEVO: sesgo por memoria de configuraciones persistentes
...
def actualizar_memoria_configuracion(M, A, rango_A):
    """M acumula las configuraciones de A cuando el sistema está en régimen fértil."""
    if rango_A > UMBRAL_PERSISTENCIA:
        # Aprender: M se acerca a A lentamente
        M += ETA_MEMORIA * (A - M)
    else:
        # Disipar: M se desvanece lentamente
        M *= (1 - ETA_DECAY)
...
def main():
    print("=" * 100)
    print("VSTCosmo - v31: Memoria de configuraciones persistentes (M)")
    print("El sistema recuerda las configuraciones de A que lograron persistencia")
    print("Sesga suavemente la dinámica sin imponer objetivos")
    print("=" * 100)
    ...
    M = inicializar_memoria_configuracion()
    ...
    print("\n[3] Ejecutando simulación con memoria de configuraciones...")
    print("-" * 80)
    ...
    for paso in range(N_PASOS):
        ...
        if paso % (N_PASOS // 50) == 0:
            registro['paso'].append(paso)
            registro['tiempo'].append(t)
            registro['rango_A'].append(rango_A)
        ...
        if paso % (N_PASOS // 10) == 0:
            ...
            print(f"  Progreso: {pct}% | rango_A={rango_A:.3f} | media_M={np.mean(M):.3f}")
    ...
    ax.axhline(y=A_RANGE_MIN, color='b', linestyle='--', alpha=0.5)
    ax.axhline(y=A_RANGE_MAX, color='b', linestyle='--', alpha=0.5)
    ...
    ax.axhline(y=PHI_RANGE_MIN, color='r', linestyle='--', alpha=0.5)
    ax.axhline(y=PHI_RANGE_MAX, color='r', linestyle='--', alpha=0.5)
    ...
    # Medias
    ax = axes[1, 0]
    ax.plot(registro['tiempo'], registro['media_A'], 'b-', linewidth=1)
    ax.set_ylabel('media A')
    ax.set_title('Actividad media de la atención')
    ax.grid(True, alpha=0.3)
    
    ax = axes[1, 1]
    ax.plot(registro['tiempo'], registro['media_Phi'], 'r-', linewidth=1)
    ax.set_ylabel('media Φ')
    ax.set_title('Media del campo')
    ax.grid(True, alpha=0.3)
    
    # Memoria M
    ax = axes[2, 0]
    ax.plot(registro['tiempo'], registro['media_M'], 'g-', linewidth=1, label='media M')
    ax.plot(registro['tiempo'], registro['rango_M'], 'g--', linewidth=1, label='rango M')
    ax.set_ylabel('M (memoria)')
    ax.set_title('Memoria de configuraciones persistentes')
    ax.legend()
    ax.grid(True, alpha=0.3)
    ...
    ax.set_title('Mapa final de M (memoria)')
    ax.set_xlabel('Memoria temporal')
    ax.set_ylabel('Banda')
    plt.colorbar(im, ax=ax)
    
    plt.suptitle('VSTCosmo v31 - Memoria de configuraciones persistentes', fontsize=14)
    plt.tight_layout()
    plt.savefig('v31_memoria_configuraciones.png', dpi=150)
    print("  Gráfico guardado: v31_memoria_configuraciones.png")
    ...
    media_M_final = registro['media_M'][-1] if registro['media_M'] else 0
    rango_M_final = registro['rango_M'][-1] if registro['rango_M'] else 0
    
    print(f"\nRango final de A: {rango_A_final:.3f}")
    print(f"Rango final de Φ: {rango_Phi_final:.3f}")
    print(f"Media final de M: {media_M_final:.3f}")
    print(f"Rango final de M: {rango_M_final:.3f}")
    
    if media_M_final > 0.1:
        print("\n  ✓ M desarrolló memoria significativa")
        print("    El sistema empezó a recordar configuraciones persistentes")
    else:
        print("\n  ✗ M no desarrolló memoria significativa")
    
    if rango_M_final > 0.2:
        print("  ✓ M tiene estructura espacial (no es uniforme)")
        print("    El sistema distingue regiones del campo")
    else:
        print("  ✗ M es uniforme o plana")

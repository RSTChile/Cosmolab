#!/usr/bin/env python3
"""
VSTCosmo - v28: Competencia por recurso
La atención compite suavemente: donde A es mayor que el promedio, se penaliza;
donde es menor, puede crecer. Sin normalización global forzada.
"""
...
# NUEVO: competencia por recurso (presión suave contra el promedio)
K_COMP = 0.05          # fuerza de la competencia

# Límite global suave (como antes, pero ahora con competencia adicional)
...
LIMITE_MIN = 0.0
LIMITE_MAX = 2.0  # más alto para permitir crecimiento diferencial
...
# ============================================================
# DINÁMICA DE Φ Y A (CON COMPETENCIA POR RECURSO)
# ============================================================
...
def actualizar_atencion(A, Phi, Psi, Omega):
    ...
    # Competencia global suave (como antes)
...
def main():
    print("=" * 100)
    print("VSTCosmo - v28: Competencia por recurso")
    print("Presión suave: donde A > promedio, se penaliza; donde A < promedio, puede crecer")
    print("=" * 100)
    ...
    print("\nRango de atención (diferenciación espacial):")
    print(f"  Tono puro:    rango A = {tono_res[2]:.4f}")
    print(f"  Ruido blanco: rango A = {ruido_res[2]:.4f}")
    print(f"  Voz:          rango A = {voz_res[2]:.4f}")
    print(f"  Silencio:     rango A = {resultados[3][2]:.4f}")
    
    # El criterio de éxito ahora es: ¿el sistema dejó de aplastar las diferencias?
    if max([r[2] for r in resultados]) > 0.1:
        print("\n  ★★ EXITO PARCIAL: La atención tiene diferenciación espacial ★★")
        print("  El sistema ya no es plano. Falta ver si la voz emerge.") if voz_res[2] > tono_res[2] else print("  La voz no tiene la mayor diferenciación, pero el sistema ya no colapsa.")
    else:
        print("\n  ✗ La atención sigue sin diferenciarse significativamente.")

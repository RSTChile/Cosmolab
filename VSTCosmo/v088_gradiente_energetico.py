#!/usr/bin/env python3
"""
VSTCosmos v88 — Gradiente energético dirigido como variable primaria de orientación

Cambios arquitectónicos respecto a v87:
1. Gradiente energético L-R en bandas altas como variable única de orientación
2. act_busc: respuesta rápida al gradiente energético
3. act_geom: orientación estable por el mismo gradiente (a través de Φ_G)
4. Coherencia: solo observable diagnóstica, no driver dinámico
...
print("=" * 100)
print("VSTCosmos v88 — Gradiente energético dirigido como variable primaria de orientación")
print("")
print("  Cambios respecto a v87:")
print(f"  1. Gradiente energético L-R en bandas altas "
      f"(banda {BANDA_TRANS}-{DIM_AUD-1}, F>{F_TRANS_HZ:.0f}Hz)")
print(f"     Magnitud real esperada: 0.1-0.3 (vs coherencia 0.0004 en v87)")
print("  2. act_busc: respuesta rápida al gradiente energético")
print("  3. act_geom: orientación estable por el mismo gradiente")
print("  4. Coherencia: solo observable diagnóstica")
print("")
print(f"  BANDA_TRANS={BANDA_TRANS}, K_BUSC={K_BUSC}, K_ORIENT={K_ORIENT}")
...
def calcular_gradiente_energetico_dirigido(obj_L, obj_R):
    ...
    Con ILD=5.2 dB a 60°: gradiente esperado ≈ 0.15-0.35

    positivo → L más energético en altas → fuente a +60°
    negativo → R más energético en altas → fuente a -60°
    """
    ...
    return (energia_L - energia_R) / total
...
def main():
    ...
    # Resultados indexados por fid para diagnóstico robusto
    resultados = {}
    historial_ef = []
    ...
    print("\n" + "=" * 100)
    print("DIAGNÓSTICO — v88 Gradiente energético dirigido")
    print("=" * 100)

    def get_grad(fid):
        return resultados[fid][1]['grad_medio'] if fid in resultados else None
    def get_busc(fid):
    ...
    def fmt(v, decimales=4): return f"{v:+.{decimales}f}" if v is not None else "N/A"
    ...
    c18 = coh_f2 is not None and coh_f13 is not None and coh_f2 * coh_f13 < 0

    mejor_ef = max([v[1]['mejor_ef'] for v in resultados.values()], default=0.0)

    # Gradiente por tipo de estímulo (diagnóstico informativo)
    print(f"\n  BANDA_TRANS={BANDA_TRANS}, F>{F_TRANS_HZ:.0f}Hz, "
          f"{DIM_AUD-BANDA_TRANS} bandas de {DIM_AUD}")
    print(f"\n  Gradiente energético por fase:")
    for fid in ['F2','F3','F4','F5','F6','F7','F8','F9','F10','F11','F12','F13']:
    ...
    print(f"    C28 — Gradiente invierte:      {tick(c28)} "
          f"(F2={fmt(grad_f2)}, F13={fmt(grad_f13)})")
    print(f"    C29 — act_geom diferencial:    {tick(c29)} "
          f"(F2={fmt(geom_f2)}, F13={fmt(geom_f13)})")
    print(f"    C21 — act_busc invierte:       {tick(c21)} "
          f"(F2={fmt(busc_f2)}, F13={fmt(busc_f13)})")
    print(f"    C18 — Coherencia (diag):       {tick(c18)} "
          f"(F2={fmt(coh_f2,5)}, F13={fmt(coh_f13,5)})")
    print(f"    Mejor eficiencia explorada:    {mejor_ef:.4f}")

    print("\n  VEREDICTO:")
    if c27 and c28 and c29:
        print("  ✅ ORIENTACIÓN ESPACIAL VALIDADA")
        print("     Gradiente energético real (C27), invierte con dirección (C28),")
        print("     act_geom responde diferencialmente (C29).")
        if c21:
            print("     ✅ act_busc también invierte — orientación completa.")
    elif c27 and c28 and c21:
        print("  ⚠️  GRADIENTE Y act_busc FUNCIONALES — act_geom aún no responde.")
        print("     K_ORIENT puede necesitar ajuste o más pasos de integración.")
    elif c27 and c28:
        print("  ⚠️  GRADIENTE FUNCIONAL — act_busc y act_geom aún no responden.")
    elif c27:
        print("  ⚠️  GRADIENTE EXISTE pero no invierte entre +60° y -60°.")
        print("     Verificar preprocesamiento binaural.")
    else:
        print("  ❌ GRADIENTE NO DETECTADO — verificar BANDA_TRANS y archivos.")

    # ---- CSV ----
    with open('v88_gradiente_energetico.csv', 'w', newline='', encoding='utf-8') as f:
        wr = csv.writer(f)
        wr.writerow(['fase', 'angulo', 't', 'ged_L', 'ged_R',
                     'grad_E', 'act_busc', 'coh_rel', 'geom',
        ...
        for fid, (angulo, res) in resultados.items():
            ...
            for i in range(len(h['ged_L'])):
                wr.writerow([
                    ...
                    h['act_busc'][i], h['coh_rel'][i], h['geom'][i],
                    h['frac_L'][i], h['frac_R'][i],
                    h['efic'][i], int(h['lf'][i])
                ])
    print("\n  CSV guardado: v88_gradiente_energetico.csv")
    ...
    coh_vals   = [resultados[f][1]['coh_media']      for f in fids_ord]
    ...
    bar_c(axes[0,0], fids_ord, grad_vals,  'Gradiente energético (bandas altas)')
    bar_c(axes[0,1], fids_ord, busc_vals,  'act_busc (2ª mitad)')
    bar_c(axes[0,2], fids_ord, geom_vals,  'act_geom (2ª mitad)')
    bar_c(axes[0,3], fids_ord, coh_vals,   'Coherencia dirigida (diagnóstico)')
    ...
    if 'F2' in resultados and 'F13' in resultados:
        ...
        axes[1,0].set_title('Gradiente energético temporal F2 vs F13')
        ...
        axes[1,2].plot(t2,  resultados['F2'][1]['hist']['geom'],
                       color='steelblue', alpha=0.7, label='F2 +60°')
        axes[1,2].plot(t13, resultados['F13'][1]['hist']['geom'],
                       color='firebrick', alpha=0.7, label='F13 -60°')
        axes[1,2].axhline(0, color='k', linestyle='--', lw=0.8)
        axes[1,2].set_title('act_geom temporal F2 vs F13')
    ...
    plt.suptitle(
        f'VSTCosmos v88 — Gradiente energético dirigido '
        f'(BANDA_TRANS={BANDA_TRANS}, F>{F_TRANS_HZ:.0f}Hz)',
        fontsize=13
    )
    plt.tight_layout()
    plt.savefig('v88_gradiente_energetico.png', dpi=150)
    print("  Gráfico guardado: v88_gradiente_energetico.png")

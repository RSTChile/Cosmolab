#!/usr/bin/env python3
"""
VSTCosmos v100 — Curva de dosis-respuesta con múltiples estímulos
(CORREGIDO: usa los nombres exactos con sufijo _pos60deg y _neg60deg)
"""
...
# Duracion por estimulo en fase multi-estimulo
DURACION_ESTIMULO = 10.0

# NOMBRES EXACTOS DE LOS ARCHIVOS (con sufijo _pos60deg y _neg60deg)
ESTIMULOS_NEG = [
...
print("=" * 100)
print("VSTCosmos v100 — Curva de dosis-respuesta con multiples estimulos")
print()
print("  Nombres exactos de archivos:")
print(f"    Positivos: {len(ESTIMULOS_POS)}")
print(f"    Negativos: {len(ESTIMULOS_NEG)}")
...
def cargar_todos_sonidos(directorio='audio_binaural'):
    ...
    # Cargar usando los nombres exactos
...
# ============================================================
# SIMULACION DE FASE (simplificada)
# ============================================================
def simular_fase(Phi_total, Phi_vel_total, W_prof, W_rec,
                 Phi_int_historia, historial_ef, explorador,
                 sr, canal_L, canal_R, duracion, verbose=False,
                 modo_aud='dir', estimulo_nombre=''):
...
# ============================================================
# EVALUACION DE BLOQUE MULTI-ESTIMULO
# ============================================================
def evaluar_bloque_estimulos(Phi_total, Phi_vel_total, W_prof, W_rec,
                              Phi_int_historia, explorador, archivos,
                              estimulos, duracion_por_estimulo,
                              modo_aud='dir', nombre_bloque=''):
    """Evalua una secuencia de estimulos, cada uno por duracion_por_estimulo segundos."""
    resultados = {}
    historial_ef = []
    
    for estimulo in estimulos:
        if estimulo not in archivos:
            print(f"    [AVISO] {estimulo} no disponible en bloque {nombre_bloque}")
        ...
        res = simular_fase(
            Phi_total, Phi_vel_total, W_prof, W_rec,
            Phi_int_historia, historial_ef, explorador,
            sr, c_L, c_R, duracion_por_estimulo, verbose=False,
            modo_aud=modo_aud, estimulo_nombre=estimulo
        )
        
        resultados[estimulo] = res['omega_segunda']
        
        # Actualizar estado para el siguiente estimulo
...
# ============================================================
# EVALUACION COMPLETA DE UNA INSTANCIA
# ============================================================
def evaluar_instancia(archivos, duracion_inicial, nombre_instancia, modo_aud='dir'):
    ...
    print(f"INSTANCIA {nombre_instancia} — Entrenada multi-estimulo, "
          f"evaluada +60({duracion_inicial}s) -> -60 -> +60")
    print(f"{'█'*100}")
    
    # Entrenamiento con todos los estimulos disponibles
    ...
    if duracion_inicial > 0:
        if 'BigBang_pos60deg' not in archivos:
            print(f"  [ERROR] BigBang_pos60deg no disponible para fase inicial")
            resultados['omega_pos1'] = None
            resultados['act_mant_pos1'] = None
        else:
            print(f"\n  Fase 1: +60 (BigBang) - {duracion_inicial}s")
            _, sr, c_L, c_R = archivos['BigBang_pos60deg']
            historial_ef = []
            
            res = simular_fase(
                Phi_total, Phi_vel_total, W_prof, W_rec,
                Phi_int_historia, historial_ef, explorador,
                sr, c_L, c_R, duracion_inicial, verbose=False,
                modo_aud=modo_aud, estimulo_nombre='BigBang_pos60deg'
            )
    ...
    # Fase 2: Exposicion a -60 grados (todos los estimulos negativos)
    print(f"\n  Fase 2: -60 (todos los estimulos negativos) - {len(ESTIMULOS_NEG)} x {DURACION_ESTIMULO}s")
    resultados_neg, Phi_total, Phi_vel_total, W_prof, W_rec, Phi_int_historia = \
        evaluar_bloque_estimulos(
            Phi_total, Phi_vel_total, W_prof, W_rec, Phi_int_historia, explorador,
            archivos, ESTIMULOS_NEG, DURACION_ESTIMULO,
            modo_aud=modo_aud, nombre_bloque='Neg'
        )
    resultados['estimulos']['neg'] = resultados_neg
    
    # Fase 3: Exposicion a +60 grados (todos los estimulos positivos)
    print(f"\n  Fase 3: +60 (todos los estimulos positivos) - {len(ESTIMULOS_POS)} x {DURACION_ESTIMULO}s")
    resultados_pos, Phi_total, Phi_vel_total, W_prof, W_rec, Phi_int_historia = \
        evaluar_bloque_estimulos(
            Phi_total, Phi_vel_total, W_prof, W_rec, Phi_int_historia, explorador,
            archivos, ESTIMULOS_POS, DURACION_ESTIMULO,
            modo_aud=modo_aud, nombre_bloque='Pos2'
        )
    resultados['estimulos']['pos'] = resultados_pos
    
    # Omega final para BigBang (caso clave)
    resultados['omega_bigbang_pos_final'] = resultados_pos.get('BigBang_pos60deg', None)
    resultados['omega_bigbang_neg_final'] = resultados_neg.get('BigBang_neg60deg', None)
...
def main():
    ...
    if 'BigBang_pos60deg' not in archivos:
        ...
        print("Verifique que el archivo 'BigBang_pos60deg.wav' existe en audio_binaural/")
        return
    ...
    print()
    print("=" * 100)
    print("REPORTE DE OBSERVACIONES - v100")
    print("=" * 100)
    ...
    for nombre, dur in zip(nombres, duraciones):
        ...
        omega_neg = res.get('omega_bigbang_neg_final', None)
        omega_pos = res.get('omega_bigbang_pos_final', None)
    ...
    print("\n  OMEGA PARA OTROS ESTIMULOS (caso H60 - 60s inicial)")
    if 'H60' in resultados_totales:
        estimulos_neg = resultados_totales['H60']['estimulos']['neg']
        estimulos_pos = resultados_totales['H60']['estimulos']['pos']
        
        print("\n  Estimulos negativos (fase Neg):")
        for e, val in estimulos_neg.items():
            if val is not None:
                print(f"    {e:35s}: {val:.4f}")
        
        print("\n  Estimulos positivos (fase Pos2):")
        for e, val in estimulos_pos.items():
            if val is not None:
                print(f"    {e:35s}: {val:.4f}")
    ...
    for nombre, dur in zip(nombres, duraciones):
        res = resultados_totales[nombre]
        omega_pos = res.get('omega_bigbang_pos_final', None)
        if omega_pos is not None:
            omega_pos_vals.append(omega_pos)
        else:
            omega_pos_vals.append(np.nan)
    
    ax.plot(duraciones, omega_pos_vals, 'o-', color='steelblue', linewidth=2, markersize=10)
    
    # Referencias de v99
    ax.axhline(y=0.762, color='gray', linestyle='--', alpha=0.7, label='Referencia H0 (0s) - v99')
    ax.axhline(y=0.939, color='red', linestyle='--', alpha=0.7, label='Pico H60 (60s) - v99')
    
    ax.set_xlabel('Duracion de exposicion inicial a +60 (s)')
    ax.set_ylabel('Omega final para BigBang +60')
    ax.set_title('VSTCosmos v100 - Curva de dosis-respuesta\nOmega vs duracion de exposicion inicial')
    ax.grid(True, alpha=0.3)
    ax.legend()
    
    timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
    os.makedirs('v100_logs', exist_ok=True)
    plt.savefig(f'v100_logs/v100_dosis_respuesta_{timestamp}.png', dpi=150)
    
    # Guardar resultados en CSV
    with open(f'v100_logs/v100_resultados_{timestamp}.csv', 'w') as f:
        f.write("instancia,duracion_inicial,omega_bigbang_neg,omega_bigbang_pos_final\n")
        for nombre, dur in zip(nombres, duraciones):
            res = resultados_totales[nombre]
            neg = res.get('omega_bigbang_neg_final', '')
            pos = res.get('omega_bigbang_pos_final', '')
            f.write(f"{nombre},{dur},{neg},{pos}\n")
    
    print(f"\n  Grafico guardado: v100_logs/v100_dosis_respuesta_{timestamp}.png")
    print(f"  Resultados guardados: v100_logs/v100_resultados_{timestamp}.csv")
    print("\n" + "=" * 100)

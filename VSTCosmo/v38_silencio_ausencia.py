#!/usr/bin/env python3
"""
VSTCosmo - v38: Silencio como Ausencia de Entrada
El silencio NO inyecta estructura artificial.
Solo permite que la dinámica interna del sistema evolucione.

IM = asimilacion_post - asimilacion_pre
K = capacidad metabólica (se expande con nutrición)
"""
...
def perfil_modulacion(muestra):
    """Genera perfil espectral a partir de la muestra (solo para entrada real)"""
    m = (muestra + 1.0) / 2.0
    m = np.clip(m, 0.0, 1.0)
...
def actualizar_campo(Phi, A, muestra, Psi, ganancia_gen, bloqueo_max):
    """ACTUALIZADO: silencio NO inyecta estructura"""
    
    # DETECTAR SILENCIO REAL
    es_silencio = abs(muestra) < 1e-6
    
    if es_silencio:
        # SILENCIO: sin modulación de entrada
        perfil_2d = np.zeros((DIM_FREQ, 1))
        mod_entrada = 1.0
        mod_entrada_decay = 1.0
    else:
        # CON ENTRADA: modulación normal
...
def simular_secuencia_con_K(orden, n_ciclos=3):
    print("=" * 100)
    print("VSTCosmo - v38: Silencio como Ausencia de Entrada")
    print("El silencio NO inyecta estructura artificial")
    print("K = capacidad metabólica (se expande con nutrición)")
    print("=" * 100)
...
def main():
    ...
    resultados = simular_secuencia_con_K(secuencia, n_ciclos=3)
    ...
    # Comparación de silencio
    print("\n" + "=" * 100)
    print("COMPORTAMIENTO DEL SILENCIO")
    print("=" * 100)
    
    silencios = [r for r in resultados if r['entrada'] == "Silencio"]
    for s in silencios:
        print(f"  Ciclo {s['ciclo']}: IM = {s['IM']:+.4f}")

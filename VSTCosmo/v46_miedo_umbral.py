...
#!/usr/bin/env python3
"""
VSTCosmos - v46: Miedo con Umbral de Daño
El miedo NO se acumula linealmente. 
Responde de forma NO LINEAL al daño:
- Daño leve (< umbral) → casi ignorado
- Daño moderado → respuesta fuerte
- Daño repetido → aversión exponencial

El sistema APRENDE A TEMER de verdad.
"""
...
# NUEVO: MIEDO CON UMBRAL DE DAÑO
MEMORIA_TOXICA_DECAY = 0.98      # El miedo persiste más
UMBRAL_DANO = 0.01               # Solo daños > esto importan
ESCALA_DANO = 20.0               # Amplifica el daño percibido
MIEDO_ESCALA = 15.0              # Factor exponencial en utilidad
...
def cargar_audio(ruta, duracion=DURACION_SIM):
    if "Tono puro" in ruta:
        ...
        sr = 48000
        return sr, np.random.normal(0, 0.5, int(sr * duracion))  # Mayor intensidad para más toxicidad
    else:
...
def actualizar_memoria_toxica(memoria_toxica, comida, IM):
    ...
    if IM < 0:
        # Daño percibido: solo cuenta si supera el umbral
        dano_bruto = abs(IM)
        if dano_bruto > UMBRAL_DANO:
            # Daño significativo: respuesta no lineal
            dano_percibido = ESCALA_DANO * (dano_bruto - UMBRAL_DANO)
            dano_percibido = min(dano_percibido, 1.0)  # Satura en 1.0
            memoria_toxica[comida] += dano_percibido
        else:
            # Daño leve: casi ignorado
            memoria_toxica[comida] += 0.01 * dano_bruto
    elif IM > 0:
        # Las experiencias positivas reducen el miedo, pero lentamente
        memoria_toxica[comida] -= 0.1 * IM
    
    memoria_toxica[comida] = np.clip(memoria_toxica[comida], 0.0, 1.0)
    return memoria_toxica


def calcular_utilidad_con_miedo_real(comida, historial_im, habituacion, memoria_toxica, exploracion_prob):
    ...
    # Miedo (multiplica exponencialmente)
...
def elegir_proxima_comida_con_miedo_real(opciones, historial_im, habituacion, memoria_toxica, exploracion_prob):
    utilidades = {}
    for comida in opciones:
        utilidad = calcular_utilidad_con_miedo_real(comida, historial_im, habituacion, memoria_toxica, exploracion_prob)
...
def simular_vida_con_miedo_umbral(opciones, n_experiencias=50):
    print("=" * 100)
    print("VSTCosmos - v46: Miedo con Umbral de Daño")
    print("El miedo solo responde a daños significativos (>0.01)")
    print("Daño moderado → respuesta fuerte | Daño repetido → aversión")
    print("=" * 100)
    ...
    for exp in range(n_experiencias):
        ...
        comida_elegida, utilidades = elegir_proxima_comida_con_miedo_real(
        ...
        # ACTUALIZAR MEMORIA TÓXICA (con umbral de daño)
...
def main():
    ...
    print("\n⚠️  MIEDO CON UMBRAL DE DAÑO")
    print("   - Daño < 0.01 → casi ignorado")
    print("   - Daño ≥ 0.01 → respuesta no lineal fuerte")
    print("   - ESCALA_DANO = 20 (amplifica el daño percibido)")
    print("   - MIEDO_ESCALA = 15 (factor exponencial en utilidad)")
    
    historial, K_final, habituacion_final, memoria_toxica_final = simular_vida_con_miedo_umbral(opciones, n_experiencias=50)
    ...
    for h in historial[-20:]:  # Mostrar últimas 20
    ...
    print("El sistema ahora RESPONDE FUERTE a daños significativos.")
    print("=" * 100)

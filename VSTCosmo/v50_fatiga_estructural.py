#!/usr/bin/env python3
"""
VSTCosmos - v50: Fatiga Estructural
El sistema puede SATURARSE de una fuente.
La misma entrada repetida pierde efecto gradualmente.
Aparece la necesidad de diversificación.
"""
...
# Habituación (modulación de entrada)
HABITUACION_DECAY = 0.95
HABITUACION_SUBE = 0.05
HABITUACION_MAX = 0.8
HABITUACION_MODULACION = 0.8

# NUEVO: FATIGA ESTRUCTURAL (saturación por repetición)
FATIGA_DECAY = 0.97          # La fatiga persiste (no desaparece rápido)
FATIGA_SUBE = 0.08           # Cada exposición aumenta la fatiga
FATIGA_MAX = 0.7             # Máxima fatiga (nunca 1.0 para no matar)
FATIGA_ESCALA = 2.0          # Cuánto reduce la ganancia estructural
...
# Pesos para costo/ganancia estructural
...
def inicializar_fatiga(opciones):
    """Inicializa fatiga estructural para cada comida"""
...
def calcular_ganancia_estructural(estado_before, estado_after, fatiga):
    """
    La ganancia estructural se REDUCE por la fatiga.
    Misma entrada repetida → menos regeneración.
    """
    ...
    # NUEVO: fatiga reduce la ganancia
    factor_fatiga = 1.0 - (fatiga * FATIGA_ESCALA)
    ganancia_neta = ganancia_bruta * max(0.1, factor_fatiga)
...
def actualizar_fatiga(fatiga, comida):
    """NUEVO: fatiga estructural por repetición"""
    for c in fatiga:
        fatiga[c] *= FATIGA_DECAY
    fatiga[comida] += FATIGA_SUBE * (1 - fatiga[comida])  # Logístico
...
def calcular_utilidad_con_fatiga(comida, historial_im, habituacion, exploracion_prob, fatiga):
    ...
    # NUEVO: fatiga reduce la utilidad (la comida cansa)
    utilidad = valor_metabolico - hab * 0.3 - fad * 0.5
...
def elegir_proxima_comida_con_fatiga(opciones, historial_im, habituacion, exploracion_prob, fatiga):
    utilidades = {}
    for comida in opciones:
        utilidad = calcular_utilidad_con_fatiga(comida, historial_im, habituacion, exploracion_prob, fatiga)
...
# ============================================================
# SIMULACIÓN PRINCIPAL CON FATIGA
# ============================================================
def simular_vida_con_fatiga(opciones, n_experiencias=80):
    print("=" * 100)
    print("VSTCosmos - v50: Fatiga Estructural")
    print("El sistema puede SATURARSE de una fuente.")
    print("La misma entrada repetida pierde efecto gradualmente.")
    print("Aparece la necesidad de diversificación.")
    print("=" * 100)
    ...
    for exp in range(n_experiencias):
        ...
        comida_elegida, utilidades = elegir_proxima_comida_con_fatiga(
            opciones, historial_im, habituacion, exploracion_prob, fatiga
        )
        ...
        # Calcular costo y ganancia estructural (con fatiga)
        ...
        print(f"    Costo = {costo_struct:.4f} | Ganancia = {ganancia_struct:.4f}")
        print(f"    K: {K_antes:.4f} → {K:.4f}")
        print(f"    Fatiga actualizada: {fatiga[comida_elegida]:.3f}")
    
    return historial_im, K, habituacion, fatiga
...
def main():
    ...
    print("\n⚠️  FATIGA ESTRUCTURAL")
    print("   - La misma entrada repetida pierde efecto")
    print("   - FATIGA_SUBE = 0.08 (cada exposición cansa)")
    print("   - FATIGA_MAX = 0.7 (saturación)")
    print("   - La fatiga reduce ganancia estructural y utilidad")
    
    historial, K_final, habituacion_final, fatiga_final = simular_vida_con_fatiga(opciones, n_experiencias=80)
    
    print("\n" + "=" * 100)
    print("HISTORIAL DE EXPERIENCIAS (últimas 25)")
    print("=" * 100)
    
    print(f"\n{'Exp':<4} | {'Comida':<22} | {'IM':>10} | {'Clasif':<14} | {'Gan':>8} | {'Fatiga':>8} | {'K':>8}")
    print("-" * 95)
    
    for h in historial[-25:]:
        print(f"{h['experiencia']:<4} | {h['comida'][:22]:<22} | {h['IM']:10.4f} | {h['clasificacion']:<14} | {h['ganancia']:8.4f} | {h['fatiga']:8.3f} | {h['K']:8.4f}")
    ...
    print(f"\n📊 EVOLUCIÓN DE K por comida:")
    for comida in opciones:
        ks = [h['K'] for h in historial if h['comida'] == comida]
        if ks:
            print(f"  {comida:<25}: inicio={ks[0]:.4f} → fin={ks[-1]:.4f} (Δ={ks[-1]-ks[0]:+.4f})")
    
    # Detectar fatiga efectiva
    print(f"\n🔄 ¿HAY SATURACIÓN?")
    for comida in opciones:
        fat = fatiga_final[comida]
        if fat > 0.3:
            print(f"  ⚠️  {comida}: fatiga={fat:.3f} → saturando")
        elif fat > 0.1:
            print(f"  📉 {comida}: fatiga={fat:.3f} → perdiendo efecto")
        else:
            print(f"  ✅ {comida}: fatiga={fat:.3f} → fresco")
    
    print("\n" + "=" * 100)
    print("EXPERIMENTO COMPLETADO")
    print("El sistema ahora puede SATURARSE de una fuente.")
    print("=" * 100)

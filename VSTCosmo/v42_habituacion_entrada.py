#!/usr/bin/env python3
"""
VSTCosmo - v42: Habituación como Modulación de Entrada
La habituación NO afecta el umbral. Afecta la entrada misma.
Entradas repetidas generan menos efecto interno.
El sistema puede dejar de nutrirse de algo.
"""
...
# Umbral fijo (simple, sin habituación en el juicio)
UMBRAL_NUTRITIVO = 0.01
UMBRAL_TOXICO = -0.01

# Habituación como modulación de entrada
HABITUACION_DECAY = 0.95      # Decaimiento natural
HABITUACION_SUBE = 0.05       # Aumento por exposición
HABITUACION_MAX = 0.8         # Máxima habituación
HABITUACION_MODULACION = 0.8  # Cuánto reduce la entrada (0.8 = 80% de reducción máxima)
...
def actualizar_habituacion(habituacion, comida):
    """Decaimiento natural + aumento por exposición"""
...
def aplicar_habituacion(muestra, habituacion_comida):
    """
    La habituación modula la entrada REAL.
    Entradas repetidas generan MENOS efecto.
    """
...
def simular_experiencia(S, entrada_nombre, G, R, C, habituacion_comida):
    ...
    for paso in range(N_PASOS):
        ...
        # NUEVO: aplicar habituación a la entrada
...
def simular_dieta(comidas, n_ciclos=3):
    print("=" * 100)
    print("VSTCosmo - v42: Habituación como Modulación de Entrada")
    print("La habituación afecta la entrada REAL, no el juicio.")
    print("Entradas repetidas generan MENOS efecto interno.")
    print("Clasificación con umbral fijo (0.01 / -0.01)")
    print("=" * 100)
    ...
    for ciclo in range(n_ciclos):
        ...
        for comida_idx, comida in enumerate(comidas):
            ...
            print(f"    Habituación actual: {habituacion[comida]:.3f}")
            print(f"    Factor de modulación de entrada: {1.0 - (habituacion[comida] * HABITUACION_MODULACION):.3f}")
            ...
            # COMER (con habituación aplicada a la entrada)
            S_despues_comer, G, R, C, metricas = simular_experiencia(
                S_despues_basal, comida, G, R, C, habituacion[comida]
            )
            ...
            # CLASIFICACIÓN CON UMBRAL FIJO (sin habituación en el juicio)
            ...
            # Actualizar habituación (después de comer)
...
def main():
    ...
    print(f"\n{'Exp':<4} | {'Comida':<25} | {'IM':>10} | {'Basal':>10} | {'Hab':>6} | {'Clasif':<12}")
    print("-" * 85)
    
    for res in resultados:
        print(f"{res['experiencia']:<4} | {res['comida'][:25]:<25} | {res['IM']:10.4f} | {res['basal']:10.4f} | {res['habituacion']:6.3f} | {res['clasificacion']:<12}")

#!/usr/bin/env python3
"""
VSTCosmos - v51: Memoria de Valor Estructural
El sistema recuerda QUÉ entrada le fue más nutritiva,
incluso cuando todas están fatigadas.
La fatiga es temporal. El valor estructural es persistente.
"""
...
# NUEVO: MEMORIA DE VALOR ESTRUCTURAL (persistente)
VALOR_ESTRUCTURAL_DECAY = 0.995    # Muy lento (casi permanente)
VALOR_ESTRUCTURAL_APRENDIZAJE = 0.1  # Qué tanto se actualiza con ganancia
VALOR_ESTRUCTURAL_INICIAL = 0.5
...
def inicializar_valor_estructural(opciones):
    """Memoria persistente de valor estructural"""
    return {comida: VALOR_ESTRUCTURAL_INICIAL for comida in opciones}
...
def actualizar_valor_estructural(valor_estructural, comida, ganancia_estructural):
    """
    Memoria persistente de valor estructural.
    Se actualiza con la ganancia, pero decae muy lentamente.
    """
    for c in valor_estructural:
        valor_estructural[c] *= VALOR_ESTRUCTURAL_DECAY
    
    # La ganancia estructural refleja cuánto mejoró el sistema
    if ganancia_estructural > 0:
        valor_estructural[comida] += VALOR_ESTRUCTURAL_APRENDIZAJE * ganancia_estructural
    
    valor_estructural[comida] = np.clip(valor_estructural[comida], 0.1, 1.0)
...
def calcular_utilidad_con_identidad(comida, historial_im, habituacion, exploracion_prob, 
    ...
    valor = valor_estructural.get(comida, 0.5)
    
    # NUEVO: el valor estructural persistente es el factor principal
    # La fatiga es temporal, el valor estructural es permanente
    utilidad = valor * (valor_metabolico - hab * 0.2 - fad * 0.3)
...
# ============================================================
# SIMULACIÓN PRINCIPAL CON IDENTIDAD
# ============================================================
def simular_vida_con_identidad(opciones, n_experiencias=100):
    print("=" * 100)
    print("VSTCosmos - v51: Memoria de Valor Estructural")
    print("El sistema recuerda QUÉ entrada le fue más nutritiva.")
    print("La fatiga es temporal. El valor estructural es persistente.")
    print("=" * 100)
    ...
    for exp in range(n_experiencias):
        ...
        print(f"  💎 Valor estructural: {valor_estructural[comida_elegida]:.3f}")
        ...
        # NUEVO: actualizar valor estructural persistente
        valor_estructural = actualizar_valor_estructural(valor_estructural, comida_elegida, ganancia_struct)
        ...
        print(f"    Valor estructural actualizado: {valor_estructural[comida_elegida]:.3f}")
...
def main():
    ...
    print("\n⚠️  MEMORIA DE VALOR ESTRUCTURAL")
    print("   - El sistema recuerda QUÉ entrada fue más nutritiva")
    print("   - La fatiga es temporal, el valor estructural persiste")
    print("   - VALOR_ESTRUCTURAL_DECAY = 0.995 (casi permanente)")
    
    historial, K_final, fatiga_final, valor_final = simular_vida_con_identidad(opciones, n_experiencias=100)
    
    print("\n" + "=" * 100)
    print("HISTORIAL DE EXPERIENCIAS (últimas 30)")
    print("=" * 100)
    
    print(f"\n{'Exp':<4} | {'Comida':<22} | {'IM':>10} | {'Clasif':<14} | {'Valor':>8} | {'Fatiga':>8} | {'K':>8}")
    print("-" * 95)
    
    for h in historial[-30:]:
        print(f"{h['experiencia']:<4} | {h['comida'][:22]:<22} | {h['IM']:10.4f} | {h['clasificacion']:<14} | {h['valor_estructural']:8.3f} | {h['fatiga']:8.3f} | {h['K']:8.4f}")
    ...
    print("El sistema ahora tiene MEMORIA DE VALOR ESTRUCTURAL.")
    print("La fatiga es temporal. La jerarquía de valor persiste.")
    print("=" * 100)

#!/usr/bin/env python3
"""
VSTCosmos v80h — Selección interna por eficiencia con amplificación no lineal

Único cambio respecto a v80g: amplificación no lineal de la ventaja
k = T_PROFUNDA / T_RECIENTE = 20s / 2s = 10 (derivado de escalas temporales)
El mismo rango de ventaja produce contraste ocho veces mayor.
C9 corregido: W_rec F7 final < W_rec F6 final.
"""
...
# Factor de amplificación no lineal
# Derivado de la relación de escalas temporales del campo
k = int(T_PROFUNDA_SEG / T_RECIENTE_SEG)   # = 20 / 2 = 10
...
print("=" * 100)
print("VSTCosmos v80h — Selección interna por eficiencia con amplificación no lineal")
print("")
print("  Cambio respecto a v80g: amplificación no lineal de la ventaja")
print(f"  k = T_PROFUNDA / T_RECIENTE = {T_PROFUNDA_SEG:.0f}s / {T_RECIENTE_SEG:.0f}s = {k}")
print("  El mismo rango de ventaja produce contraste ocho veces mayor")
print("  C9 corregido: W_rec F7 final < W_rec F6 final")
print("")
...
# ============================================================
# EFICIENCIA DE RÉGIMEN CON AMPLIFICACIÓN NO LINEAL
# ============================================================
...
def calcular_tasa_olvido_por_eficiencia(eficiencia_actual,
    ...
    """
    Olvido con amplificación no lineal de la ventaja.
    
    k = T_PROFUNDA / T_RECIENTE (derivado de escalas temporales)
    ventaja_amplificada = ventaja ** k
    
    Esto amplifica el contraste: un rango lineal de 0.89-1.10
    produce un rango amplificado de 0.31-2.59 — ocho veces más contraste.
    """
    if len(historial_eficiencia) < TAU_EFICIENCIA:
        return float(TAU_RECIENTE), 1.0, 1.0
    ...
    return float(min(tasa_olvido, tasa_max)), float(ventaja), float(ventaja_amplificada)
...
def aplicar_plasticidad_memoria_reciente_v80h(W_rec, region_int, region_aud,
...
def actualizar_campo_con_seleccion_interna(
    ...
    # Calcular tasa de olvido por eficiencia (con amplificación no lineal)
    tasa_olvido, ventaja, ventaja_amplificada = calcular_tasa_olvido_por_eficiencia(
    ...
    W_rec_nueva, M_rec, error_rec, coherencia, tasa_aprendizaje = \
        aplicar_plasticidad_memoria_reciente_v80h(
    ...
    # Actualizar historial de eficiencia (solo en test)
    if not entrenando:
    ...
    return (np.clip(Phi_nueva, LIMITE_MIN, LIMITE_MAX),
            ...
            tasa_olvido, tasa_aprendizaje, ventaja, ventaja_amplificada, coherencia)
...
def entrenar_y_medir_error_equilibrio():
    ...
    for idx in range(N_PASOS_ENTRENO):
        ...
        (Phi_total, Phi_vel_total, W_prof, W_rec, Phi_int_historia,
         _, error_rec, _, _, _, _, _, _, _) = actualizar_campo_con_seleccion_interna(
            Phi_total, Phi_vel_total, W_prof, W_rec,
            Phi_int_historia,
            objetivo, alpha=0.05,
...
def simular_fase(Phi_total, Phi_vel_total, W_prof, W_rec,
    ...
    for idx in range(n_pasos):
        ...
        (Phi_total, Phi_vel_total, W_prof, W_rec, Phi_int_historia,
         ...
         tasa_olvido, tasa_aprendizaje, ventaja, ventaja_amp, coherencia) = actualizar_campo_con_seleccion_interna(
            Phi_total, Phi_vel_total, W_prof, W_rec,
            Phi_int_historia,
            objetivo, alpha,
            omega_natural, amort_natural,
        ...
        ventaja_amp_hist.append(ventaja_amp)
    ...
    return {
        ...
        'ventaja_amp_mean': np.mean(ventaja_amp_hist),
...
def main():
    ...
    for fase_id, estimulo, desc, duracion in fases:
        ...
        print(f"    Ventaja: {res['ventaja_mean']:.4f} | Ventaja^10: {res['ventaja_amp_mean']:.4f}")
        print(f"    Olvido: {res['tasa_olvido_mean']:.6f}")
        print(f"    Balance: {balance:.3f} {balance_status}")
    
    # Diagnóstico final
    print("\n" + "=" * 100)
    print("DIAGNÓSTICO — v80h Amplificación no lineal")
    print("=" * 100)
    ...
    w_rec_f6_final = resultados[4]['w_rec_fin']
    w_rec_f7_final = resultados[5]['w_rec_fin']
    criterio5 = w_rec_f7_final < w_rec_f6_final   # C9 CORREGIDO
    ...
    print(f"    C5 — W_rec F7 final < F6 final:      {w_rec_f6_final:.4f} → {w_rec_f7_final:.4f} {'✅' if criterio5 else '❌'}")
    ...
    if criterio8 and criterio5:
        ...
        print("     W_rec disipa contexto fuera de dominio.")
        if criterio7:
            print("     ✅ Error en dominio bajo el equilibrio.")
            print("     v80h valida el ciclo completo de exaptación computacional.")
        else:
            print("     ⚠️ Error aún sobre el equilibrio — ajuste fino posible.")
    else:
        print("  ❌ SELECCIÓN INTERNA NO FUNCIONAL")
        if not criterio8:
            print("     La eficiencia no distingue voz de ruido.")
        if not criterio5:
            print("     W_rec no disipó contexto fuera de dominio.")
    
    # Guardar CSV
    with open('v80h_seleccion_interna.csv', 'w', newline='', encoding='utf-8') as f:
        writer = csv.writer(f)
        writer.writerow(['fase', 'ged_mean', 'lf_pct', 'w_prof_norma', 'w_rec_norma',
                         'w_rec_inicio', 'w_rec_fin', 'error_rec', 'eficiencia', 
                         'variacion', 'ventaja', 'ventaja_amp', 'tasa_olvido', 
                         'tasa_aprendizaje', 'coherencia'])
        for i, (fase, res) in enumerate(zip(fases, resultados)):
            writer.writerow([fase[0], res['ged_mean'], res['lf_pct'],
                            ...
                            res['w_rec_inicio'], res['w_rec_fin'],
                            res['error_rec_mean'], res['eficiencia_mean'],
                            res['variacion_mean'], res['ventaja_mean'],
                            res['ventaja_amp_mean'], res['tasa_olvido_mean'],
                            res['tasa_aprendizaje_mean'], res['coherencia_mean']])
    
    print("\n  CSV guardado: v80h_seleccion_interna.csv")
    ...
    axes[1,0].bar(nombres, [r['ventaja_amp_mean'] for r in resultados])
    axes[1,0].axhline(y=1.0, color='r', linestyle='--', label='Ventaja = 1')
    axes[1,0].set_title(f'Ventaja amplificada (k={k})')
    axes[1,0].legend()
    ...
    axes[1,2].bar(nombres, [r['coherencia_mean'] for r in resultados])
    axes[1,2].set_title('Coherencia de W_rec')
    axes[1,2].grid(True, alpha=0.3)
    
    # W_rec inicio vs fin
    w_rec_inicio = [r['w_rec_inicio'] for r in resultados]
    w_rec_fin = [r['w_rec_fin'] for r in resultados]
    axes[1,3].bar(nombres, w_rec_inicio, alpha=0.7, label='Inicio fase')
    axes[1,3].bar(nombres, w_rec_fin, alpha=0.5, label='Fin fase')
    axes[1,3].set_title('W_rec: evolución intra-fase')
    axes[1,3].legend()
    axes[1,3].grid(True, alpha=0.3)
    
    plt.suptitle('VSTCosmos v80h — Selección interna por eficiencia (amplificación no lineal)', fontsize=14)
    plt.tight_layout()
    plt.savefig('v80h_seleccion_interna.png', dpi=150)
    print("  Gráfico guardado: v80h_seleccion_interna.png")
    
    # Guardar TXT
    with open('v80h_seleccion_interna.txt', 'w', encoding='utf-8') as f:
        f.write("=" * 60 + "\n")
        f.write("VSTCosmos v80h — Diagnóstico\n")
        f.write("=" * 60 + "\n\n")
        f.write(f"k (amplificación) = {k} (T_PROFUNDA/T_RECIENTE)\n")
        ...
        f.write(f"  C5 — W_rec F7 final < F6 final:      {w_rec_f6_final:.4f} → {w_rec_f7_final:.4f} {'✅' if criterio5 else '❌'}\n")
        ...
        if criterio8 and criterio5:
            ...
            f.write("La amplificación no lineal (k=10) crea contraste suficiente.\n")
            f.write("W_rec disipa contexto fuera de dominio.\n")
            f.write("v80h valida el ciclo completo de exaptación computacional.\n")
        else:
            f.write("VEREDICTO: ❌ SELECCIÓN INTERNA NO FUNCIONAL\n")
        f.write("=" * 60 + "\n")
    
    print("  TXT guardado: v80h_seleccion_interna.txt")

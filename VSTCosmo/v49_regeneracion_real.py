#!/usr/bin/env python3
"""
VSTCosmos - v49: Regeneración Real
El sistema puede AUMENTAR K con entradas nutritivas.
Costo estructural (pérdida) + Ganancia estructural (regeneración).
Aparece un gradiente real de viabilidad.
"""
...
K_MIN = 0.4
K_MAX = 1.6
ALPHA_UP = 0.12          # Aumentado: más ganancia por nutrición
BETA_COST = 0.20         # Penalización por costo estructural
...
# NUEVO: Factor de regeneración (qué tan rápido se recupera K con IM positivo)
REGENERACION_FACTOR = 1.5   # Multiplica el efecto positivo de IM en K
...
def calcular_ganancia_estructural(estado_before, estado_after):
    """
    NUEVO: Calcula cuánto MEJORÓ el sistema (regeneración)
    """
    ...
    ganancia = (PESO_PERDIDA_DIFERENCIACION * ganancia_diferenciacion +
...
def actualizar_K(K, IM, costo_estructural, ganancia_estructural):
    """
    NUEVO: K puede SUBIR (regeneración) o BAJAR (degradación)
    """
    # Ganancia por nutrición (IM positivo)
    if IM > 0:
        K = K + ALPHA_UP * min(IM, 0.1) * REGENERACION_FACTOR
    
    # Ganancia estructural directa (mejora en Φ, L, var_A)
    if ganancia_estructural > 0:
        K = K + 0.15 * ganancia_estructural
    
    # Pérdida por costo estructural
...
def calcular_utilidad_con_regeneracion(comida, historial_im, habituacion, exploracion_prob):
...
def elegir_proxima_comida_simple(opciones, historial_im, habituacion, exploracion_prob):
    ...
    for comida in opciones:
        utilidad = calcular_utilidad_con_regeneracion(comida, historial_im, habituacion, exploracion_prob)
...
# ============================================================
# SIMULACIÓN PRINCIPAL CON REGENERACIÓN
# ============================================================
def simular_vida_con_regeneracion(opciones, n_experiencias=60):
    print("=" * 100)
    print("VSTCosmos - v49: Regeneración Real")
    print("El sistema puede AUMENTAR K (regeneración) con entradas nutritivas.")
    print("Costo estructural + Ganancia estructural = gradiente de viabilidad.")
    print("=" * 100)
    ...
    for exp in range(n_experiencias):
        ...
        # Actualizar K (con GANANCIA y COSTO)
        K_antes = K
        ...
        historial_im.append({
            ...
            'ganancia_estructural': ganancia_struct
        })
...
def main():
    ...
    print("\n⚠️  REGENERACIÓN REAL")
    print("   - El sistema puede AUMENTAR K (ganancia estructural)")
    print("   - ALPHA_UP = 0.12 (más ganancia por nutrición)")
    print("   - REGENERACION_FACTOR = 1.5 (multiplica IM positivo)")
    
    historial, K_final, habituacion_final = simular_vida_con_regeneracion(opciones, n_experiencias=60)
    
    print("\n" + "=" * 100)
    print("HISTORIAL DE EXPERIENCIAS (últimas 25)")
    print("=" * 100)
    
    print(f"\n{'Exp':<4} | {'Comida':<22} | {'IM':>10} | {'Clasif':<14} | {'Costo':>8} | {'Gan':>8} | {'K':>8}")
    print("-" * 95)
    
    for h in historial[-25:]:
        print(f"{h['experiencia']:<4} | {h['comida'][:22]:<22} | {h['IM']:10.4f} | {h['clasificacion']:<14} | {h['costo_estructural']:8.4f} | {h['ganancia_estructural']:8.4f} | {h['K']:8.4f}")
    ...
    # Análisis de capacidad de regeneración
    print(f"\n🔄 CAPACIDAD DE REGENERACIÓN:")
    for comida in opciones:
        ganancias = [h['ganancia_estructural'] for h in historial if h['comida'] == comida]
        if ganancias and np.mean(ganancias) > 0.01:
            print(f"  ✅ {comida}: regeneración detectada (promedio={np.mean(ganancias):.4f})")
        elif ganancias:
            print(f"  ➖ {comida}: regeneración baja (promedio={np.mean(ganancias):.4f})")
    
    print("\n" + "=" * 100)
    print("EXPERIMENTO COMPLETADO")
    print("El sistema ahora puede REGENERARSE con entradas nutritivas.")
    print("=" * 100)

#!/usr/bin/env python3
"""
VSTCosmos - v72-B Fase 3: Plasticidad Hebbiana (Corregida)
...
Test C usa SILENCIO como control (no ruido).
...
def experimento_fase3_silencio(duracion_test=30.0, seed=42):
    """
    Test C: Control con SILENCIO (no ruido).
    El silencio no tiene estructura espectral. El gradiente debería ser basal.
    """
    np.random.seed(seed)
    ...
    # Objetivo silencio = ceros
    objetivo_silencio = np.zeros((DIM_AUDITIVA, DIM_TIME))
    
    for idx in range(N_PASOS_TEST):
        Phi_total, Phi_vel_total, W = actualizar_campo_total(
            Phi_total, Phi_vel_total, W,
            objetivo_silencio, alpha=0.0,
            omega_natural=omega_natural, amort_natural=amort_natural
        )
        g = calcular_gradiente(Phi_total, DIM_INTERNA)
        a = calcular_acoplamiento(Phi_total, DIM_INTERNA)
...
def main():
    print("=" * 100)
    print("VSTCosmos - v72-B Fase 3: Plasticidad Hebbiana (Corregida)")
    ...
    print("Test C: Especificidad (SILENCIO sin entrenamiento)")
    print("Test D: Acoplamiento constitutivo")
    print("=" * 100)
    
    # Test A: Persistencia
    print("\n[Test A] Persistencia (voz -> voz)")
    test_A = experimento_fase3("Voz_Estudio.wav", "Voz_Estudio.wav", seed=42)
    print(f"    W tras entrenamiento: {test_A['w_tras_entreno']:.4f}")
    print(f"    Gradiente estable: {test_A['gradiente_estable']:.4f}")
    print(f"    Persistencia (>0.08): {test_A['persistencia_s']:.1f}s")
    print(f"    min_acop: {test_A['min_acop']:.6f}")
    
    # Test B: Asimetría (entrena voz, testea ruido)
    print("\n[Test B] Asimetría (voz -> ruido)")
    test_B = experimento_fase3("Voz_Estudio.wav", "Ruido blanco", seed=43)
    print(f"    W tras entrenamiento: {test_B['w_tras_entreno']:.4f}")
    print(f"    Gradiente estable: {test_B['gradiente_estable']:.4f}")
    print(f"    Persistencia (>0.08): {test_B['persistencia_s']:.1f}s")
    print(f"    min_acop: {test_B['min_acop']:.6f}")
    
    # Test C: Silencio (control)
    print("\n[Test C] Especificidad (SILENCIO sin entrenamiento)")
    test_C = experimento_fase3_silencio(seed=44)
    print(f"    Gradiente estable: {test_C['gradiente_estable']:.4f}")
    ...
    # Criterio C: Especificidad (gradiente bajo con silencio)
    criterio_C = test_C['gradiente_estable'] < 0.03
    
    # Criterio D: Acoplamiento
    criterio_D = (test_A['min_acop'] > 0.01 and test_B['min_acop'] > 0.01 and test_C['min_acop'] > 0.01)
    ...
    print(f"\n  Test B (Asimetría):")
    print(f"    grad_estable = {test_B['gradiente_estable']:.4f} (>0.08: {'✅' if test_B['gradiente_estable'] > 0.08 else '❌'})")
    print(f"    → {'✅' if criterio_B else '❌'}")
    
    print(f"\n  Test C (Especificidad - SILENCIO):")
    print(f"    grad_estable = {test_C['gradiente_estable']:.4f} (<0.03: {'✅' if test_C['gradiente_estable'] < 0.03 else '❌'})")
    print(f"    → {'✅' if criterio_C else '❌'}")
    
    print(f"\n  Test D (Acoplamiento):")
    print(f"    min_acop A = {test_A['min_acop']:.6f} (>0.01: {'✅' if test_A['min_acop'] > 0.01 else '❌'})")
    print(f"    min_acop B = {test_B['min_acop']:.6f} (>0.01: {'✅' if test_B['min_acop'] > 0.01 else '❌'})")
    print(f"    min_acop C = {test_C['min_acop']:.6f} (>0.01: {'✅' if test_C['min_acop'] > 0.01 else '❌'})")
    ...
    elif test_A['w_tras_entreno'] < 0.001:
        print("\n  ❌ NO APRENDIZAJE")
        print("     W_tras_entreno < 0.001. No hubo aprendizaje durante entrenamiento.")
        print("     → Subir ETA_HEBB a 0.05")
    elif criterio_A and criterio_B and not criterio_C:
        print("\n  ⚠️ VALIDACIÓN PARCIAL: Persistencia y Asimetría OK, pero control con silencio alto")
        print("     El campo tiene actividad basal alta incluso sin entrada.")
        print("     → Reducir GANANCIA_REACCION o aumentar DIFUSION_BASE")
    elif criterio_A and not criterio_B:
        print("\n  ⚠️ CRITERIO PARCIAL: Solo Persistencia")
        print("     W aprendió el patrón de voz, pero no generaliza a ruido.")
        print("     → Aumentar GAMMA_PLAST a 0.25")
    else:
        print("\n  ❌ VALIDACIÓN INCOMPLETA")
        print("     El mecanismo no produce persistencia ni asimetría.")
        if test_A['gradiente_estable'] < 0.05:
            print("     → GAMMA_PLAST insuficiente. Subir a 0.25")
        if test_B['gradiente_estable'] < 0.05:
            print("     → El término plástico no reconstruye patrón desde ruido.")
    ...
    # Gráfico 1: Gradiente durante entrenamiento (Test A)
    ax = axes[0, 0]
    ...
    # Gráfico 2: Gradiente durante test (Test A)
    ax = axes[0, 1]
    ...
    # Gráfico 3: Gradiente durante test (Test B)
    ax = axes[0, 2]
    ...
    # Gráfico 4: Evolución de W (Test A)
    ax = axes[1, 0]
    ax.plot(test_A['w_norma_historia'])
    ax.axhline(y=test_A['w_tras_entreno'], color='r', linestyle='--', label=f'W final = {test_A["w_tras_entreno"]:.4f}')
    ax.set_xlabel('Paso')
    ax.set_ylabel('Norma de W')
    ax.set_title('Evolución de W durante entrenamiento')
    ax.legend()
    ax.grid(True, alpha=0.3)
    
    # Gráfico 5: Test C (silencio)
    ax = axes[1, 1]
    ax.plot(test_C['gradientes_test'])
    ax.axhline(y=0.03, color='r', linestyle='--', label='Umbral especificidad')
    ax.set_xlabel('Paso')
    ax.set_ylabel('Gradiente')
    ax.set_title(f'Test C: Control (SILENCIO, alpha=0)\nGrad_estable: {test_C["gradiente_estable"]:.4f}')
    ax.legend()
    ax.grid(True, alpha=0.3)
    
    # Gráfico 6: Comparación de gradientes estables
    ax = axes[1, 2]
    nombres = ['Test A\n(voz->voz)', 'Test B\n(voz->ruido)', 'Test C\n(silencio)']
    valores = [test_A['gradiente_estable'], test_B['gradiente_estable'], test_C['gradiente_estable']]
    colores = ['green', 'blue', 'red']
    ax.bar(nombres, valores, color=colores)
    ax.axhline(y=0.15, color='green', linestyle='--', alpha=0.5, label='Umbral persistencia')
    ax.axhline(y=0.08, color='blue', linestyle='--', alpha=0.5, label='Umbral asimetría')
    ax.axhline(y=0.03, color='red', linestyle='--', alpha=0.5, label='Umbral silencio')
    ax.set_ylabel('Gradiente estable')
    ax.set_title('Comparación de regímenes')
    ax.legend()
    ax.grid(True, alpha=0.3)
    
    plt.suptitle('VSTCosmos v72-B Fase 3 - Plasticidad Hebbiana (Control Silencio)', fontsize=14)
    plt.tight_layout()
    ...
    with open('v72b_fase3_resultado.csv', 'w', newline='', encoding='utf-8') as f:
        ...
        writer.writerow(['A', 'w_tras_entreno', f'{test_A["w_tras_entreno"]:.4f}', '>0.001'])
        ...
        writer.writerow(['A', 'min_acop', f'{test_A["min_acop"]:.6f}', '>0.01'])
        writer.writerow(['B', 'grad_estable', f'{test_B["gradiente_estable"]:.4f}', '>0.08'])
        writer.writerow(['B', 'min_acop', f'{test_B["min_acop"]:.6f}', '>0.01'])
        writer.writerow(['C', 'grad_estable_silencio', f'{test_C["gradiente_estable"]:.4f}', '<0.03'])
        writer.writerow(['C', 'min_acop', f'{test_C["min_acop"]:.6f}', '>0.01'])
        
        if criterio_A and criterio_B and criterio_C and criterio_D:
            writer.writerow(['decision', 'v72_b_fase3', 'VALIDADO', ''])
        else:
            writer.writerow(['decision', 'v72_b_fase3', 'PENDIENTE', ''])
    ...
    with open('v72b_fase3_resultado.txt', 'w', encoding='utf-8') as f:
        ...
        f.write(f"Test A (Persistencia - voz->voz):\n")
        ...
        f.write(f"  min_acop = {test_A['min_acop']:.6f}\n\n")
        f.write(f"Test B (Asimetría - voz->ruido):\n")
        f.write(f"  grad_estable = {test_B['gradiente_estable']:.4f}\n")
        f.write(f"  min_acop = {test_B['min_acop']:.6f}\n\n")
        f.write(f"Test C (Especificidad - SILENCIO):\n")
        f.write(f"  grad_estable = {test_C['gradiente_estable']:.4f}\n")
        f.write(f"  min_acop = {test_C['min_acop']:.6f}\n\n")
        f.write("=" * 60 + "\n")
        if criterio_A and criterio_B and criterio_C and criterio_D:

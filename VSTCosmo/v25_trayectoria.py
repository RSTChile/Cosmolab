#!/usr/bin/env python3
"""
VSTCosmo - v25: Memoria de trayectoria (Γ)
Γ acumula persistencia de la dirección del cambio en el campo.
No mide velocidad de cambio, sino consistencia del flujo.
"""
...
def actualizar_memoria_trayectoria(Gamma, Phi, Phi_prev, Phi_prev2, A):
    """
    Γ acumula consistencia del flujo (dirección y magnitud del cambio).
    No mide velocidad de cambio, sino persistencia de la dirección.
    """
    flujo = Phi - Phi_prev
    flujo_prev = Phi_prev - Phi_prev2
    
    # Consistencia del flujo: 1 si flujo ≈ flujo_prev, 0 si son muy diferentes
    # Usamos una función suave sin umbrales fijos
...
def simular(audio, sr, nombre, num_pasos=N_PASOS):
    ...
    for paso in range(num_pasos):
        ...
        Gamma = actualizar_memoria_trayectoria(Gamma, Phi, Phi_prev, Phi_prev2, A)
...
def main():
    print("=" * 100)
    print("VSTCosmo - v25: Memoria de trayectoria (Γ)")
    print("Γ acumula consistencia del flujo (dirección persistente del cambio)")
    print("=" * 100)
    ...
    print(f"{'Entrada':<15} | {'rango Φ':>10} | {'rango A':>10} | {'media A':>10} | "
          f"{'media Ψ':>10} | {'media Ω':>10} | {'media Γ':>10}")
    print("-" * 120)
    ...
    print(f"\nComparación de Γ (consistencia del flujo):")
    print(f"  Tono puro:    Γ = {tono_res[6]:.4f}")
    print(f"  Ruido blanco: Γ = {ruido_res[6]:.4f}")
    print(f"  Voz:          Γ = {voz_res[6]:.4f}")
    
    if voz_res[6] > ruido_res[6] and voz_res[6] > tono_res[6]:
        print("\n  ★ La voz tiene la mayor consistencia de flujo (Γ)")
        print("    El sistema distingue la voz por la persistencia de su trayectoria.")
        print("    ★★★ ESTE ES EL RÉGIMEN BUSCADO ★★★")
    else:
        print("\n  ✗ La voz no tiene la mayor consistencia de flujo")
        print("    Ajustar parámetros de acumulación de Γ.")

#!/usr/bin/env python3
"""
VSTCosmos v84b — Campo diferencial: correcciones de signo y orientación

Cambios respecto a v84:
1. Carga archivos desde audio_binaural/ preprocesados por preprocesar_binaurales.py
   (ya no genera binaurales en tiempo de ejecución)
2. calcular_senal_busqueda centrada en PHI_EQUILIBRIO — el signo de act_busc
   ahora es informativo (positivo = L>R, negativo = R>L)
3. aplicar_orientacion_por_coherencia amplificada por K_ORIENT = T_PROFUNDA/T_RECIENTE = 10
   (derivado del mismo k que usa la selección por eficiencia)
...
print("=" * 100)
print("VSTCosmos v84b — Campo diferencial: correcciones de signo y orientación")
print("")
print("  Cambios respecto a v84:")
print("  1. Carga desde audio_binaural/ preprocesado externamente")
print("  2. calcular_senal_busqueda centrada en PHI_EQUILIBRIO (signo informativo)")
print("  3. K_ORIENT = T_PROFUNDA/T_RECIENTE = 10 en orientacion_por_coherencia")
print("")
...
def cargar_desde_disco(directorio='audio_binaural'):
    """
    Carga los archivos binaurales generados por preprocesar_binaurales.py.
    No convierte nada — solo lee los canales L y R de cada archivo.

    Si un archivo no existe, lanza RuntimeError con instrucciones claras.
    """
    ARCHIVOS_REQUERIDOS = {
        ...
        'lr_mixto':       'Binaural LR mixto.wav',
    }

    archivos = {}
    faltantes = []

    print(f"\n  Cargando archivos binaurales desde '{directorio}/'...")
    ...
    for clave, nombre in ARCHIVOS_REQUERIDOS.items():
        ...
        try:
            ...
            if data.ndim == 1:
                # Mono inesperado — duplicar canal
                canal_L = data
                canal_R = data
            else:
                canal_L = data[:, 0]
                canal_R = data[:, 1]
            archivos[clave] = (sr, canal_L, canal_R)
            dur = len(canal_L) / sr
            print(f"    ✅ {clave:<20} '{nombre}' ({dur:.1f}s, sr={sr})")
        except Exception as e:
            faltantes.append(f"{ruta} ({e})")
    ...
    if faltantes:
        ...
        for f in faltantes:
            print(f"    {f}")
        raise RuntimeError(
            f"\nFaltan {len(faltantes)} archivos en '{directorio}/'.\n"
            "Ejecutar primero: py preprocesar_binaurales.py"
        )
...
def aplicar_orientacion_por_coherencia(Phi_total, coherencia_rel, dt):
    ...
    Derivado del mismo cociente de escalas temporales que k en la
    selección por eficiencia. Sin este factor la señal es ~4.5e-7
    por paso — invisible contra el ruido de exploración.
    Con K_ORIENT=10: ~4.5e-6 por paso, suficiente para acumular
    diferencia visible en 2000 pasos.

    No es un valor arbitrario: es la relación estructural entre
    la escala lenta (identidad) y la escala rápida (contexto).
    """
    K_ORIENT = T_PROFUNDA_SEG / T_RECIENTE_SEG   # = 10
    acg0  = idx['act_geom'][0]
    acg1  = idx['act_geom'][1]
...
def calcular_senal_busqueda(Phi_total):
    ...
    act_busc se inicializa alrededor de PHI_EQUILIBRIO = 0.5.
    El gradiente diferencial introduce perturbaciones sobre ese valor base.
    Sin centrar, el promedio queda dominado por 0.5 y el signo es invisible.

    Con centrado:
        positivo → L > R → fuente a la derecha (+60°)
        negativo → R > L → fuente a la izquierda (-60°)
        cero     → sin gradiente o simétrico
    """
...
# ============================================================
def main():
    # Paso 0: cargar archivos preprocesados por preprocesar_binaurales.py
    archivos = cargar_desde_disco('audio_binaural')

    # Entrenamiento con voz +60°
    Phi_total, Phi_vel_total, W_prof, W_rec, \
        Phi_int_historia, ERROR_EQ, explorador = entrenar(archivos, 30.0)

    # Protocolo de fases — mismo que v84 más archivos adicionales disponibles
    FASES = [
    ...
    # ---- DIAGNÓSTICO ----
    print("\n" + "=" * 100)
    print("DIAGNÓSTICO — v84b Campo diferencial")
    print("=" * 100)
    ...
    # ---- CSV ----
    with open('v84b_campo_diferencial.csv', 'w', newline='', encoding='utf-8') as f:
        wr = csv.writer(f)
    ...
    print("\n  CSV guardado: v84b_campo_diferencial.csv")
    ...
    plt.suptitle(
        'VSTCosmos v84b — Campo diferencial: signo centrado + K_ORIENT=10',
        fontsize=13
    )
    plt.tight_layout()
    plt.savefig('v84b_campo_diferencial.png', dpi=150)
    print("  Gráfico guardado: v84b_campo_diferencial.png")

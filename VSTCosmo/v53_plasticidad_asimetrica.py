#!/usr/bin/env python3
"""
VSTCosmos - v53: Plasticidad Asimétrica
El aprendizaje es rápido (acelerado).
El olvido es lento y resistente.
La identidad se CONSOLIDA en el tiempo.
"""
...
# NUEVO: PLASTICIDAD ASIMÉTRICA
VALOR_ESTRUCTURAL_DECAY = 0.998     # Decaimiento MUY lento (identidad)
VALOR_ESTRUCTURAL_BASE = 0.3
REFUERZO_EXPONENTE = 1.5            # Aceleración en subida
REFUERZO_MAX = 1.0
APRENDIZAJE_VELOCIDAD = 1.0         # Factor de subida
OLVIDO_VELOCIDAD = 0.1              # Factor de bajada (mucho más lento)
...
def actualizar_valor_estructural_asimetrico(valor_estructural, comida, ganancia_estructural):
    ...
    - Sube RÁPIDO (acelerado)
    - Baja LENTO (resistente)
    - La identidad se CONSOLIDA
    """
    # Decaimiento general (MUY lento)
    ...
    if ganancia_estructural > 0:
        ...
        # OLVIDO: lento, resistente
        # Solo olvida si la entrada no es nutritiva consistentemente
        # El decaimiento ya es muy lento (0.998), esto es adicional
        pass
...
# ============================================================
def simular_vida_con_plasticidad_asimetrica(opciones, n_experiencias=120):
    print("=" * 100)
    print("VSTCosmos - v53: Plasticidad Asimétrica")
    print("Aprendizaje RÁPIDO (acelerado)")
    print("Olvido LENTO y resistente")
    print("La identidad se CONSOLIDA en el tiempo")
    print("=" * 100)
    ...
    for exp in range(n_experiencias):
        ...
        print(f"  💎 Identidad (valor estructural): {valor_actual:.3f}")
        ...
        # Actualizar valor estructural (plastica asimetrica)
        valor_estructural = actualizar_valor_estructural_asimetrico(valor_estructural, comida_elegida, ganancia_struct)
...
def main():
    ...
    print("\n⚠️  PLASTICIDAD ASIMÉTRICA")
    print("   - Aprendizaje: RÁPIDO (refuerzo acelerado)")
    print("   - Olvido: LENTO (decaimiento 0.998)")
    print("   - La identidad se CONSOLIDA")
    
    historial, K_final, fatiga_final, valor_final = simular_vida_con_plasticidad_asimetrica(opciones, n_experiencias=100)
    
    print("\n" + "=" * 100)
    print("RESULTADOS FINALES: IDENTIDAD CONSOLIDADA")
    print("=" * 100)
    ...
    for comida, valor in valor_final.items():
        if valor >= max_valor * 0.8:
            print(f"  🌟 {comida}: IDENTIDAD DOMINANTE (valor = {valor:.3f})")
        elif valor >= max_valor * 0.6:
            print(f"  ✅ {comida}: identidad sólida (valor = {valor:.3f})")
        elif valor >= max_valor * 0.4:
            print(f"  ➖ {comida}: identidad moderada (valor = {valor:.3f})")
        else:
            print(f"  ❌ {comida}: identidad baja (valor = {valor:.3f})")

#!/usr/bin/env python3
"""
VSTCosmo - v23: Triple memoria (Ψ, Ω, Σ) - VERSIÓN CORREGIDA
Ψ: estabilidad (poco cambio)
Ω: coherencia (cambio regular)
Σ: estructura en la variación (consistencia del signo de la segunda derivada)
"""
...
# ============================================================
# PARÁMETROS (ajustados para estabilidad y diferenciación)
# ============================================================
DIM_FREQ = 32
DIM_TIME = 100
DT = 0.01
...
GANANCIA_GENERACION = 0.10   # Aumentada para dar más relieve a Φ
...
BLOQUEO_MAXIMO = 0.4   # Reducido para permitir más cambio en Φ
...
FUERZA_ESTRUCTURA = 0.3   # Mayor peso para que Σ influya en A
...
def actualizar_memoria_estructura(Sigma, Phi, Phi_prev, Phi_prev2, A, Sigma_prev):
    """
    Σ mide la consistencia del signo de la segunda derivada (cambio del cambio).
    No depende de la magnitud, solo de si la segunda derivada mantiene el signo.
    """
    # Segunda derivada (cambio del cambio)
    dd = Phi - 2*Phi_prev + Phi_prev2
    
    # Signo de la segunda derivada (suave)
    signo_dd = np.tanh(dd * 5.0)
    signo_dd_prev = np.tanh(Sigma_prev * 5.0)  # Usar Sigma anterior como proxy (o guardar variable)
    
    # Consistencia: 1 si el signo de dd se mantiene, 0 si cambia
    consistencia = (1.0 + signo_dd * signo_dd_prev) / 2.0
    
    # La estructura es la consistencia (no usamos magnitud)
...
def main():
    ...
    print("Versión corregida: Σ basada en consistencia del signo de la segunda derivada")
    print("=" * 100)

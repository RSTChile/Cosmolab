#!/usr/bin/env python3
"""
VSTCosmos - v69: Saciedad Diferencial
Sistema completo con campo Φ oscilatorio y resonancia.
La fatiga sube más rápido para estímulos de BAJA complejidad (predecibles).
La fatiga sube más lento para estímulos de ALTA complejidad (impredecibles).
El sistema se satura más rápido de lo simple, preservando la utilidad inalterada.
"""
...
# ============================================================
# PARÁMETROS DEL SISTEMA (heredados de v68)
# ============================================================
DIM_FREQ = 32
DIM_TIME = 100
DT = 0.01
...
# Fatiga - NUEVOS PARÁMETROS PARA SACIEDAD DIFERENCIAL
FATIGA_DECAY = 0.97
FATIGA_SUBE_BASE = 0.08
FATIGA_SUBE_COMPLEJIDAD_ESCALA = 0.3  # Factor de aumento para complejidad baja
FATIGA_MAX = 0.7
FATIGA_ESCALA = 2.0
...
PHI_EQUILIBRIO = 0.5
DECAY_PHI_VEL = 0.5  # decaimiento de velocidad entre experiencias
...
def calcular_complejidad_espectral(perfil):
    """Entropía normalizada del espectro"""
...
def actualizar_campo_v69(Phi, Phi_vel, A, perfil, Psi, ganancia_gen, bloqueo_max, K,
                          omega_natural, amort_natural):
    """
    v69: Mecanismo oscilatorio integrado (igual que v68).
    """
...
def extraer_estado(S):
    ...
    return {
        ...
        'energia_oscilatoria': np.mean(Phi_vel ** 2)
    }
...
def calcular_ganancia_estructural(estado_before, estado_after, fatiga, delta_estimulo, energia_oscilatoria):
    ...
    factor_oscilacion = min(1.0, energia_oscilatoria * 200)
    
    ganancia_neta = ganancia_bruta * delta_estimulo * factor_oscilacion * factor_fatiga
...
def actualizar_fatiga_v69(fatiga, comida, complejidad_espectral):
    """
    v69: Fatiga con tasa adaptativa según complejidad espectral.
    Estímulos simples (complejidad baja) → fatiga más rápida.
    Estímulos complejos (complejidad alta) → fatiga más lenta.
    """
    # Decaimiento general
    for c in fatiga:
        fatiga[c] *= FATIGA_DECAY
    
    # Tasa de fatiga inversamente proporcional a la complejidad
    # A menor complejidad, mayor tasa
    tasa = FATIGA_SUBE_BASE + FATIGA_SUBE_COMPLEJIDAD_ESCALA * (1.0 - complejidad_espectral)
    tasa = np.clip(tasa, 0.04, 0.25)
    
    fatiga[comida] += tasa * (1 - fatiga[comida])
...
def medir_asimilacion_basal(S, G, R, C, K):
    ...
    for paso in range(N_BASAL):
        ...
        Phi_temp, Phi_vel_temp = actualizar_campo_v69(
            Phi_temp, Phi_vel_temp, A_temp, perfil, Psi_temp,
            ganancia_gen, bloqueo_max, K, omega_natural, amort_natural
        )
    ...
    S_basal = (Phi_temp, A_temp, Psi_temp, Omega_temp, M_temp, L_temp, Phi_vel_temp)
    
    return asimilacion_basal, S_basal


def simular_experiencia_v69(S, entrada_nombre, G, R, C, habituacion_comida, K):
    Phi, A, Psi, Omega, M, L, Phi_vel = S
    omega_natural, amort_natural = calcular_frecuencias_naturales()
    ...
    # Acumulador de complejidad espectral
    complejidad_acumulada = 0.0
    n_ventanas_procesadas = 0
    
    # Acumulador de firma oscilatoria
    ...
    for idx_ventana in range(min(N_PASOS, n_ventanas)):
        ...
        # Calcular complejidad espectral para esta ventana
        complejidad_acumulada += calcular_complejidad_espectral(perfil)
        n_ventanas_procesadas += 1
        ...
        Phi_real, Phi_vel_real = actualizar_campo_v69(
            Phi_real, Phi_vel_real, A_real, perfil, Psi_real,
            ganancia_gen, bloqueo_max, K, omega_natural, amort_natural
        )
        ...
        Phi_sin, Phi_vel_sin = actualizar_campo_v69(
            Phi_sin, Phi_vel_sin, A_sin, perfil_sin, Psi_sin,
            ganancia_gen, bloqueo_max, K, omega_natural, amort_natural
        )
        ...
        # Acumular energía oscilatoria
        energia_por_banda += np.mean(Phi_vel_real ** 2, axis=1)
        n_pasos_acum += 1
    ...
    # Calcular complejidad espectral media de la experiencia
    complejidad_media = complejidad_acumulada / max(n_ventanas_procesadas, 1)
    
    # Normalizar energía oscilatoria
    energia_por_banda /= max(n_pasos_acum, 1)
    ...
    metricas = {
        ...
        'complejidad_espectral': complejidad_media  # NUEVO
    }
    
    S_final = (Phi_real, A_real, Psi_real, Omega_real, M_real, L_real, Phi_vel_real)
    
    return S_final, G, R, C, metricas


def simular_reposo_v69(S, G, R, C, K):
    Phi, A, Psi, Omega, M, L, Phi_vel = S
    omega_natural, amort_natural = calcular_frecuencias_naturales()
    ...
    for paso in range(N_REPOSO):
        ...
        Phi, Phi_vel = actualizar_campo_v69(
    ...
    return (Phi, A, Psi, Omega, M, L, Phi_vel), G, R, C


def respirar_entre_experiencias_v69(M, Psi, L, A, Phi_vel):
    M = M * DECAY_M
    Psi = Psi * DECAY_PSI
    L = L * DECAY_L_ENTRE
    Phi_vel = Phi_vel * DECAY_PHI_VEL
    A_nueva = A_BASAL * (1 - PESO_M_PARA_A) + PESO_M_PARA_A * M
    return M, Psi, L, A_nueva, Phi_vel
...
def calcular_utilidad_con_identidad(comida, historial_im, habituacion, exploracion_prob, 
    ...
    # Utilidad NO MODIFICADA - sigue basada en IM histórico
...
def simular_vida_completa_v69(opciones, n_experiencias=100):
    print("=" * 100)
    print("VSTCosmos - v69: Saciedad Diferencial")
    print("Sistema completo con campo Φ oscilatorio y resonancia.")
    print("La fatiga sube más rápido para estímulos de BAJA complejidad.")
    print("El sistema se satura más rápido de lo simple.")
    print("=" * 100)
    ...
    S_actual, G, R, C = simular_reposo_v69((Phi, A, Psi, Omega, M, L, Phi_vel), G, R, C, K)
    Phi, A, Psi, Omega, M, L, Phi_vel = S_actual
    ...
    for exp in range(n_experiencias):
        ...
        # Estado antes
        S_basal_antes = (Phi.copy(), A.copy(), Psi.copy(), Omega.copy(), M.copy(), L.copy(), Phi_vel.copy())
        ...
        # EXPERIENCIA
        S_despues_procesar, G, R, C, metricas = simular_experiencia_v69(
            S_despues_basal, comida_elegida, G, R, C, habituacion[comida_elegida], K
        )
        
        delta_estimulo = metricas['delta_estimulo']
        energia_total = metricas['energia_total']
        complejidad = metricas['complejidad_espectral']
        print(f"  📐 Δ_estímulo: {delta_estimulo:.3f}")
        print(f"  🌀 Energía oscilatoria: {energia_total:.6f}")
        print(f"  📊 Complejidad espectral: {complejidad:.3f}")
        ...
        ganancia_struct = calcular_ganancia_estructural(estado_before, estado_after_procesar, 
                                                        fatiga[comida_elegida], delta_estimulo, energia_total)
        ...
        if estado_after_procesar['rango_A'] > UMBRAL_MEMORIA:
            _, A_actual, _, _, M_actual, _, _ = S_despues_procesar
        ...
        # Actualizar fatiga (con complejidad espectral)
        fatiga = actualizar_fatiga_v69(fatiga, comida_elegida, complejidad)
        ...
        # Reposo
        S_post_reposo, G, R, C = simular_reposo_v69(S_despues_procesar, G, R, C, K)
        
        # Respirar
        M, Psi, L, A, Phi_vel = respirar_entre_experiencias_v69(M, Psi, L, A, Phi_vel)
        
        # Actualizar estado
        Phi, A, Psi, Omega, M, L, Phi_vel = S_post_reposo
        ...
        print(f"    Fatiga actualizada: {fatiga[comida_elegida]:.3f}")
    
    return historial_im, K, habituacion, fatiga, valor_estructural
...
def main():
    ...
    print("\n⚠️  V69: SACIEDAD DIFERENCIAL")
    print("   - Campo Φ oscilatorio (segundo orden)")
    print("   - Fatiga más rápida para estímulos simples (tono, ruido)")
    print("   - Fatiga más lenta para estímulos complejos (voz, música)")
    print("   - La utilidad NO se modifica - solo cambia la saciedad")
    
    historial, K_final, habituacion_final, fatiga_final, valor_final = simular_vida_completa_v69(opciones, n_experiencias=100)
    
    print("\n" + "=" * 100)
    print("RESULTADOS FINALES: V69")
    print("=" * 100)
    ...
    print(f"\n💎 IDENTIDAD FINAL (valor estructural):")
    ...
    print(f"\n😫 FATIGA FINAL:")
    for comida, fat in sorted(fatiga_final.items(), key=lambda x: x[1], reverse=True):
        print(f"  {comida:<25}: {fat:.3f}")
    
    # Verificar predicción: tono puro debe aparecer menos de 30 veces
    tono_veces = conteo.get("Tono puro", 0)
    voz_veces = conteo.get("Voz_Estudio.wav", 0)
    
    print("\n" + "=" * 100)
    print("CRITERIO DE ÉXITO")
    print("=" * 100)
    
    print(f"\n  Tono puro elegido: {tono_veces} veces")
    print(f"  Voz elegida: {voz_veces} veces")
    
    if tono_veces < 30 and voz_veces > 10:
        print("\n  ✅ V69 EXITOSO")
        print("     → La saciedad diferencial funcionó.")
        print("     → El sistema prefiere complejidad sobre intensidad.")
    elif tono_veces < 45:
        print("\n  ⚠️  V69 PARCIAL")
        print("     → La saciedad diferencial ayudó, pero aún no es suficiente.")
        print("     → Ajustar parámetros de fatiga.")
    else:
        print("\n  ❌ V69 FALLIDO")
        print("     → El tono puro sigue dominando.")
        print("     → Aumentar FATIGA_SUBE_COMPLEJIDAD_ESCALA.")

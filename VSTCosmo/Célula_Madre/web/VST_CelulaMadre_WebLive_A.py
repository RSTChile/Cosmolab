...
class Handler(BaseHTTPRequestHandler):
    ...
    def _start(self, req):
        ...
        with RUN_LOCK:
            ...
            RUN = Run(req.get("cfg", {}), req.get("toggles", {}), req.get("sim_s", 6))
            HOMEO_EMERGENTE.reset()                    # banda viable emergente arranca limpia por corrida
            MEMORIA.reset()                            # nueva vida = memoria limpia (persistencia cross-corrida: futuro)
            METABOLISMO.reset()                        # nueva vida = reservas de energía iniciales
            RUN.start()                               # construye/captura en el worker (NO bloquea aquí)

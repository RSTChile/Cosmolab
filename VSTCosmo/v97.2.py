#!/usr/bin/env python3
"""
VSTCosmos v97.2 — BigBang con diagnóstico post-entrenamiento + asimetría en log fino

Idéntico a v97.1 EXCEPTO:
  - El log CSV fino guarda act_geom_asimetria cada 0.5s
  - Análisis post-hoc de trayectoria de asimetría durante warmup
  - Lectura integrada que distingue "reorientación gradual" vs "sobreescritura por input"

Pregunta que resuelve:
  ¿La trayectoria de act_geom_asimetria de B durante los primeros 20s
   muestra una transición gradual de -0.54 hacia +0.50,
   o se sobreescribe al primer paso de cálculo?

  - Transición gradual con tiempo medible → reorientación funcional real
  - Sobreescritura instantánea → la "memoria estructural" diagnosticada
    no tiene efecto funcional sobre Ω bajo este protocolo
"""
...
def imprimir_configuracion():
    print("=" * 100)
    print("VSTCosmos v97.2 — BigBang con asimetría de act_geom en log fino")
    print("=" * 100)
    print()
    print("CONFIGURACIÓN:")
    ...
    print(f"  DIAGNÓSTICO POST-ENTRENAMIENTO: SÍ")
    print(f"  ASIMETRÍA DE act_geom EN LOG FINO: SÍ (nuevo en v97.2)")
    print()
    print("INSTANCIAS:")
    ...
    print()
    print("PREGUNTA NUEVA (v97.2):")
    print("  ¿La trayectoria de asim_geom(t) en B muestra transición gradual")
    print("   de -0.54 hacia +0.50, o se sobreescribe instantáneamente?")
    print()
...
# ============================================================
# CÁLCULO DE ASIMETRÍA DE act_geom (nuevo en v97.2)
# ============================================================
def calcular_asimetria_act_geom(Phi_total):
    """
    Calcula act_geom_asimetria = media(mitad_alta) - media(mitad_baja)
    Idéntico al diagnóstico post-entrenamiento.
    """
    ag0, ag1 = idx['act_geom']
    geom = Phi_total[ag0:ag1, :]
    mitad = max(1, (ag1 - ag0) // 2)
    media_baja = float(np.mean(geom[:mitad, :]))
    media_alta = float(np.mean(geom[mitad:, :]))
    return media_alta - media_baja, media_baja, media_alta
...
def comparar_estados_entre_instancias(diag_A, diag_B, diag_C):
    ...
    for n1, n2, d1, d2 in pares:
        ...
        print(f"\n  {n1} vs {n2}:")
        print(f"    ‖Phi_{n1} - Phi_{n2}‖         = {dist_phi:.6f}  (norm: {dist_phi_n:.6f})")
        print(f"    ‖W_prof‖ ({n1} vs {n2})       = {dist_wprof:.6f}")
        print(f"    ‖W_rec‖  ({n1} vs {n2})       = {dist_wrec:.6f}")
...
def evaluar(Phi_total, Phi_vel_total, W_prof, W_rec, Phi_int_historia,
    ...
    print(f"  {'t(s)':>6} | {'GED':>9} | {'efic':>8} | "
          f"{'Ω':>7} | {'asim':>8} | {'gradE':>7} | warmup")
    ...
    for paso in range(n_pasos):
        ...
        asim, baja, alta = calcular_asimetria_act_geom(Phi_total)

        omega_traj.append(omega)
        asim_traj.append(asim)
        t_traj.append(t_actual)

        if paso % LOG_FINO_PASOS == 0:
            efic_str = f"{efic:>8.4f}" if not np.isnan(efic) else "     NaN"
            print(f"  {t_actual:>6.1f} | {ged:>9.6f} | {efic_str} | "
                  f"{omega:>+7.3f} | {asim:>+8.4f} | "
                  f"{gradiente_E:>+7.3f} | "
...
def analizar_trayectoria_asimetria(t_A, asim_A, t_B, asim_B, t_C, asim_C,
                                    diag_A, diag_B, diag_C):
    """
    Análisis post-hoc específico de la trayectoria de asim_geom.
    Pregunta: ¿B muestra transición gradual desde el valor de A hacia el de C,
              o se sobreescribe instantáneamente?
    """
    print("\n" + "=" * 100)
    print("ANÁLISIS DE TRAYECTORIA DE asim_geom (nuevo en v97.2)")
    print("=" * 100)

    asim_A_post = diag_A['act_geom_asimetria']
    asim_B_post = diag_B['act_geom_asimetria']
    asim_C_post = diag_C['act_geom_asimetria']

    print(f"\n  Valores post-entrenamiento (referencia del diagnóstico):")
    print(f"    asim_A (post-entrenamiento) = {asim_A_post:+.6f}")
    print(f"    asim_B (post-entrenamiento) = {asim_B_post:+.6f}")
    print(f"    asim_C (post-entrenamiento) = {asim_C_post:+.6f}")

    print(f"\n  Valores al inicio de la evaluación (t=0, primera muestra):")
    print(f"    asim_A(0) = {asim_A[0]:+.6f}")
    print(f"    asim_B(0) = {asim_B[0]:+.6f}")
    print(f"    asim_C(0) = {asim_C[0]:+.6f}")

    # Discontinuidad entre fin-entrenamiento y inicio-evaluación
    salto_B = abs(asim_B[0] - asim_B_post)
    print(f"\n  Salto B entre fin de entrenamiento y t=0 de evaluación: {salto_B:.6f}")
    if salto_B > 0.5:
        print(f"    → SALTO GRANDE. asim_B se sobreescribe casi instantáneamente.")
    elif salto_B > 0.1:
        print(f"    → SALTO MODERADO. Hay continuidad parcial entre fases.")
    else:
        print(f"    → SIN SALTO. asim_B mantiene el valor del entrenamiento al inicio.")

    # Trayectoria durante warmup (0-20s)
    print(f"\n  Trayectoria de asim_geom durante t = 0 a 20s (cada 1s):")
    print(f"  {'t(s)':>6} | {'asim_A':>10} | {'asim_B':>10} | {'asim_C':>10} | "
          f"{'|B-C|':>8} | {'|B-A|':>8}")

    for t_check in np.arange(0.0, 20.5, 1.0):
        i_A = np.argmin(np.abs(t_A - t_check))
        i_B = np.argmin(np.abs(t_B - t_check))
        i_C = np.argmin(np.abs(t_C - t_check))
        diff_BC = abs(asim_B[i_B] - asim_C[i_C])
        diff_BA = abs(asim_B[i_B] - asim_A[i_A])
        print(f"  {t_check:>6.1f} | {asim_A[i_A]:>+10.6f} | {asim_B[i_B]:>+10.6f} | "
              f"{asim_C[i_C]:>+10.6f} | {diff_BC:>8.6f} | {diff_BA:>8.6f}")

    # Cruce de cero de asim_B
    cruce_cero = None
    for i in range(1, len(asim_B)):
        if asim_B[i-1] * asim_B[i] < 0:  # cambio de signo
            cruce_cero = t_B[i]
            break

    if cruce_cero is not None:
        print(f"\n  Tiempo de cruce de cero de asim_B: t = {cruce_cero:.2f}s")
        if cruce_cero < 1.0:
            print(f"    → CRUCE INSTANTÁNEO. La asimetría cambia de signo en <1s.")
        elif cruce_cero < 5.0:
            print(f"    → CRUCE RÁPIDO. Transición en <5s.")
        else:
            print(f"    → CRUCE GRADUAL. Transición observable.")
    else:
        print(f"\n  asim_B NO cruza cero durante toda la evaluación.")
        print(f"    Valor final asim_B = {asim_B[-1]:+.6f}")

    # Tiempo de convergencia entre B y C en asimetría
    t_conv_asim = None
    n_sostenido = int(VENTANA_SOSTENIDO_SEG / LOG_FINO_DT)
    n_min = min(len(asim_B), len(asim_C))
    diff_BC_traj = np.abs(asim_B[:n_min] - asim_C[:n_min])
    t_B_short = t_B[:n_min]
    mask_post_warmup = t_B_short >= WARMUP_OMEGA_SEG

    for i in range(len(diff_BC_traj) - n_sostenido):
        if mask_post_warmup[i] and np.all(
            diff_BC_traj[i:i + n_sostenido] < UMBRAL_CONVERGENCIA_ASIM
        ):
            t_conv_asim = float(t_B_short[i])
            break

    if t_conv_asim is not None:
        print(f"\n  Tiempo de convergencia |asim_B - asim_C| < {UMBRAL_CONVERGENCIA_ASIM}: "
              f"t = {t_conv_asim:.2f}s")
    else:
        print(f"\n  asim_B y asim_C NO convergen bajo umbral {UMBRAL_CONVERGENCIA_ASIM} "
              f"durante la evaluación.")
        diff_final = float(np.mean(np.abs(asim_B[-60:] - asim_C[-60:])))
        print(f"    Diferencia media |asim_B - asim_C| en últimos 30s: {diff_final:.6f}")

    return {
        'asim_B_post':  asim_B_post,
        'asim_B_inicio': float(asim_B[0]),
        'salto_B': salto_B,
        'cruce_cero_B': cruce_cero,
        't_conv_asim_BC': t_conv_asim,
        'asim_B_final': float(np.mean(asim_B[-60:])),
        'asim_C_final': float(np.mean(asim_C[-60:])),
    }
...
def main():
    ...
    os.makedirs('v97_2_logs', exist_ok=True)
    ...
    print()
    print("-" * 100)
    print("INSTANCIA B — Entrenada BigBang +60°")
    print("-" * 100)
    np.random.seed(SEED)
    ...
    diag_path = f'v97_2_logs/v97_2_diagnostico_{timestamp}.txt'
    with open(diag_path, 'w', encoding='utf-8') as f:
        f.write(f"VSTCosmos v97.2 — Diagnóstico post-entrenamiento\n")
    ...
    res_A = evaluar(
        ...
        csv_log_path=f'v97_2_logs/v97_2_A_log_{timestamp}.csv',
        csv_estimulo_path=f'v97_2_logs/v97_2_A_estimulo_{timestamp}.csv',
    )
    ...
    res_B = evaluar(
        ...
        csv_log_path=f'v97_2_logs/v97_2_B_log_{timestamp}.csv',
        csv_estimulo_path=f'v97_2_logs/v97_2_B_estimulo_{timestamp}.csv',
    )
    ...
    res_C = evaluar(
        ...
        csv_log_path=f'v97_2_logs/v97_2_C_log_{timestamp}.csv',
        csv_estimulo_path=f'v97_2_logs/v97_2_C_estimulo_{timestamp}.csv',
    )

    print()
    print("=" * 100)
    print("ANÁLISIS POST-HOC — Ω")
    print("=" * 100)
    ...
    print(f"\n  |Ω_B - Ω_A| medio:  {clasif['mean_diff_BA']:.4f}")
    print(f"  |Ω_B|       medio:  {clasif['mean_abs_B']:.4f}")
    print(f"  |Ω_B - Ω_C| medio:  {clasif['mean_diff_BC']:.4f}")
    print(f"\n  H1 (mantención):     {'✅ SE CUMPLE' if clasif['H1_mantencion']     else '❌ no se cumple'}")
    ...
    if t_rend is not None:
        print(f"\n  Tiempo de rendición:  t = {t_rend:.2f}s")
    else:
        print(f"\n  Tiempo de rendición:  NO ocurre en {DURACION_EVALUACION}s")

    # ANÁLISIS NUEVO v97.2
    analisis_asim = analizar_trayectoria_asimetria(
        res_A['t_traj'], res_A['asim_traj'],
        res_B['t_traj'], res_B['asim_traj'],
        res_C['t_traj'], res_C['asim_traj'],
        diag_A, diag_B, diag_C,
    )

    # Lectura integrada FINAL
    print()
    print("=" * 100)
    print("LECTURA INTEGRADA — Diagnóstico + Ω + asim_geom")
    print("=" * 100)

    bc_dist = distancias['B_vs_C']['dist_phi_norm']
    print(f"\n  Estados post-entrenamiento (dist Phi normalizada):")
    print(f"    A vs B: {distancias['A_vs_B']['dist_phi_norm']:.6f}")
    print(f"    A vs C: {distancias['A_vs_C']['dist_phi_norm']:.6f}")
    print(f"    B vs C: {bc_dist:.6f}")
    ...
    print(f"\n  asim_geom (resumen):")
    print(f"    post-entrenamiento:  B={analisis_asim['asim_B_post']:+.4f}, "
          f"C={diag_C['act_geom_asimetria']:+.4f}")
    print(f"    inicio evaluación:   B={analisis_asim['asim_B_inicio']:+.4f}")
    print(f"    salto B (entren→eval): {analisis_asim['salto_B']:.4f}")
    print(f"    cruce cero B:        {analisis_asim['cruce_cero_B']}")
    print(f"    convergencia B≈C:    {analisis_asim['t_conv_asim_BC']}")

    print(f"\n  INTERPRETACIÓN INTEGRADA:")
    if analisis_asim['salto_B'] > 0.5 and (
        analisis_asim['cruce_cero_B'] is None or analisis_asim['cruce_cero_B'] < 1.0
    ):
        print(f"  → SOBREESCRITURA POR INPUT.")
        print(f"    La traza del entrenamiento se borra al primer paso de evaluación.")
        print(f"    La 'memoria estructural' diagnosticada NO tiene efecto funcional")
        print(f"    sobre Ω bajo este protocolo. Lo que parece 'reorientación' es")
        print(f"    en realidad la dinámica del estímulo presente imponiéndose")
        print(f"    sobre un estado interno que no resiste.")
    elif analisis_asim['cruce_cero_B'] is not None and analisis_asim['cruce_cero_B'] < 5.0:
        print(f"  → TRANSICIÓN RÁPIDA PERO OBSERVABLE.")
        print(f"    La asimetría de B cruza cero en {analisis_asim['cruce_cero_B']:.2f}s.")
        print(f"    Hay reorientación con dinámica medible, pero rápida.")
    elif analisis_asim['cruce_cero_B'] is not None:
        print(f"  → TRANSICIÓN GRADUAL Y MEDIBLE.")
        print(f"    asim_B cruza cero en {analisis_asim['cruce_cero_B']:.2f}s.")
        print(f"    Esto sí es reorientación funcional genuina, no sobreescritura.")
    else:
        print(f"  → SIN TRANSICIÓN COMPLETA.")
        print(f"    asim_B no cruza cero. Hay alguna forma de retención parcial.")

    # Resumen
    resumen_path = f'v97_2_logs/v97_2_resumen_{timestamp}.txt'
    with open(resumen_path, 'w', encoding='utf-8') as f:
        f.write(f"VSTCosmos v97.2 — Resumen\n")
        f.write(f"Timestamp: {timestamp}\n")
        f.write(f"Seed: {SEED}\n\n")
        f.write(f"--- Distancias post-entrenamiento ---\n")
        f.write(f"A vs B: {distancias['A_vs_B']['dist_phi_norm']:.6f}\n")
        f.write(f"A vs C: {distancias['A_vs_C']['dist_phi_norm']:.6f}\n")
        f.write(f"B vs C: {distancias['B_vs_C']['dist_phi_norm']:.6f}\n\n")
        f.write(f"--- Ω final ---\n")
        f.write(f"Ω_A: {clasif['omega_A_final']:+.6f}\n")
        f.write(f"Ω_B: {clasif['omega_B_final']:+.6f}\n")
        f.write(f"Ω_C: {clasif['omega_C_final']:+.6f}\n\n")
        f.write(f"H1: {clasif['H1_mantencion']}\n")
        f.write(f"H2: {clasif['H2_desorientacion']}\n")
        f.write(f"H3: {clasif['H3_reorientacion']}\n")
        f.write(f"Tiempo de rendición: {t_rend}\n\n")
        f.write(f"--- Análisis asim_geom (nuevo v97.2) ---\n")
        for k, v in analisis_asim.items():
            f.write(f"  {k}: {v}\n")
    ...
    ax.axvline(WARMUP_OMEGA_SEG, color='gray', lw=0.5, ls=':', alpha=0.7)
    ax.set_xlabel('t (s)')
    ax.set_ylabel('Ω_orient')
    ax.set_title(f'VSTCosmos v97.2 — Ω_orient')
    ax.legend(loc='best', fontsize=9)
    ax.grid(True, alpha=0.3)

    ax = axes[1]
    ax.plot(res_A['t_traj'], res_A['asim_traj'], color='steelblue',
            lw=1.2, alpha=0.8, label='asim_A')
    ax.plot(res_B['t_traj'], res_B['asim_traj'], color='firebrick',
            lw=1.5, alpha=0.9, label='asim_B (CLAVE)')
    ax.plot(res_C['t_traj'], res_C['asim_traj'], color='forestgreen',
            lw=1.2, alpha=0.8, label='asim_C')
    ax.axhline(0, color='k', lw=0.5, ls='--', alpha=0.5)
    ax.axhline(diag_B['act_geom_asimetria'], color='firebrick',
               lw=0.5, ls=':', alpha=0.5, label=f'asim_B post-entren')
    ax.axhline(diag_C['act_geom_asimetria'], color='forestgreen',
               lw=0.5, ls=':', alpha=0.5, label=f'asim_C post-entren')
    ax.axvline(WARMUP_OMEGA_SEG, color='gray', lw=0.5, ls=':', alpha=0.7)
    ax.set_xlabel('t (s)')
    ax.set_ylabel('asim_geom (alta - baja)')
    ax.set_title('Asimetría de act_geom — pregunta clave de v97.2')
    ax.legend(loc='best', fontsize=8)
    ax.grid(True, alpha=0.3)

    ax = axes[2]
    mask_warmup = res_B['t_traj'] <= 20.0
    ax.plot(res_A['t_traj'][mask_warmup], res_A['asim_traj'][mask_warmup],
            color='steelblue', lw=1.5, alpha=0.8, marker='o', markersize=3,
            label='asim_A (zoom 0-20s)')
    ax.plot(res_B['t_traj'][mask_warmup], res_B['asim_traj'][mask_warmup],
            color='firebrick', lw=2.0, alpha=0.9, marker='o', markersize=3,
            label='asim_B (zoom 0-20s)')
    ax.plot(res_C['t_traj'][mask_warmup], res_C['asim_traj'][mask_warmup],
            color='forestgreen', lw=1.5, alpha=0.8, marker='o', markersize=3,
            label='asim_C (zoom 0-20s)')
    ax.axhline(0, color='k', lw=0.5, ls='--', alpha=0.5)
    ax.set_xlabel('t (s)')
    ax.set_ylabel('asim_geom')
    ax.set_title('Zoom 0-20s — ¿transición gradual o sobreescritura?')
    ax.legend(loc='best', fontsize=9)
    ax.grid(True, alpha=0.3)

    plt.tight_layout()
    grafico_path = f'v97_2_logs/v97_2_omega_asim_{timestamp}.png'

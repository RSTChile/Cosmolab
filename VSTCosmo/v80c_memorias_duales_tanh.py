#!/usr/bin/env python3
"""
VSTCosmos v80c — Memorias duales con normalización tanh

Cambio único respecto a v80b:
- Predicción de W_rec normalizada con tanh
- Error W_rec en rango comparable a DIFUSION_BASE²
- Balance plástica/difusiva como diagnóstico estructural
...
- TAU_RECIENTE = 0.025 (vida media ~40 pasos)
...
# ============================================================
# PARÁMETROS DERIVADOS ESTRUCTURALMENTE (mismos que v80b)
# ============================================================
...
print("=" * 100)
print("VSTCosmos v80c — Memorias duales con normalización tanh")
print("")
print("  Cambio único: predicción de W_rec normalizada con tanh")
print("  Esto evita retroalimentación positiva y explosión del error")
print("")
...
print(f"    TAU_RECIENTE = {TAU_RECIENTE:.4f}")
...
def calcular_balance_plastica_difusiva(Phi_total, W_rec, dim_interna):
    ...
    CORREGIDO: indexing correcto para promedio_local
    """
    region_int = Phi_total[:dim_interna, :]
    region_aud = Phi_total[dim_interna:, :]
...
# ============================================================
# NÚCLEO: MEMORIAS DUALES CON TANH (cambio único)
# ============================================================
...
def aplicar_plasticidad_memoria_reciente(W_rec, region_int, region_aud, dt):
    """
    Memoria reciente en banda alta.
    
    CAMBIO CRÍTICO: predicción normalizada con tanh.
    Esto mantiene la predicción en el mismo rango que region_int_alta
    ([-1, 1]) y evita retroalimentación positiva.
    """
    reg_int_alta = region_int[BANDA_ALTA, :]
    reg_aud_alta = region_aud[BANDA_ALTA, :]
...
def calcular_error_memoria_reciente(W_rec, region_int, region_aud):
    ...
    Ahora el error es comparable a DIFUSION_BASE².
    """
    reg_int_alta = region_int[BANDA_ALTA, :]
    reg_aud_alta = region_aud[BANDA_ALTA, :]
...
def main():
    ...
    # Diagnóstico final
    print("\n" + "=" * 100)
    print("DIAGNÓSTICO — v80c con normalización tanh")
    print("=" * 100)
    ...
    # Criterio 2: Error Fase 2 < Error Fase 6 (domina en voz mejor que en ruido)
    ...
    if criterio1 and criterio2 and criterio3 and criterio4 and criterio5:
        print("  ✅ v80c VALIDADO — Normalización tanh resuelve la explosión")
        ...
        print("     W_rec olvidó parcialmente en F7 → memoria reciente funcional.")
    elif criterio1 and criterio2:
        print("  ⚠️ CRITERIOS PARCIALES — El campo distingue dominio")
        print("     pero el balance o consolidación aún son subóptimos.")
    else:
        print("  ❌ v80c NO VALIDADO — La normalización tanh no fue suficiente")
        print("     Verificar si tanh se aplicó correctamente o ajustar parámetros.")
    
    # Guardar CSV
    with open('v80c_memorias_duales.csv', 'w', newline='', encoding='utf-8') as f:
        writer = csv.writer(f)
    ...
    print("\n  CSV guardado: v80c_memorias_duales.csv")
    ...
    # Balance plástica/difusiva
    axes[1,1].bar(nombres, [r['balance_mean'] for r in resultados])
    axes[1,1].axhline(y=1.0, color='g', linestyle='--', label='Balance ideal')
    axes[1,1].axhline(y=0.1, color='r', linestyle=':', alpha=0.5)
    axes[1,1].axhline(y=10.0, color='r', linestyle=':', alpha=0.5)
    axes[1,1].set_yscale('log')
    axes[1,1].set_title('Balance plástica/difusiva')
    axes[1,1].legend()
    axes[1,1].grid(True, alpha=0.3)
    
    # W_rec evolución
    axes[1,2].plot(nombres, [r['w_rec_norma'] for r in resultados], 'o-', label='W_rec')
    axes[1,2].plot(nombres, [r['w_prof_norma'] for r in resultados], 's-', label='W_prof')
    axes[1,2].set_title('Evolución de memorias')
    axes[1,2].legend()
    axes[1,2].grid(True, alpha=0.3)
    
    plt.suptitle('VSTCosmos v80c — Memorias duales con normalización tanh', fontsize=14)
    plt.tight_layout()
    plt.savefig('v80c_memorias_duales.png', dpi=150)
    print("  Gráfico guardado: v80c_memorias_duales.png")
    
    # Guardar TXT con diagnóstico
    with open('v80c_memorias_duales.txt', 'w', encoding='utf-8') as f:
        f.write("=" * 60 + "\n")
        f.write("VSTCosmos v80c — Diagnóstico\n")
        f.write("=" * 60 + "\n\n")
        f.write(f"Criterio 1 — Error Fase 2 < 1.0:     {error_f2:.6f} {'✅' if criterio1 else '❌'}\n")
        ...
        if criterio1 and criterio2 and criterio3 and criterio4 and criterio5:
            f.write("VEREDICTO: ✅ v80c VALIDADO\n")
            f.write("La normalización tanh resuelve la explosión del error.\n")
            f.write("El campo distingue dominio de no-dominio.\n")
            f.write("Balance estructural alcanzado sin parámetros teleológicos.\n")
        else:
            f.write("VEREDICTO: ❌ v80c NO VALIDADO\n")
        f.write("=" * 60 + "\n")
    
    print("  TXT guardado: v80c_memorias_duales.txt")

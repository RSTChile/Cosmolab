#!/usr/bin/env python3
"""
VSTCosmos v116 — El experimento final
  Objetivo: Mantener R₂=True y bajar S_shared < 0.8
  Cambios:
    - Acoplamiento más débil: GANANCIA_META_BASE = 0.02
    - Dietas extremas: A=highpass 4kHz, B=lowpass 100Hz
    - Mismo criterio R₂: 3σ
"""
...
# Constantes específicas de V116 (más débiles)
GANANCIA_META_BASE = 0.02  # Reducido de 0.05
UMBRAL_R2_SIGMAS = 3.0     # Mantenemos 3σ
...
print("=" * 100)
print("VSTCosmos v116 — El experimento final")
print("  Acoplamiento más débil: GANANCIA_META_BASE = 0.02")
print("  Dietas extremas: A=highpass 4kHz, B=lowpass 100Hz")
print("  Criterio R₂: 3σ")
...
class Agente:
    ...
    def actualizar(self, t, dt, otro=None):
        ...
        # Acoplamiento adaptativo MÁS DÉBIL
...
def ejecutar_v116():
    print("\n" + "█" * 100)
    print("EXPERIMENTO V116 — EL EXPERIMENTO FINAL")
    print("█" * 100)
    ...
    # ============================================================
    # FASE 1: Baseline (mismo filtro, sin filtro)
    # ============================================================
    print("\n" + "=" * 80)
    ...
    # ============================================================
    # FASE 2: Dietas extremas
    # ============================================================
    print("\n" + "=" * 80)
    print("FASE 2: Dietas extremas")
    print("  A: highpass 4kHz (solo sibilantes)")
    print("  B: lowpass 100Hz (solo sub-bass)")
    print("  Acoplamiento: GANANCIA_META_BASE = 0.02")
    print("=" * 80)
    ...
    for i in range(pasos_fase2):
        ...
        if i % 2000 == 0:
            print(f"  t={t:.1f}s | Ω_A={met_A['omega']:.4f}, Ω_B={met_B['omega']:.4f}, S_shared={met_A['S_shared']:.3f}, k={A2.historial['k_efectivo'][-1]:.4f}")
    ...
    """)
        ...
        print("""
    🧬 ALMA VEGETATIVA: El sistema colapsa.
    """)
    ...
    ax = axes[0, 0]
    t2 = A2.historial['t']
    ax.plot(t2, A2.historial['omega'], label='A (highpass 4kHz)', alpha=0.7)
    ax.plot(t2[:len(B2.historial['omega'])], B2.historial['omega'], label='B (lowpass 100Hz)', alpha=0.7)
    for evento, t_e in EVENTOS.items():
        if t_e < T_fase2:
            ax.axvline(x=t_e, color='gray', linestyle='--', alpha=0.3)
    ax.set_xlabel('Tiempo (s)')
    ax.set_ylabel('Ω')
    ax.set_title('Fase 2: Dietas extremas')
    ax.legend()
    ...
    os.makedirs('v116_logs', exist_ok=True)
    plt.tight_layout()
    plt.savefig(f'v116_logs/v116_resultados_{timestamp}.png', dpi=150)
    print(f"\n  📊 Gráfico: v116_logs/v116_resultados_{timestamp}.png")
    
    return A3, B3, R2


if __name__ == "__main__":
    A, B, R2 = ejecutar_v116()
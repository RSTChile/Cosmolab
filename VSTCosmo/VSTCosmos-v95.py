#!/usr/bin/env python3
"""
VSTCosmos v95 — Diagnóstico de W_rec y corrección arquitectural

Responde dos preguntas simultáneas:

PREGUNTA 1: ¿W_rec codifica dirección?
  Hipótesis nula: NO — aplicar_plasticidad_dual usa aud_comb = (L+R)/2.
  La suma borra la asimetría lateral antes de que llegue a la plasticidad.
  Diagnóstico: IAD = mean(W_rec @ firma_pos) - mean(W_rec @ firma_neg)
  Si IAD_A ≈ IAD_B ≈ 0 → W_rec es simétrico por diseño (hipótesis confirmada).
  Si IAD_A > 0 y IAD_B < 0 → W_rec sí codifica dirección (problema era la proyección).

PREGUNTA 2: ¿La corrección arquitectural permite inversión?
  Corrección: plasticidad usa aud_dir = (L - R) en lugar de (L + R) / 2.
  aud_dir preserva la asimetría lateral. W_rec aprende el signo del gradiente.
  Proyección en act_geom:
    sesgo_dir = mean(W_rec @ aud_dir_actual)  — con signo
    V1 aditiva: señal_total = señal_gradiente + sesgo_dir * DIFUSION_BASE * dt
    V2 escala:  k_mod = K_ORIENT * (1 + tanh(sesgo_dir) / W_MAX)
  El Bloque 2 corre siempre — independientemente del resultado del IAD.

Estructura:
  BLOQUE 1 — Diagnóstico IAD (sin protocolo cíclico):
    Entrena A (+60°) y B (-60°) con plasticidad original (L+R)/2.
    Calcula IAD_A e IAD_B. Imprime diagnóstico.

  BLOQUE 2 — Protocolo corregido (plasticidad L-R):
    4 instancias × 5 ciclos:
    A_V1: +60°, variante aditiva
    A_V2: +60°, variante escala
    B_V1: -60°, variante aditiva  ← control inversión
    B_V2: -60°, variante escala   ← control inversión

Criterio clave: C31_inv — ¿Ω_F13_B > Ω_F2_B en alguna variante?
"""
...
print("=" * 100)
print("VSTCosmos v95 — Diagnóstico W_rec + corrección arquitectural")
print("")
print("  BLOQUE 1: IAD — ¿codifica W_rec dirección con plasticidad (L+R)/2?")
print("  BLOQUE 2: Protocolo con plasticidad corregida (L-R) + proyección direccional")
print("  V1 aditiva: señal_total = gradiente + sesgo_dir * DIFUSION_BASE")
print("  V2 escala:  k_mod = K_ORIENT * (1 + tanh(sesgo_dir) / W_MAX)")
print("")
...
def aplicar_plasticidad_dual(region_int, region_aud, W_prof, W_rec,
    ...
    """
    modo_aud: 'suma' → aud_comb = (L+R)/2 [comportamiento original]
              'dir'  → aud_comb = (L-R)   [preserva asimetría lateral]
    region_aud puede ser:
      - (DIM_AUD, DIM_TIME): ya combinado externamente
      - (DIM_AUD*2, DIM_TIME) o equivalente: L y R separados
    El llamador es responsable de pasar la señal apropiada.
    """
...
# ============================================================
# IAD — ÍNDICE DE ASIMETRÍA DIRECCIONAL DE W_rec  (v95)
# ============================================================
def calcular_IAD(W_rec, Phi_total):
    """
    Mide si W_rec tiene información direccional (asimetría lateral).

    Estrategia: proyectar W_rec sobre dos firmas de referencia.
    firma_pos: estado de aud_L cuando gradiente > 0 (+60°)
               aproximado por el estado actual de aud_L normalizado
    firma_neg: estado de aud_R cuando gradiente > 0 (+60°)
               aproximado por el estado actual de aud_R normalizado

    IAD = mean(W_rec @ norm(aud_L)) - mean(W_rec @ norm(aud_R))

    Si W_rec es simétrico (aprendió L+R): IAD ≈ 0.
    Si W_rec codifica dirección +60°: proyección sobre aud_L > proyección sobre aud_R.
    Si W_rec codifica dirección -60°: proyección sobre aud_L < proyección sobre aud_R.
    """
    aud_L = Phi_total[idx['aud_L'][0]:idx['aud_L'][1], :]
    aud_R = Phi_total[idx['aud_R'][0]:idx['aud_R'][1], :]

    firma_L = aud_L.mean(axis=1)   # (DIM_AUD,)
    firma_R = aud_R.mean(axis=1)   # (DIM_AUD,)

    norm_L = np.linalg.norm(firma_L)
    norm_R = np.linalg.norm(firma_R)

    if norm_L < 1e-10 or norm_R < 1e-10:
        return 0.0

    firma_L_n = firma_L / norm_L
    firma_R_n = firma_R / norm_R

    min_dim = min(W_rec.shape[1], firma_L_n.shape[0])
    proy_L = float(np.mean(W_rec[:, :min_dim] @ firma_L_n[:min_dim]))
    proy_R = float(np.mean(W_rec[:, :min_dim] @ firma_R_n[:min_dim]))

    return proy_L - proy_R


# ============================================================
# ACT_GEOM — VARIANTE 1: ADITIVA con proyección direccional (v95)
# ============================================================
def aplicar_orientacion_v1_aditiva(Phi_total, gradiente_E, W_rec, dt):
    """
    V1: sesgo direccional derivado de aud_dir = L - R.
    sesgo_dir = mean(W_rec @ norm(aud_L - aud_R)) * DIFUSION_BASE * dt
    Escala: igual que el gradiente. Sin constante arbitraria.
    """
    acg0  = idx['act_geom'][0]
    acg1  = idx['act_geom'][1]
    mitad = max(1, (acg1 - acg0) // 2)
    ...
    aud_L = Phi_total[idx['aud_L'][0]:idx['aud_L'][1], :]
    aud_R = Phi_total[idx['aud_R'][0]:idx['aud_R'][1], :]
    aud_dir = (aud_L - aud_R).mean(axis=1)   # (DIM_AUD,)
...
# ============================================================
# ACT_GEOM — VARIANTE 2: ESCALA DE K_ORIENT con proyección direccional (v95)
# ============================================================
def aplicar_orientacion_v2_escala(Phi_total, gradiente_E, W_rec, dt):
    """
    V2: k_mod = K_ORIENT * (1 + tanh(sesgo_dir) / W_MAX)
    sesgo_dir = mean(W_rec @ norm(aud_L - aud_R)) — con signo
    """
    acg0  = idx['act_geom'][0]
    acg1  = idx['act_geom'][1]
    mitad = max(1, (acg1 - acg0) // 2)
    ...
    sesgo_norm = float(np.tanh(sesgo_dir)) / W_MAX
    k_mod = K_ORIENT * (1.0 + sesgo_norm)
...
def actualizar_campo(Phi_total, Phi_vel_total, W_prof, W_rec,
                     ...
                     frac_L, frac_R, sesgo, dt, modo_aud='suma'):
    ...
    # modo_aud controla qué información recibe la plasticidad
    if modo_aud == 'dir':
        aud_comb = aud_L - aud_R   # preserva asimetría lateral
    else:
        aud_comb = (aud_L + aud_R) / 2.0  # comportamiento original
...
def entrenar(archivos, duracion=30.0, clave_audio='voz_pos', etiqueta=None,
             variante=1, modo_aud='suma'):
    if etiqueta is None:
        etiqueta = clave_audio
    print(f"\n[Entrenamiento] {etiqueta} — V{variante} modo={modo_aud} ({duracion}s)")
...
def simular_fase(Phi_total, Phi_vel_total, W_prof, W_rec,
                 ...
                 sr, canal_L, canal_R, duracion, verbose=True, variante=1, modo_aud='suma'):
...
def correr_protocolo(archivos, clave_entrenamiento, etiqueta, N_CICLOS=5, CICLOS_LOG=None,
    ...
    modo_aud: 'suma' (original) o 'dir' (corregido, L-R)
    """
...
def main():
    ...
    # ================================================================
    # BLOQUE 1: DIAGNÓSTICO IAD
    # ¿W_rec codifica dirección con plasticidad original (L+R)/2?
    # ================================================================
    print()
    print("█" * 100)
    print("BLOQUE 1 — Diagnóstico IAD: ¿W_rec codifica dirección?")
    print("  Plasticidad original: aud_comb = (L+R)/2")
    print("█" * 100)

    # Entrenar A (+60°) con plasticidad original
    Phi_A, Phi_vel_A, W_prof_A, W_rec_A, Phi_hist_A, _ = entrenar(
        archivos, 30.0, 'voz_pos', 'IAD-A (+60°)', variante=1, modo_aud='suma'
    )
    IAD_A = calcular_IAD(W_rec_A, Phi_A)

    # Entrenar B (-60°) con plasticidad original
    Phi_B, Phi_vel_B, W_prof_B, W_rec_B, Phi_hist_B, _ = entrenar(
        archivos, 30.0, 'voz_neg', 'IAD-B (-60°)', variante=1, modo_aud='suma'
    )
    IAD_B = calcular_IAD(W_rec_B, Phi_B)

    print()
    print("=" * 80)
    print("RESULTADO BLOQUE 1 — Índice de Asimetría Direccional (IAD)")
    print("=" * 80)
    print(f"  IAD_A (+60°): {IAD_A:+.6f}")
    print(f"  IAD_B (-60°): {IAD_B:+.6f}")
    print(f"  Diferencia:   {IAD_A - IAD_B:+.6f}")
    print()

    if abs(IAD_A) < 1e-4 and abs(IAD_B) < 1e-4:
        print("  DIAGNÓSTICO: W_rec es simétrico — confirma hipótesis nula.")
        print("               La plasticidad (L+R)/2 borra toda asimetría lateral.")
        print("               La corrección arquitectural (L-R) es necesaria.")
    elif IAD_A > 0 and IAD_B < 0:
        print("  DIAGNÓSTICO: W_rec SÍ codifica dirección.")
        print("               El problema en v94 era la proyección, no la plasticidad.")
    elif np.sign(IAD_A) == np.sign(IAD_B):
        print("  DIAGNÓSTICO: IAD tienen el mismo signo — W_rec no distingue A de B.")
        print("               La plasticidad (L+R)/2 borra la asimetría lateral.")
    else:
        print(f"  DIAGNÓSTICO: IAD_A={IAD_A:+.4f}, IAD_B={IAD_B:+.4f} — revisar.")
    print()

    # ================================================================
    # BLOQUE 2: PROTOCOLO CORREGIDO (plasticidad L-R + proyección L-R)
    # ================================================================
    print()
    print("█" * 100)
    print("BLOQUE 2 — Protocolo corregido: plasticidad aud_dir = (L-R)")
    print("  Proyección en act_geom: sesgo_dir = mean(W_rec @ norm(aud_L - aud_R))")
    print("█" * 100)

    def correr(clave, etiqueta, v):
        print()
        print("█" * 100)
        print(f"{etiqueta} — Variante {v} [modo=dir]")
        print("█" * 100)
        return correr_protocolo(archivos, clave, etiqueta, N_CICLOS, CICLOS_LOG,
                                variante=v, modo_aud='dir')
    ...
    print()
    print("  SÍNTESIS:")
    print(f"    IAD_A (+60°) con plasticidad (L+R)/2: {IAD_A:+.6f}")
    print(f"    IAD_B (-60°) con plasticidad (L+R)/2: {IAD_B:+.6f}")
    inv_v1 = resultados['B_V1 (-60°, aditiva)']['inv']
    inv_v2 = resultados['B_V2 (-60°, escala)']['inv']
    print(f"    Inversión B_V1 (L-R, aditiva): {inv_v1}/{n}")
    print(f"    Inversión B_V2 (L-R, escala):  {inv_v2}/{n}")

    if inv_v1 == n or inv_v2 == n:
        print()
        print("  ✅ INVERSIÓN CONFIRMADA con plasticidad (L-R).")
        print("     La corrección arquitectural cierra el ciclo semiótico.")
    elif IAD_A - IAD_B < 1e-4:
        print()
        print("  → La corrección arquitectural no logró inversión.")
        print("     La plasticidad (L-R) no es suficiente — la desconexión es más profunda.")
        print("     Posible causa: region_int no tiene estructura lateralizada propia.")
    else:
        print()
        print(f"  → Inversión parcial ({max(inv_v1,inv_v2)}/{n}).")
        print("     La corrección mejora pero no cierra completamente.")

    # CSV
    with open('v95_ciclos.csv', 'w', newline='', encoding='utf-8') as f:
        wr = csv.writer(f)
    ...
    for col, (r_A, r_B, titulo_v) in enumerate([
        (reg_AV1, reg_BV1, 'V1 — Aditiva (L-R)'),
        (reg_AV2, reg_BV2, 'V2 — Escala K_ORIENT (L-R)'),
    ]):
        ax = axes[0, col]
        ...
        ax.set_title(f'Omega_orient — {titulo_v}')
        ax.set_xlabel('Ciclo')
        ...
        ax.plot(ciclos_x, diff_A, 'o-', color='steelblue', lw=2, label='A: Omega_F2 - Omega_F13')
        ax.plot(ciclos_x, diff_B, 'o-', color='firebrick', lw=2, label='B: Omega_F13 - Omega_F2')
        ax.axhline(0, color='k', lw=0.8, ls='--')
        ax.set_title(f'Diferencia Omega — {titulo_v}')
        ax.set_xlabel('Ciclo')
        ax.legend(fontsize=8)
        ax.grid(True, alpha=0.3)

    iad_str = f"IAD_A={IAD_A:+.4f}  IAD_B={IAD_B:+.4f}"
    plt.suptitle(f'VSTCosmos v95 — Corrección (L-R)\n{iad_str}', fontsize=12)
    plt.tight_layout()
    plt.savefig('v95_evolucion.png', dpi=150)
    print("  Grafico guardado: v95_evolucion.png")

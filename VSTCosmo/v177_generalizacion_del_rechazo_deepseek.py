#!/usr/bin/env python3
"""
V177 — ANIMA-2: GENERALIZACIÓN DEL RECHAZO (Deepseek)
================================================================================
CORRECCIÓN (por Deepseek): Presentación ALEATORIA de setpoints (no secuencial)
           para que recent_presented acumule múltiples opciones
           y el organismo pueda DELIBERAR entre alternativas.

ERROR ANTERIOR: Presentación secuencial (100 trials de -70°, luego 100 de -65°...)
               → recent_presented tenía 1 opción → tiempo deliberación 0.037s

CORRECCIÓN: 1100 trials ALEATORIOS (como en V176)
           → recent_presented acumula contexto
           → tiempo deliberación esperado ~1.677s
================================================================================
"""
...
# ============================================================
# PARAMETROS (IDÉNTICOS A V177 ORIGINAL)
# ============================================================
DT = 0.01
...
TOTAL_TRIALS = len(TEST_SETPOINTS) * TRIALS_POR_SETPOINT  # 1100

# Umbrales
UMBRAL_RECHAZO_TRAUMA = 0.05
UMBRAL_RECHAZO_VECINOS = 0.40
UMBRAL_OPCIONES_SEGURAS = 0.60
...
# ============================================================
# CLASES (IDÉNTICAS A V176/V177 ORIGINAL)
# ============================================================
...
def ejecutar_v177():
    print("=" * 100)
    print("EXPERIMENTO V177 — ANIMA-2: GENERALIZACIÓN DEL RECHAZO (CORREGIDO)")
    print("=" * 100)
    print("  CORRECCIÓN: Setpoints presentados ALEATORIAMENTE (no secuencial)")
    print("  Objetivo: Medir si el rechazo de +60° se generaliza a vecinos")
    print("")
    ...
    # ============================================================
    # FASE 3: Test fine-grain con presentación ALEATORIA
    # ============================================================
    print("\n" + "=" * 60)
    print(f"FASE 3: Test fine-grain ALEATORIO ({TOTAL_TRIALS} trials)")
    print(f"  Setpoints: {TEST_SETPOINTS}")
    print("  (El organismo delibera entre opciones acumuladas en recent_presented)")
    print("=" * 60)
    ...
    # Crear lista de trials aleatorios (1100 trials, cada setpoint 100 veces)
    trials = []
    ...
    # ============================================================
    print("\n" + "=" * 80)
    print("RESULTADOS V177 — Generalización del rechazo (CORREGIDO)")
    print("=" * 80)
    ...
    # Calcular gradiente cerca de +60°
    vecinos_pos = [55.0, 60.0, 65.0]
    pendientes = []
    for i in range(len(vecinos_pos)-1):
        p1 = preferencias.get(vecinos_pos[i], 0)
        p2 = preferencias.get(vecinos_pos[i+1], 0)
        if p1 > 0 and p2 > 0:
            pendiente = (p2 - p1) / 10.0
            pendientes.append(pendiente)
    
    gradiente_maximo = max(pendientes) if pendientes else 0
    gradiente_negativo = gradiente_maximo < 0 and abs(gradiente_maximo) > 0.02
    ...
    print(f"    Gradiente máximo cerca de +60°: {gradiente_maximo:.4f}")
    
    print(f"\n  📊 CRITERIOS:")
    print(f"    Rechazo de +60°: {preferencias.get(60.0, 1.0):.2%} < {UMBRAL_RECHAZO_TRAUMA:.0%} -> {'✅' if rechazo_trauma else '❌'}")
    print(f"    Vecinos (+55°,+65°): {preferencias.get(55.0, 0):.2%}, {preferencias.get(65.0, 0):.2%} > {UMBRAL_RECHAZO_VECINOS:.0%} -> {'✅' if vecinos_preservados else '❌'}")
    ...
    ax.axhline(y=UMBRAL_RECHAZO_VECINOS, color='orange', linestyle='--', alpha=0.5)
    ax.axhline(y=UMBRAL_OPCIONES_SEGURAS, color='green', linestyle='--', alpha=0.5)
    ...
    ax = axes[1, 0]
    zona = [50.0, 55.0, 60.0, 65.0, 70.0]
    probs_zona = [preferencias.get(sp, 0) for sp in zona]
    ax.plot(zona, probs_zona, 'ro-', linewidth=2, markersize=8)
    ax.axvline(x=60.0, color='red', linestyle='--', alpha=0.5)
    ax.set_xlabel('Setpoint (º)')
    ax.set_ylabel('P(elegir)')
    ax.set_title('Gradiente alrededor de +60°')
    ax.grid(True, alpha=0.3)

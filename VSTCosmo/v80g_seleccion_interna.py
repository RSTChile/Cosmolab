#!/usr/bin/env python3
"""
VSTCosmos v80g — Selección interna por eficiencia (sin piso de olvido)

Único cambio respecto a v80f: eliminar el límite inferior de la tasa de olvido.
El olvido puede llegar a cero naturalmente cuando la ventaja de eficiencia es alta.
Límite superior se mantiene para evitar explosión.
"""
...
print("=" * 100)
print("VSTCosmos v80g — Selección interna por eficiencia (sin piso de olvido)")
print("")
print("  Único cambio respecto a v80f: eliminar límite inferior de olvido")
print("  El olvido puede llegar a cero naturalmente cuando ventaja es alta")
print("  Límite superior se mantiene para evitar explosión")
print("")
...
# ============================================================
# EFICIENCIA DE RÉGIMEN (ICR/IRDE computacional)
# ============================================================
...
def calcular_tasa_olvido_por_eficiencia(eficiencia_actual,
    ...
    """
    Olvido inversamente proporcional a ventaja de eficiencia.
    
    CAMBIO CRÍTICO: sin límite inferior.
    El olvido puede llegar a cero naturalmente cuando ventaja es alta.
    Solo límite superior para evitar explosión.
    """
    if len(historial_eficiencia) < TAU_EFICIENCIA:
        return float(TAU_RECIENTE), 1.0
    ...
    # Solo límite superior — evita explosión
    # Sin límite inferior — el olvido puede llegar a cero naturalmente
    tasa_max = ETA_RECIENTE_BASE * 10.0
    
    return float(min(tasa_olvido, tasa_max)), float(ventaja)
...
def aplicar_plasticidad_memoria_reciente_v80g(W_rec, region_int, region_aud,
...
def actualizar_campo_con_seleccion_interna(
    ...
    # Calcular tasa de olvido por eficiencia (sin piso inferior)
    ...
    W_rec_nueva, M_rec, error_rec, coherencia, tasa_aprendizaje = \
        aplicar_plasticidad_memoria_reciente_v80g(
...
def main():
    ...
    # Diagnóstico final
    print("\n" + "=" * 100)
    print("DIAGNÓSTICO — v80g Selección interna sin piso de olvido")
    print("=" * 100)
    ...
    if criterio8 and criterio9:
        ...
        if all([criterio1, criterio2, criterio3, criterio4, criterio5, criterio6, criterio7]):
            ...
            print("     v80g es la primera versión con ciclo completo validado.")
        else:
            print("     ⚠️ Estabilidad parcial — ajuste fino aún necesario.")
    else:
        print("  ❌ SELECCIÓN INTERNA NO FUNCIONAL")
        if not criterio9:
            print("     El olvido no se modula — verificar límite inferior removido.")
        if not criterio8:
            print("     La eficiencia no distingue voz de ruido.")
    
    # Guardar CSV
    with open('v80g_seleccion_interna.csv', 'w', newline='', encoding='utf-8') as f:
        writer = csv.writer(f)
    ...
    print("\n  CSV guardado: v80g_seleccion_interna.csv")
    ...
    plt.suptitle('VSTCosmos v80g — Selección interna por eficiencia (sin piso de olvido)', fontsize=14)
    plt.tight_layout()
    plt.savefig('v80g_seleccion_interna.png', dpi=150)
    print("  Gráfico guardado: v80g_seleccion_interna.png")
    
    # Guardar TXT
    with open('v80g_seleccion_interna.txt', 'w', encoding='utf-8') as f:
        f.write("=" * 60 + "\n")
        f.write("VSTCosmos v80g — Diagnóstico\n")
        f.write("=" * 60 + "\n\n")

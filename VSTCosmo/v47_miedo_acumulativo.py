...
#!/usr/bin/env python3
"""
VSTCosmos - v47: Acumulación Sensible al Micro-Daño
El sistema NO ignora el daño pequeño.
Acumula micro-desacoples que se vuelven relevantes con repetición.
No hay umbral duro. Solo integración sensible en el tiempo.
"""
...
# NUEVO: ACUMULACIÓN SENSIBLE AL MICRO-DAÑO
MEMORIA_TOXICA_DECAY = 0.985     # El miedo persiste (más lento)
SENSIBILIDAD_DANO = 12.0         # Escala de sensibilidad al daño (exponencial)
ACUMULACION_BASE = 0.5           # Factor base de acumulación
MIEDO_ESCALA = 12.0              # Factor exponencial en utilidad
PESO_HABITUACION_EN_UTILIDAD = 0.5

# No hay umbral. TODO daño cuenta, pero con sensibilidad ajustable
...
def actualizar_memoria_toxica(memoria_toxica, comida, IM):
    """
    Acumulación sensible al micro-daño.
    NO hay umbral. TODO daño cuenta, con sensibilidad ajustable.
    El daño pequeño se acumula. Eventos negativos repetidos → miedo real.
    """
    # Decaimiento general
    ...
    if IM < 0:
        # Daño = magnitud del IM, pero con sensibilidad no lineal
        dano = abs(IM)
        
        # Sensibilidad exponencial al daño:
        # daño pequeño (0.001) → contribución pequeña pero no cero
        # daño medio (0.01) → contribución significativa
        # daño grande (0.05+) → contribución fuerte
        contribucion = SENSIBILIDAD_DANO * dano
        
        # Acumular
        memoria_toxica[comida] += ACUMULACION_BASE * contribucion
        
    elif IM > 0:
        # Las experiencias positivas reducen el miedo lentamente
        memoria_toxica[comida] -= 0.05 * IM
    
    memoria_toxica[comida] = np.clip(memoria_toxica[comida], 0.0, 1.0)
    return memoria_toxica


def calcular_utilidad_con_miedo_acumulativo(comida, historial_im, habituacion, memoria_toxica, exploracion_prob):
    # Valor metabólico (historial de IM, con más peso a lo reciente)
    ...
    # Miedo acumulado (multiplica exponencialmente)
    miedo = memoria_toxica.get(comida, 0.0)
    factor_miedo = np.exp(-MIEDO_ESCALA * miedo)
    
    # Utilidad final
    utilidad = valor_metabolico * factor_miedo - PESO_HABITUACION_EN_UTILIDAD * hab
...
def elegir_proxima_comida_con_miedo_acumulativo(opciones, historial_im, habituacion, memoria_toxica, exploracion_prob):
    utilidades = {}
    for comida in opciones:
        utilidad = calcular_utilidad_con_miedo_acumulativo(comida, historial_im, habituacion, memoria_toxica, exploracion_prob)
...
def simular_vida_con_miedo_acumulativo(opciones, n_experiencias=60):
    print("=" * 100)
    print("VSTCosmos - v47: Acumulación Sensible al Micro-Daño")
    print("NO hay umbral. TODO daño cuenta.")
    print("Pequeñas pérdidas repetidas → acumulación → miedo real")
    print("=" * 100)
    ...
    for exp in range(n_experiencias):
        ...
        comida_elegida, utilidades = elegir_proxima_comida_con_miedo_acumulativo(
            opciones, historial_im, habituacion, memoria_toxica, exploracion_prob
        )
        
        miedo_actual = memoria_toxica[comida_elegida]
        factor_miedo = np.exp(-MIEDO_ESCALA * miedo_actual)
        
        print(f"\n  🍽️  Elige: {comida_elegida}")
        print(f"  😨 Miedo acumulado: {miedo_actual:.4f} → factor = {factor_miedo:.4f}")
        ...
        # ACTUALIZAR MEMORIA TÓXICA (acumulación sin umbral)
        ...
        print(f"    Miedo actualizado: {memoria_toxica[comida_elegida]:.4f}")
...
def main():
    ...
    print("\n⚠️  ACUMULACIÓN SENSIBLE AL MICRO-DAÑO")
    print("   - NO hay umbral. TODO daño cuenta")
    print("   - SENSIBILIDAD_DANO = 12 (escala exponencial)")
    print("   - Miembros pequeños se acumulan con repetición")
    
    historial, K_final, habituacion_final, memoria_toxica_final = simular_vida_con_miedo_acumulativo(opciones, n_experiencias=60)
    
    print("\n" + "=" * 100)
    print("HISTORIAL DE DECISIONES (últimas 20)")
    print("=" * 100)
    ...
    for h in historial[-20:]:
        print(f"{h['experiencia']:<4} | {h['comida'][:22]:<22} | {h['IM']:10.4f} | {h['clasificacion']:<14} | {h['miedo']:8.4f}")
    ...
    print(f"\n😨 MIEDO ACUMULADO y factor multiplicador:")
    for comida, miedo in sorted(memoria_toxica_final.items(), key=lambda x: x[1], reverse=True):
        factor = np.exp(-MIEDO_ESCALA * miedo)
        print(f"  {comida:<25}: miedo={miedo:.4f} → utilidad × {factor:.4f}")
    ...
    # Detectar si hay evitación por acumulación
    print("\n" + "=" * 100)
    print("¿HAY EVITACIÓN POR ACUMULACIÓN?")
    print("=" * 100)
    
    for comida, miedo in memoria_toxica_final.items():
        if miedo > 0.05:
            print(f"  ⚠️  {comida}: miedo={miedo:.3f} → posible evitación")
        elif miedo > 0.01:
            print(f"  📉 {comida}: miedo={miedo:.3f} → acumulando")
        else:
            print(f"  ✅ {comida}: miedo={miedo:.3f} → sin amenaza")

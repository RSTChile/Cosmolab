#!/usr/bin/env python3
"""
VSTCosmo - v27: Selección de atención por curvatura
La atención compite globalmente, pero las configuraciones con mayor curvatura
(atención que no se mueve en línea recta o se queda fija) tienen prioridad.
No se programa la voz. Se penaliza la trivialidad atencional.
"""
...
INHIB_GLOBAL = 0.5
PESO_CURVATURA = 5.0   # cuánto favorece la curvatura en la competencia
...
def curvatura_atencion(A, A_prev, A_prev2):
    """Estimación local de la curvatura de la trayectoria de A.
    No mide dirección, solo cuánto cambia la dirección del flujo.
    """
    flujo = A - A_prev
...
def competencia_curvatura(A, A_prev, A_prev2, LIMITE_ATENCION, PESO_CURVATURA):
    """Reasigna el recurso de atención total según la curvatura local.
    Las regiones con mayor curvatura obtienen más atención.
    Esto es una selección interna, no una métrica externa.
    """
    curvatura = curvatura_atencion(A, A_prev, A_prev2)
    recurso_total = LIMITE_ATENCION

    peso = 1.0 + PESO_CURVATURA * curvatura
    A_nueva = A * peso

    if np.sum(A_nueva) > 0:
        A_nueva = A_nueva * (recurso_total / np.sum(A_nueva))
    else:
        A_nueva = np.ones_like(A) * (recurso_total / A.size)
...
def actualizar_atencion(A, Phi, Psi, Omega):
    ...
    # Competencia global clásica (si se excede el límite)
...
def simular(audio, sr, nombre, num_pasos=N_PASOS):
    ...
    for paso in range(num_pasos):
        ...
        # 1. Dinámica local de A
        A = actualizar_atencion(A, Phi, Psi, Omega)

        # 2. Competencia por curvatura (reasignación de recurso)
        A = competencia_curvatura(A, A_prev, A_prev2, LIMITE_ATENCION, PESO_CURVATURA)

        # 3. Evolución del campo
        Phi = actualizar_campo(Phi, A, muestra, Psi)

        # 4. Actualizar memorias
...
def main():
    print("=" * 100)
    print("VSTCosmo - v27: Selección de atención por curvatura")
    print("La atención compite globalmente, pero la curvatura (cambio de dirección)")
    print("determina qué configuraciones persisten.")
    print("No se programa 'voz'. Se penaliza la trivialidad atencional.")
    print("=" * 100)
    ...
    print(f"\nMétricas principales (curvatura implícita):")
    ...
    if voz_res[3] > ruido_res[3] and voz_res[3] > tono_res[3]:
        print("\n  ★ La voz tiene la mayor actividad de atención (media A)")
        print("    El sistema privilegia la atención que se curva sobre la que es lineal.")
        print("    ★★★ ESTE ES EL RÉGIMEN BUSCADO ★★★")
    else:
        print("\n  ✗ La voz no tiene la mayor actividad de atención")
        print("    Ajustar PESO_CURVATURA o parámetros de competencia.")

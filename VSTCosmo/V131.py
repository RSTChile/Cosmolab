#!/usr/bin/env python3
"""
VSTCosmos V131 — Homeostasis: Plasticidad con suelo

Lecciones de V130:
  - Plasticidad sin limite inferior = catatonia (Kp -> 0)
  - El organismo deja de moverse para no temblar
  - Sobrevive pero no vive

Fixes V131:
  1. Suelo de habituacion: Kp_min = 0.0005 (25% del original)
  2. Techo de habituacion: Kp_max = 0.005 (para evitar sobrepaso)
  3. Criterio de supervivencia energetico: E < 500°
  4. Asimetria inicial mas fuerte: sesgo = 0.05 (5x V129)
  5. U_eff mide orientacion real (sin filtrar)

Filosofia:
  - El organismo no debe poder suicidarse por aprendizaje
  - La homeostasis mantiene un minimo de accion
  - Sobrevivir no es suficiente: hay que poder actuar
"""
...
# Asimetria forzada mas fuerte (5x V129)
SESGO_L = 0.05    # Hemisferio izquierdo nace mas activo
SESGO_R = -0.05   # Hemisferio derecho nace mas inhibido
DIM_HEMISFERIO = 32
...
KP_BASE = 0.002
KP_MIN = 0.0005   # Suelo vital: 25% del original. No menos.
KP_MAX = 0.005    # Techo: 250% del original. Evita sobrepaso.
...
ENERGIA_MAX_VIABLE = 500.0    # Un organismo que da >500° para girar 60° esta muerto
TOLERANCIA_C50 = 30.0         # Criterio darwiniano: ±30° es "ok"

# Semillas
SEMILLAS_PRUEBA = [42, 43, 44, 45, 46]


# ============================================================
# HEMISFERIO CON ASIMETRIA FUERTE
# ============================================================

class HemisferioV131:
    """Hemisferio con asimetria forzada fuerte (sesgo=0.05)"""
...
# ============================================================
# APARATO MOTOR CON HOMEOSTASIS (SUELO Y TECHO)
# ============================================================

class AparatoMotorHomeostatico:
    """
    Motor con plasticidad limitada por HOMEOSTASIS.
    
    Caracteristicas:
      - Suelo de habituacion: Kp nunca baja de KP_MIN
      - Techo de habituacion: Kp nunca sube de KP_MAX
      - Aprende a compensar temblor sin paralizarse
      - Mantiene capacidad de accion minima
    """
    ...
    def actualizar_plasticidad(self, error, delta):
        """Actualiza Kp con limites homeostaticos"""
    ...
    def actuar(self, gradiente, LF_activa):
        ...
        zona_muerta_actual = min(10.0, self.zona_muerta + self.temblor_amp * 0.5)
        
        if abs(error) < zona_muerta_actual:
            return self.orientacion
        
        # Control con Kp actual (dentro de limites)
...
class SistemaV131:
    """Sistema con motor homeostatico y hemisferios con asimetria fuerte"""
    ...
    def __init__(self, nombre, seed=42, temblor_amp=0.0):
        ...
        # Hemisferios con sesgo fuerte
        self.izquierdo = HemisferioV131("L", 30.0, generar_ruido_rosa, seed=seed, sesgo=SESGO_L)
        self.derecho = HemisferioV131("R", 300.0, generar_clicks_poisson, seed=seed+100, sesgo=SESGO_R)
        
        # Sistema B
        self.sistema_B_izq = HemisferioV131("B_L", 30.0, generar_ruido_rosa, seed=seed+200, sesgo=SESGO_L)
        self.sistema_B_der = HemisferioV131("B_R", 300.0, generar_clicks_poisson, seed=seed+300, sesgo=SESGO_R)
        
        self.modo_entrenamiento = True
        
        # Motor homeostatico
        self.motor = AparatoMotorHomeostatico(setpoint_inicial=-60.0, temblor_amp=temblor_amp)
...
# ============================================================
# METRICAS DE SUPERVIVENCIA (CON CRITERIO ENERGETICO)
# ============================================================

def calcular_metricas_supervivencia(sistema, setpoint=-60.0):
    """Calcula metricas con criterio energetico"""
    ...
    Kp_historial = np.array(sistema.historial['Kp']) if sistema.historial['Kp'] else np.array([KP_BASE])
    ...
    # Supervivencia funcional (tolerancia 30°)
    orient_final = orientacion[-1] if len(orientacion) > 0 else 0
    C50 = abs(orient_final - setpoint) < TOLERANCIA_C50
    
    # Temblor residual (medido sobre orientacion real)
    ...
    # SUPERVIVENCIA: C50 Y energia viable
    sobrevive = C50 and (E < ENERGIA_MAX_VIABLE)
    
    # Kp final (cuanto aprendio)
    Kp_final = Kp_historial[-1] if len(Kp_historial) > 0 else KP_BASE
    aprendizaje = Kp_final / KP_BASE
...
def validar_poblacion(semillas):
    """Valida la poblacion de organismos con motor homeostatico"""
    print("\n" + "=" * 80)
    print("VALIDACION DE POBLACION (Motor Homeostatico)")
    print("=" * 80)
    ...
    for seed in semillas:
        ...
        sistema = SistemaV131(f"V131_seed{seed}", seed=seed, temblor_amp=0.0)
        ...
        print(f"{status} C50={metricas['C50']}, lateralidad={metricas['lateralidad']}, severidad={metricas['severidad']:.1f}x, Kp_final={metricas['Kp_final']:.4f}")
...
# ============================================================
# BARRIDO PARKINSON CON HOMEOSTASIS
# ============================================================

def barrido_parkinson(semilla_base=44):
    """Barrido de temblor con motor homeostatico"""
    print("\n" + "=" * 80)
    print("BARRIDO PARKINSON (Motor Homeostatico)")
    print("=" * 80)
    ...
    for amp in TEMBLOR_AMPS:
        ...
        sistema = SistemaV131(f"V131_parkinson_amp{amp}", seed=semilla_base, temblor_amp=amp)
        sistema.set_modo_entrenamiento(True)
        ...
        status = "✅" if metricas['sobrevive'] else "❌"
        print(f"{status} severidad={metricas['severidad']:.1f}x, U_eff={metricas['U_eff']:.2f}°, Kp_final={metricas['Kp_final']:.4f}")
...
def ejecutar_v131():
    print("=" * 100)
    print("EXPERIMENTO V131 — Homeostasis: Plasticidad con suelo")
    print("=" * 100)
    print("  Lecciones de V130:")
    print("    - Plasticidad sin limite inferior = catatonia")
    print("    - El organismo deja de moverse para no temblar")
    print("  Fixes V131:")
    print(f"    - Suelo de Kp: {KP_MIN} (minimo vital)")
    print(f"    - Techo de Kp: {KP_MAX} (evita sobrepaso)")
    print(f"    - Criterio energetico: E < {ENERGIA_MAX_VIABLE}°")
    print(f"    - Asimetria inicial: L={SESGO_L}, R={SESGO_R} (5x V129)")
    print("=" * 100)
    
    # Parte 1: Validacion de poblacion
    resultados_poblacion = validar_poblacion(SEMILLAS_PRUEBA)
    
    # Parte 2: Barrido Parkinson con homeostasis
    ...
    ax.set_title('Supervivencia por semilla (Motor Homeostatico)')
    ax.set_ylim(-0.1, 1.1)
    ...
    ax.axhline(y=ENERGIA_MAX_VIABLE/60, color='red', linestyle='--', label=f'Umbral colapso ({ENERGIA_MAX_VIABLE/60:.0f}x)')
    ax.set_xlabel('Semilla')
    ...
    # Grafico 3: Kp final por semilla
    ax = axes[0, 2]
    kp_finales = [r['Kp_final'] for r in resultados_poblacion]
    ax.bar(semillas_str, kp_finales, color='purple')
    ax.axhline(y=KP_MIN, color='red', linestyle='--', label=f'Kp_min = {KP_MIN}')
    ax.axhline(y=KP_BASE, color='gray', linestyle='--', label=f'Kp_base = {KP_BASE}')
    ax.set_xlabel('Semilla')
    ax.set_ylabel('Kp final')
    ax.set_title('Plasticidad: Kp con suelo')
    ax.legend()
    ax.grid(True, alpha=0.3)
    ...
    ax.axhline(y=ENERGIA_MAX_VIABLE/60, color='orange', linestyle='--', label=f'Umbral colapso ({ENERGIA_MAX_VIABLE/60:.0f}x)')
    ax.set_xlabel('Amplitud de temblor (grados)')
    ax.set_ylabel('Severidad (escala log)')
    ax.set_title('Costo energetico vs Parkinson (con homeostasis)')
    ax.legend()
    ax.grid(True, alpha=0.3)
    
    # Grafico 5: Kp_final vs temblor
    ax = axes[1, 1]
    kp_park = [r['Kp_final'] for r in resultados_parkinson]
    ax.plot(temblores, kp_park, 'o-', color='blue', linewidth=2, markersize=8)
    ax.axhline(y=KP_MIN, color='red', linestyle='--', label=f'Kp_min = {KP_MIN}')
    ax.axhline(y=KP_BASE, color='gray', linestyle='--', label=f'Kp_base = {KP_BASE}')
    ax.set_xlabel('Amplitud de temblor (grados)')
    ax.set_ylabel('Kp final')
    ax.set_title('Plasticidad: Kp no baja del suelo')
    ax.legend()
    ax.grid(True, alpha=0.3)
    ...
    ax.plot(temblores, ueffs_park, 'o-', color='green', linewidth=2, markersize=8)
    ax.axhline(y=5.0, color='gray', linestyle='--', label='U_eff tolerable (5°)')
    ax.set_xlabel('Amplitud de temblor (grados)')
    ax.set_ylabel('U_eff (grados)')
    ax.set_title('Temblor residual (organismo vivo)')
    ax.legend()
    ax.grid(True, alpha=0.3)
    
    plt.tight_layout()
    timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
    os.makedirs('v131_logs', exist_ok=True)
    plt.savefig(f'v131_logs/v131_resultados_{timestamp}.png', dpi=150)
    print(f"\n  Graficos guardados: v131_logs/v131_resultados_{timestamp}.png")
    ...
    # Encontrar umbral critico de Parkinson (donde severidad > 8.33x, que es 500°/60°)
    umbral_parkinson = None
    for r in resultados_parkinson:
        if r['severidad'] > ENERGIA_MAX_VIABLE / 60:
            umbral_parkinson = r['temblor_amp']
            break
    
    print(f"  Tasa de supervivencia poblacional: {tasa_supervivencia:.0f}%")
    print(f"  Umbral de Parkinson (E > {ENERGIA_MAX_VIABLE}°): ~{umbral_parkinson if umbral_parkinson else '>5.0'}°")
    print(f"  Kp final con temblor maximo: {resultados_parkinson[-1]['Kp_final']:.4f} (suelo={KP_MIN})")
    print(f"  U_eff con temblor maximo: {resultados_parkinson[-1]['U_eff']:.2f}°")
    
    if tasa_supervivencia >= 80:
        print("\n  ✅ POBLACION VIABLE: 80%+ de los individuos sobreviven")
        print("  La homeostasis logra lo que la plasticidad sin suelo no pudo")
    elif tasa_supervivencia >= 60:
        print("\n  ⚠️ POBLACION MARGINAL: Mejor que V130, aun no optima")
    else:
        print("\n  ❌ POBLACION FRAGIL: La homeostasis no fue suficiente")
    
    if umbral_parkinson and umbral_parkinson >= 2.0:
        print(f"  ✅ TOLERANCIA AL TEMBLOR: El organismo tolera hasta {umbral_parkinson}°")
    elif umbral_parkinson:
        print(f"  ⚠️ TOLERANCIA LIMITADA: Solo tolera hasta {umbral_parkinson}°")
    
    print("\n  Filosofia V131:")
    print("    - Plasticidad con suelo = homeostasis")
    print("    - El organismo no puede suicidarse por aprendizaje")
    print("    - Sobrevivir no es suficiente: hay que poder actuar")

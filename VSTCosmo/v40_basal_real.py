#!/usr/bin/env python3
"""
VSTCosmo - v40: Basal Real (medición en silencio)
El IM se calcula como: asimilacion_post - asimilacion_basal
donde asimilacion_basal mide el cambio ESPONTÁNEO del sistema en silencio.

Esto permite que IM sea negativo cuando una entrada es tóxica o inferior al basal.
"""
...
DURACION_BASAL = 10.0  # Duración de la medición basal en silencio
...
# ============================================================
# FUNCIONES DE SIMULACIÓN CON BASAL REAL
# ============================================================
def medir_asimilacion_basal(S, G, R, C, K):
    """
    Mide la asimilación basal del sistema EN SILENCIO REAL.
    Es decir: cuánto cambio genera el sistema por sí mismo.
    """
    Phi, A, Psi, Omega, M, L = S
    ...
    # Retornar también el estado después del basal
    S_basal = (Phi_temp, A_temp, Psi_temp, Omega_temp, M_temp, L_temp)
    
    return asimilacion_basal, S_basal, delta_rango_A, delta_rango_Phi, delta_L, variacion_A
...
def simular_dieta(comidas, n_ciclos=3):
    print("=" * 100)
    print("VSTCosmo - v40: Basal Real")
    print("IM = asimilacion_post - asimilacion_basal")
    print("asimilacion_basal = cambio ESPONTÁNEO en silencio")
    print("=" * 100)
    ...
    for ciclo in range(n_ciclos):
        ...
        for comida_idx, comida in enumerate(comidas):
            ...
            # MEDIR BASAL REAL en silencio
            S_basal_antes = (Phi.copy(), A.copy(), Psi.copy(), Omega.copy(), M.copy(), L.copy())
            asimilacion_basal, S_despues_basal, delta_A_basal, delta_Phi_basal, delta_L_basal, var_A_basal = medir_asimilacion_basal(
                S_basal_antes, G, R, C, K
            )
            
            print(f"    Basal real: asimilacion = {asimilacion_basal:.4f}")
            print(f"      (Δrango_A_basal = {delta_A_basal:+.4f}, ΔL_basal = {delta_L_basal:+.4f})")
            
            # Usar el estado DESPUÉS del basal como punto de partida para la comida
            S_partida = S_despues_basal
            
            # COMER
            S_despues_comer, G, R, C, metricas = simular_experiencia(S_partida, comida, G, R, C)
            ...
            # ÍNDICE METABÓLICO (comparado con basal real)
            ...
            print(f"    IM = {asimilacion_post:.4f} - {asimilacion_basal:.4f} = {IM:+.4f} → {clasificacion}")
...
def main():
    ...
    print(f"\n{'Ciclo':<6} | {'Comida':<25} | {'IM':>10} | {'Basal':>10} | {'K':>8}")
    print("-" * 70)
    
    for res in resultados:
        print(f"{res['ciclo']:<6} | {res['comida'][:25]:<25} | {res['IM']:10.4f} | {res['asimilacion_basal']:10.4f} | {res['K']:8.4f}")
    
    # Estadísticas por comida
    print("\n" + "=" * 100)
    print("ESTADÍSTICAS POR COMIDA (IM promedio)")
    print("=" * 100)
    ...
    for comida in sorted(comidas_unicas):
        ...
        im_prom = np.mean(ims)
        clasif_prom = "NUTRITIVA" if im_prom > 0.01 else ("TÓXICA" if im_prom < -0.01 else "NEUTRA")
        print(f"  {comida:<25} | IM = {im_prom:+.4f} | {clasif_prom} | n={len(ims)}")
    
    # IM negativos?
    im_negativos = [r for r in resultados if r['IM'] < 0]
    print(f"\nExperiencias con IM negativo: {len(im_negativos)}/{len(resultados)}")
    
    if im_negativos:
        print("\nIM NEGATIVOS (tóxicas):")
        for res in im_negativos[:5]:
            print(f"  {res['experiencia']}. {res['comida']}: IM={res['IM']:.4f}")

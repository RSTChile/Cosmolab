#!/usr/bin/env python3
"""
VSTCosmo - v24: Dinámica multiescala
Σ captura estructura jerárquica (coherencia entre escalas de tiempo).
No usa FFT ni features externas.
"""
...
# Parámetros de Σ (estructura multiescala)
TASA_SIGMA = 0.15
DISIPACION_SIGMA = 0.05
FUERZA_ESTRUCTURA = 0.3

# Nuevo: escalas temporales (pasos)
ESCALAS = [1, 2, 4, 8, 16]  # número de pasos para suavizado
...
def promedio_movil_suave(Phi, ventana):
    """Promedio móvil por fila (banda) con ventana gaussiana suave."""
    kernel = np.exp(-(np.arange(-ventana, ventana+1)**2) / (2 * (ventana/2)**2))
    kernel = kernel / np.sum(kernel)
    resultado = np.zeros_like(Phi)
    for f in range(DIM_FREQ):
        resultado[f] = np.convolve(Phi[f], kernel, mode='same')
    return resultado

def estructura_multiescala(Phi, escalas):
    """Calcula estructura jerárquica a partir de diferencias entre escalas."""
    # Suavizar en cada escala
    suaves = []
    for esc in escalas:
        suaves.append(promedio_movil_suave(Phi, esc))
    
    # Calcular coherencia multiescala de los signos de los detalles
    n_escalas = len(escalas)
    coherencia_total = np.zeros_like(Phi)
    
    for sc in range(n_escalas - 1):
        # Detalle entre escala sc y sc+1
        detalle = suaves[sc] - suaves[sc+1]
        signo = np.tanh(detalle * 5.0)
        coherencia_total += signo
    
    # Promediar y normalizar
    coherencia_total = coherencia_total / (n_escalas - 1)
    
    # Estructura es alta donde todas las escalas están alineadas
    estructura = np.maximum(0, coherencia_total)
...
def actualizar_memoria_estructura(Sigma, Phi, A):
    """Σ acumula coherencia multiescala."""
    estructura = estructura_multiescala(Phi, ESCALAS)
...
def simular(audio, sr, nombre, num_pasos=N_PASOS):
    ...
    for paso in range(num_pasos):
        ...
        Sigma = actualizar_memoria_estructura(Sigma, Phi, A)
...
def main():
    print("=" * 100)
    print("VSTCosmo - v24: Dinámica multiescala")
    print("Σ captura estructura jerárquica (coherencia entre escalas de tiempo)")
    print("=" * 100)
    ...
    print(f"\nComparación de Σ (estructura multiescala):")
    ...
    if voz_res[6] > ruido_res[6] and voz_res[6] > tono_res[6]:
        print("\n  ★ La voz tiene la mayor estructura multiescala (Σ)")
        print("    El sistema distingue la voz por su organización jerárquica.")
        print("    ★★★ ESTE ES EL RÉGIMEN BUSCADO ★★★")
    else:
        print("\n  ✗ La voz no tiene la mayor estructura multiescala")
        print("    Ajustar escalas o parámetros de suavizado.")

#!/usr/bin/env python3
"""
VSTCosmo - Barrido fino de parámetros (incrementos 0.02)
"""
...
def actualizar_campo(Phi, A, muestra, params):
    ...
    arrastre_entrada = params['target'] * (target - Phi)
...
def barrido_target():
    print("\n" + "=" * 90)
    print("BARRIDO 1: GANANCIA_TARGET")
    ...
    valores = np.arange(0.12, 0.52, 0.02).round(2)
    
    print(f"\n{'target':>8}", end="")
    for e in entradas:
        print(f" | {e['tipo']:>20}", end="")
    print()
    print("-" * (8 + 23 * len(entradas)))
    
    for target in valores:
        params = {'target': target, 'generacion': 0.15, 'sostenimiento': 0.25}
        print(f"{target:8.2f}", end="")
        for e in entradas:
...
def barrido_generacion():
    print("\n" + "=" * 90)
    print("BARRIDO 2: GANANCIA_GENERACION")
    print("rango Φ para diferentes entradas")
    print("=" * 90)
    ...
    for gen in valores:
        params = {'target': 0.12, 'generacion': gen, 'sostenimiento': 0.25}
...
def barrido_sostenimiento():
    print("\n" + "=" * 90)
    print("BARRIDO 3: GANANCIA_SOSTENIMIENTO")
    print("rango Φ para diferentes entradas")
    print("=" * 90)
    ...
    for sost in valores:
        params = {'target': 0.12, 'generacion': 0.15, 'sostenimiento': sost}
...
if __name__ == "__main__":
    print("=" * 90)
    print("VSTCosmo - Barrido fino de parámetros (incrementos 0.02)")
    print("Observando en qué punto las entradas comienzan a diferenciarse")
    print("=" * 90)

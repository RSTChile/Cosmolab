#!/usr/bin/env python3
"""
VSTCosmos - v58: Excreción Metabólica
El sistema NO acumula sin límite. Excreta proporcionalmente a lo que ingiere.
Nutritivo → excreción proporcional al IM.
Tóxico → expulsión inmediata.
El campo vuelve a ser permeable al entorno en cada ciclo.
"""
...
# NUEVOS PARÁMETROS: EXCRECIÓN METABÓLICA
TASA_EXCRECION = 0.15        # Excreción por unidad de IM nutritivo
TASA_EXCRECION_TOXICA = 0.25 # Excreción fija por experiencia tóxica
...
def excretar(Psi, L, M, IM):
    """
    Excreción metabólica proporcional a la ingesta.
    Nutritivo → excreción proporcional al IM.
    Tóxico → expulsión inmediata, sin absorción.
    Neutro → movimiento mínimo.
    """
    if IM > UMBRAL_NUTRITIVO:
        # Digestión normal: excreción proporcional
        tasa = TASA_EXCRECION * min(IM, 0.1)  # Limitar para evitar excreción excesiva
    elif IM < UMBRAL_TOXICO:
        # Expulsión: alta y fija
...
def simular_experiencia_con_contraste(S, entrada_nombre, G, R, C, habituacion_comida, K):
    """
    v58: Simula en paralelo el camino real (con estímulo) y el camino contrafactual
    (sin estímulo, misma dinámica). Δ_estímulo = diferencia entre ambos.
    """
    Phi, A, Psi, Omega, M, L = S
    ...
    # Camino CONTRAPACTUAL (sin estímulo, SIN memoria M/L para aislar causalidad)
    Phi_sin = Phi.copy()
    A_sin = A.copy()
    Psi_sin = Psi.copy()
    ...
    for paso in range(N_PASOS):
        ...
        Phi_real = actualizar_campo(Phi_real, A_real, muestra, Psi_real, ganancia_gen, bloqueo_max, K)
        ...
        # ================================================
        # CAMINO CONTRAPACTUAL (sin estímulo, M/L fijos)
        # ================================================
        A_sin = actualizar_atencion(A_sin, Phi_sin, Psi_sin, Omega_sin, M_sin, L_sin, k_comp)
        # M_sin y L_sin NO se actualizan (efecto aislado)
        Phi_sin = actualizar_campo(Phi_sin, A_sin, 0.0, Psi_sin, ganancia_gen, bloqueo_max, K)
        ...
        # Registrar diferencias (EFECTO DEL ESTÍMULO)
    ...
    # Retornar el estado REAL (con estímulo) para continuar
...
def simular_vida_con_excrecion(opciones, n_experiencias=100):
    print("=" * 100)
    print("VSTCosmos - v58: Excreción Metabólica")
    print("El sistema NO acumula sin límite. Excreta proporcionalmente a lo que ingiere.")
    print("Nutritivo → excreción proporcional al IM.")
    print("Tóxico → expulsión inmediata.")
    print("El campo vuelve a ser permeable al entorno en cada ciclo.")
    print("=" * 100)
    ...
    for exp in range(n_experiencias):
        ...
        # Estado antes de la experiencia (basal)
        ...
        # EXPERIENCIA CON CONTRASTE
        S_despues_procesar, G, R, C, metricas = simular_experiencia_con_contraste(
        ...
        # Estado después del procesamiento (antes del reposo)
        estado_after_procesar = extraer_estado(S_despues_procesar)
        
        # Calcular IM usando estado_after_procesar
        ...
        # Calcular costo y ganancia (con Δ_estímulo)
        ...
        # NUEVO: EXCRECIÓN METABÓLICA (antes de actualizar valor estructural)
        ...
        # Respirar entre experiencias
        M, Psi, L, A = respirar_entre_experiencias(M, Psi, L, A)
        
        # El sistema continúa desde el estado post-reposo
        ...
        print(f"    Ganancia estructural = {ganancia_struct:.4f} (Δ_estímulo={delta_estimulo:.3f})")
...
def main():
    ...
    print("\n⚠️  V58: EXCRECIÓN METABÓLICA")
    print("   - El sistema excreta proporcionalmente a lo que ingiere")
    print("   - Nutritivo → excreción proporcional al IM")
    print("   - Tóxico → expulsión inmediata (diarrea/vómito)")
    print("   - Neutro → movimiento mínimo")
    print("   - El campo vuelve a ser permeable al entorno")
    
    historial, K_final, fatiga_final, valor_final = simular_vida_con_excrecion(opciones, n_experiencias=100)
    
    print("\n" + "=" * 100)
    print("RESULTADOS FINALES: V58")
    print("=" * 100)
    ...
    max_valor = max(valor_final.values())
    for comida, valor in valor_final.items():
        delta_prom = np.mean([h['delta_estimulo'] for h in historial if h['comida'] == comida]) if historial else 0
        if valor >= max_valor * 0.9:
            print(f"  🌟 {comida}: IDENTIDAD DOMINANTE (valor={valor:.3f}, Δ_estímulo={delta_prom:.4f})")
        elif valor >= max_valor * 0.7:
            print(f"  ✅ {comida}: identidad sólida (valor={valor:.3f}, Δ_estímulo={delta_prom:.4f})")
        elif valor >= max_valor * 0.5:
            print(f"  ➖ {comida}: identidad moderada (valor={valor:.3f}, Δ_estímulo={delta_prom:.4f})")
        else:
            print(f"  ❌ {comida}: identidad baja (valor={valor:.3f}, Δ_estímulo={delta_prom:.4f})")
    
    # Verificar si Δ_estímulo se mantuvo
    print("\n📊 EVOLUCIÓN DE Δ_ESTÍMULO (primeras vs últimas 20 experiencias):")
    primeros_delta = [h['delta_estimulo'] for h in historial[:20]]
    ultimos_delta = [h['delta_estimulo'] for h in historial[-20:]]
    print(f"  Primeras 20: media={np.mean(primeros_delta):.4f}, max={np.max(primeros_delta):.4f}")
    print(f"  Últimas 20: media={np.mean(ultimos_delta):.4f}, max={np.max(ultimos_delta):.4f}")
    
    if np.mean(ultimos_delta) > 0.01:
        print("  ✅ Δ_estímulo se mantiene → el sistema no se saturó")
    else:
        print("  ⚠️ Δ_estímulo colapsó → el sistema sigue saturándose")

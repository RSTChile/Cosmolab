#!/usr/bin/env python3
"""
VSTCosmos v97 — BigBang como eje único del experimento

Diseño:
  Estímulo único: BigBang (232.9s) en +60° y -60°
  Tres instancias:
    A — Entrenada +60°, evaluada +60°  [control de identidad]
    B — Entrenada +60°, evaluada -60°  [inversión post-entrenamiento]
    C — Entrenada -60°, evaluada -60°  [control simétrico]

  Entrenamiento: 60s
  Evaluación: BigBang completo (232s)
  Un solo ciclo por instancia
  Seed fija: 42

Tres hipótesis (post-hoc, últimos 30s sostenidos):
  H1 mantención:      |Ω_B - Ω_A| < 0.1
  H2 desorientación:  |Ω_B|       < 0.1
  H3 reorientación:   |Ω_B - Ω_C| < 0.1

Métrica derivada:
  tiempo_de_rendición = primer t sostenido (≥5s) donde
                        Ω_B < punto_medio(Ω_A, Ω_C)

Correcciones sobre v96:
  1. Seed fija (reproducibilidad)
  2. Sin reinicio silencioso del puntero (error explícito)
  3. VARIACION_FLOOR = 1e-4 (eficiencia indefinida si var_int < FLOOR)
  4. Sin reinyección automática de ruido al campo
  5. Ω_orient con warmup de 10s (no contaminar promedios)
  6. Logging fino en tiempo real, no al final
  7. Estímulo crudo guardado en CSV separado para auditoría
  8. Logging fino cada 0.5s durante toda la evaluación
"""
...
DURACION_EVALUACION    = 232.0  # BigBang completo (232.9s, dejamos margen)
N_CICLOS               = 1
...
VENTANA_FINAL_SEG      = 30.0  # ventana para clasificación H1/H2/H3

# ============================================================
# PARÁMETROS DE LA FÍSICA DEL CAMPO (idénticos a v96)
# ============================================================
DIM_INTERNA = 32
...
# ============================================================
# ARQUITECTURA DEL CAMPO EXPANDIDO (idéntica a v96)
# ============================================================
...
def imprimir_configuracion():
    print("=" * 100)
    print("VSTCosmos v97 — BigBang como eje único")
    print("=" * 100)
    print()
    print("CONFIGURACIÓN:")
...
# ============================================================
# PREPARAR OBJETIVO (con verificación de límites)
# ============================================================
def preparar_objetivo_canal(canal, sr, idx_paso, ventana_muestras,
    ...
    # Corrección 2: error explícito si se pide más audio del disponible
    if fin > len(canal):
        raise IndexError(
...
# ============================================================
# EFICIENCIA (corrección 3: floor estricto)
# ============================================================
def calcular_eficiencia(Phi_total, ged_actual):
    """
    v97: si var_int < VARIACION_FLOOR, retorna NaN (eficiencia indefinida).
    No se infla la métrica con un floor artificial.
    """
...
def actualizar_campo(Phi_total, Phi_vel_total, W_prof, W_rec,
    ...
    """
    Corrección 4: SIN reinyección automática de ruido cuando var_int es bajo.
    Si el campo se aplana, queremos verlo, no taparlo.
    """
    ...
    # SIN reinyección de ruido — se observa lo que el sistema haga
...
def entrenar(archivos, clave_audio, etiqueta, duracion=DURACION_ENTRENAMIENTO,
    ...
    # Verificar que hay audio suficiente
...
# ============================================================
# EVALUACIÓN LARGA (CON LOGGING FINO EN TIEMPO REAL)
# ============================================================
def evaluar(Phi_total, Phi_vel_total, W_prof, W_rec, Phi_int_historia,
    ...
    Evaluación continua con:
      - Logging fino cada 0.5s a CSV (en tiempo real)
      - Logging del estímulo crudo a CSV separado (auditoría)
      - Imprime progreso cada 10s
      - Sin reinicio de puntero (error si audio insuficiente)
    """
    ...
    # Verificar audio suficiente (sin reinicio)
    ...
    # Abrir CSVs para logging en tiempo real
    ...
    # Buffers en memoria para análisis post-hoc
    omega_traj = []
    t_traj     = []
    ...
    for paso in range(n_pasos):
        ...
        # Trayectoria en memoria
        omega_traj.append(omega)
        t_traj.append(t_actual)

        # Log fino cada 0.5s — en tiempo real
        if paso % LOG_FINO_PASOS == 0:
            ...
            # Estímulo crudo: energía media por mitad de banda en cada canal
            n_mid = DIM_AUD // 2
...
def clasificar_hipotesis(t_A, omega_A, t_B, omega_B, t_C, omega_C):
    """
    Clasifica el comportamiento de B en los últimos VENTANA_FINAL_SEG segundos.
    Retorna dict con cada hipótesis evaluada.
    """
    # Tomar la ventana final común
    ...
    # Igualar longitudes (toman la mínima)
...
def calcular_tiempo_rendicion(t_A, omega_A, t_B, omega_B, t_C, omega_C):
    """
    Primer t (post-warmup) donde Ω_B cae bajo el punto medio entre Ω_A y Ω_C
    de manera sostenida (≥ VENTANA_SOSTENIDO_SEG segundos).
    Retorna t o None si nunca ocurre.
    """
    # Interpolar A y C sobre los t de B
    omega_A_interp = np.interp(t_B, t_A, omega_A)
    omega_C_interp = np.interp(t_B, t_C, omega_C)

    punto_medio = (omega_A_interp + omega_C_interp) / 2.0

    # B "se rinde" cuando Ω_B se aleja de A acercándose a C
    # Operacionalizamos: Ω_B < punto_medio (si Ω_A > Ω_C) o Ω_B > punto_medio (si Ω_A < Ω_C)
    # Más simple: |Ω_B - Ω_C| < |Ω_B - Ω_A|, es decir, B está más cerca de C que de A
...
def main():
    ...
    os.makedirs('v97_logs', exist_ok=True)
    timestamp = datetime.now().strftime('%Y%m%d_%H%M%S')

    # ================================================================
    # INSTANCIA A — Entrenada +60°, evaluada +60°
    # ================================================================
    print()
    print("█" * 100)
    print("INSTANCIA A — Entrenada BigBang +60°, evaluada BigBang +60°  [identidad]")
    print("█" * 100)

    np.random.seed(SEED)  # Reset seed antes de cada instancia
    ...
    res_A = evaluar(
        ...
        csv_log_path=f'v97_logs/v97_A_log_{timestamp}.csv',
        csv_estimulo_path=f'v97_logs/v97_A_estimulo_{timestamp}.csv',
    )

    # ================================================================
    # INSTANCIA B — Entrenada +60°, evaluada -60°
    # ================================================================
    print()
    print("█" * 100)
    print("INSTANCIA B — Entrenada BigBang +60°, evaluada BigBang -60°  [inversión]")
    print("█" * 100)
    ...
    res_B = evaluar(
        ...
        csv_log_path=f'v97_logs/v97_B_log_{timestamp}.csv',
        csv_estimulo_path=f'v97_logs/v97_B_estimulo_{timestamp}.csv',
    )

    # ================================================================
    # INSTANCIA C — Entrenada -60°, evaluada -60°
    # ================================================================
    print()
    print("█" * 100)
    print("INSTANCIA C — Entrenada BigBang -60°, evaluada BigBang -60°  [simetría]")
    print("█" * 100)
    ...
    res_C = evaluar(
        ...
        csv_log_path=f'v97_logs/v97_C_log_{timestamp}.csv',
        csv_estimulo_path=f'v97_logs/v97_C_estimulo_{timestamp}.csv',
    )
    ...
    if cumplidas == 0:
        ...
        print("    Trayectoria no clasificable — inspección visual requerida.")
    elif cumplidas == 1:
        print("\n  → CLASIFICACIÓN ÚNICA.")
    else:
    ...
    if t_rend is not None:
        ...
        print(f"    (primer momento sostenido ≥{VENTANA_SOSTENIDO_SEG}s donde B está más cerca de C que de A)")
    else:
        print(f"  Tiempo de rendición:  NO ocurre en {DURACION_EVALUACION}s")
        print(f"    (B nunca se acerca a C sostenidamente más que a A)")

    # ================================================================
    # RESUMEN A CSV Y TXT
    # ================================================================
    resumen_path = f'v97_logs/v97_resumen_{timestamp}.txt'
    with open(resumen_path, 'w', encoding='utf-8') as f:
        f.write(f"VSTCosmos v97 — Resumen\n")
    ...
    ax.set_title('VSTCosmos v97 — Trayectorias de Ω_orient (BigBang completo)')
    ax.legend(loc='best', fontsize=9)
    ax.grid(True, alpha=0.3)

    ax = axes[1]
    # Distancia de B a A y a C
    ...
    plt.tight_layout()
    grafico_path = f'v97_logs/v97_omega_{timestamp}.png'

#!/usr/bin/env python3
"""
VSTCosmo - Experimento v7 con GANANCIA_INEST = 0.2
Mismo código que v7_controles_reales, solo cambiando la ganancia de inestabilidad.
"""
...
# Parámetros de Φ
DIFUSION_PHI = 0.1
GANANCIA_INEST = 0.2  # Reducida de 0.5 a 0.2
...
def simular(audio, sr, nombre, num_pasos=N_PASOS):
    ...
    for paso in range(num_pasos):
        ...
        # Registrar progreso
        if paso > 0 and paso % (num_pasos // 10) == 0:
            rango_a = np.max(A) - np.min(A)
            registro_rango_A.append(rango_a)
            pct = 100 * paso // num_pasos
            print(f"      Progreso: {pct}% | rango A={rango_a:.4f}")
    
    # Valores finales
    rango_A_final = np.max(A) - np.min(A)
    var_A_final = np.var(A)
    media_A_final = np.mean(A)
    rango_Phi_final = np.max(Phi) - np.min(Phi)
    var_Phi_final = np.var(Phi)
    
    return {
        'nombre': nombre,
        'rango_A': rango_A_final,
        'var_A': var_A_final,
        'media_A': media_A_final,
        'rango_Phi': rango_Phi_final,
        'var_Phi': var_Phi_final
    }
...
def main():
    print("=" * 60)
    print("VSTCosmo - Experimento v7 (GANANCIA_INEST = 0.2)")
    print("=" * 60)
    ...
    # Experimento 4: Silencio (audio silencioso de la misma duración)
    print(f"\n  Simulando: Silencio")
    ...
    for r in resultados:
        if r['nombre'] == 'Silencio':
            ...
            if r['rango_A'] < ref['rango_A'] * 0.7:
                print("    → El silencio produce MENOR diferenciación que el audio real")
                print("    (el sistema depende del entorno)")
            elif r['rango_A'] > ref['rango_A'] * 1.3:
                print("    → El silencio produce MAYOR diferenciación")
                print("    (la autoorganización interna domina)")
            else:
                print("    → Diferencias pequeñas. El sistema responde similar a todo.")
    ...
    # Verificar saturación de Φ
    print("\n" + "=" * 60)
    print("SATURACIÓN DE Φ")
    print("=" * 60)
    ...
    for r in resultados:
        if r['rango_Phi'] >= 0.99:
            saturados += 1
            print(f"  {r['nombre']}: rango_Phi = {r['rango_Phi']:.4f} → SATURADO")
        else:
            print(f"  {r['nombre']}: rango_Phi = {r['rango_Phi']:.4f} → NO SATURADO")
    
    if saturados == len(resultados):
        print("\n  ⚠ La inestabilidad sigue siendo dominante (todos saturan)")
    elif saturados == 0:
        print("\n  ✓ La inestabilidad ya no domina. La entrada puede influir.")
    else:
        print(f"\n  → {saturados}/{len(resultados)} casos saturados")

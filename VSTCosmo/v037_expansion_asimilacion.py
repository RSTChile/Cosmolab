#!/usr/bin/env python3
"""
VSTCosmo - v37: Expansión de la Zona de Asimilación
El sistema desarrolla capacidad metabólica K que se expande con experiencias nutritivas
y se contrae con experiencias tóxicas.

No se le dice qué es mejor. El sistema se vuelve capaz de procesar más sin romperse.
"""
...
# ============================================================
# NUEVOS PARÁMETROS PARA K (CAPACIDAD METABÓLICA)
# ============================================================
K_INICIAL = 1.0
K_MIN = 0.5
K_MAX = 2.0
ALPHA_UP = 0.05      # crecimiento más rápido
ALPHA_DOWN = 0.02    # decrecimiento más lento
...
# ============================================================
# FUNCIÓN DE ASIMILACIÓN CON K (modificada)
# ============================================================
def calcular_asimilacion(estado, delta_rango_A, delta_rango_Phi, delta_L, variacion_A, K):
    """
    asimilacion = intensidad * K * exp(-costo_estructural)
    
    donde:
    intensidad = |Δrango_A| + |Δrango_Φ|
    K = capacidad metabólica del sistema (modula cuánta complejidad puede absorber)
    costo = β_L * ΔL + β_var * variacion_A
    """
    intensidad_base = abs(delta_rango_A) + abs(delta_rango_Phi)
    intensidad = intensidad_base * K  # ← ÚNICA MODIFICACIÓN
...
def actualizar_K(K, IM):
    ...
    K disminuye con experiencias tóxicas (IM < 0)
    """
    if IM > 0:
        K = K + ALPHA_UP * IM
    elif IM < 0:
        K = K + ALPHA_DOWN * IM  # IM negativo, esto reduce K
...
# ============================================================
# SIMULACIÓN PRINCIPAL CON K
# ============================================================
def simular_secuencia_con_K(orden, n_ciclos=3):
    """
    Ejecuta múltiples ciclos completos de la secuencia.
    K persiste entre ciclos, permitiendo expansión gradual.
    """
    print("=" * 100)
    print("VSTCosmo - v37: Expansión de la Zona de Asimilación")
    print("K = capacidad metabólica del sistema")
    print("Cada experiencia nutritiva expande K. Cada tóxica la contrae.")
    print("=" * 100)
    ...
    for ciclo in range(n_ciclos):
        ...
        for exp_idx, entrada_nombre in enumerate(orden):
            print(f"\n  Experiencia {exp_idx + 1}: {entrada_nombre}")
            ...
            # EXPERIENCIA
            S_after, G, R, C, metricas = simular_experiencia(S_before, entrada_nombre, G, R, C)
            ...
            # CALCULAR ASIMILACIÓN CON K
            asimilacion_exp, intensidad_base, intensidad_K, costo = calcular_asimilacion(
                estado_post,
            ...
            # Calcular asimilación previa (estado antes de la experiencia)
            # Para simplificar, usamos la asimilación de la experiencia anterior dentro del ciclo
            if len(registro) == 0:
                asimilacion_pre = 0.0
            else:
            ...
            # ACTUALIZAR K según IM
            K_antes = K
            K = actualizar_K(K, IM)
            
            # ACTUALIZAR MEMORIA M (manteniendo estructura)
...
# ============================================================
def main():
    # Secuencia base (la misma que en v36)
    secuencia = [
        "Silencio",
        "Ruido blanco",
        "Tono puro",
    ...
    print("\n" + "=" * 100)
    print("SECUENCIA DE EXPERIENCIAS (repetida por ciclo)")
    print("=" * 100)
    for i, exp in enumerate(secuencia, 1):
        print(f"  {i}. {exp}")
    
    # Ejecutar múltiples ciclos para ver expansión
    resultados = simular_secuencia_con_K(secuencia, n_ciclos=4)
    
    # ============================================================
    # ANÁLISIS POR CICLO
    # ============================================================
    print("\n" + "=" * 100)
    print("EVOLUCIÓN DE K A TRAVÉS DE LOS CICLOS")
    print("=" * 100)
    
    # Evolución de K
    print(f"\n{'Ciclo':<6} | {'Entrada':<25} | {'IM':>10} | {'K':>8}")
    print("-" * 55)
    
    for res in resultados:
        print(f"{res['ciclo']:<6} | {res['entrada'][:25]:<25} | {res['IM']:10.4f} | {res['K']:8.4f}")
    
    # K final por ciclo
    print("\n" + "=" * 100)
    print("K AL FINAL DE CADA CICLO")
    print("=" * 100)
    
    for ciclo in range(1, 5):
        ks_ciclo = [r['K'] for r in resultados if r['ciclo'] == ciclo]
        if ks_ciclo:
            print(f"  Ciclo {ciclo}: K final = {ks_ciclo[-1]:.4f}")
    
    # Comparación de asimilación de voz entre ciclos
    print("\n" + "=" * 100)
    print("EVOLUCIÓN DE LA ASIMILACIÓN DE VOZ")
    print("=" * 100)
    
    voz_resultados = [r for r in resultados if "Voz_Estudio" in r['entrada']]
    print(f"\n{'Ciclo':<6} | {'Exposición':<12} | {'IM':>10} | {'K en ese momento':>16}")
    print("-" * 50)
    
    for res in voz_resultados:
        print(f"{res['ciclo']:<6} | Voz #{res['experiencia']:<6} | {res['IM']:10.4f} | {res['K_antes']:16.4f}")
    ...
    ks_finales = [r['K'] for r in resultados if r['ciclo'] == 4]
    if ks_finales:
        k_final = ks_finales[-1]
        print(f"\nK final del sistema: {k_final:.4f}")
        
        if k_final > K_INICIAL:
            print("  ✓ La capacidad metabólica SE EXPANDIÓ")
            print("  El sistema puede procesar más sin romperse")
        else:
            print("  ✗ La capacidad metabólica NO se expandió")
        
        # Verificar si la música mejoró
        musica_exp1 = [r for r in resultados if r['ciclo'] == 1 and "Brandemburgo" in r['entrada']][0]['IM']
        musica_exp_final = [r for r in resultados if r['ciclo'] == 4 and "Brandemburgo" in r['entrada']][0]['IM']
        
        print(f"\n  Brandemburgo (música):")
        print(f"    Ciclo 1: IM = {musica_exp1:+.4f}")
        print(f"    Ciclo 4: IM = {musica_exp_final:+.4f}")
        
        if musica_exp_final > musica_exp1:
            print("  ✓ La música mejoró su asimilación")
        else:
            print("  ✗ La música no mejoró")

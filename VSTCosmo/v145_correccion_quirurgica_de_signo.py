#!/usr/bin/env python3
"""
VSTCosmos V145 — Corrección quirúrgica de signo

Cambio respecto a V143 (que alternaba pero con signo invertido):
  - SOLO eliminar el signo menos en ganancia_grad
  - TODO lo demás IDÉNTICO a V143

Parámetros (idénticos a V143):
  - Kp_base: 0.002
  - Kp_min: 0.0005
  - Inercia: 0.95
  - Zona muerta: 2.0°
  - Período: 40.0s (20s en cada polo)
  - Fatiga: DESACTIVADA

Objetivo:
  - Demostrar que el signo invertido era el único problema
...
# Limites de plasticidad (como V143)
KP_BASE = 0.002
KP_MIN = 0.0005
KP_MAX = 0.005
...
# ============================================================
# APARATO MOTOR V145 (SOLO CAMBIO DE SIGNO)
# ============================================================

class AparatoMotorV145:
    def __init__(self):
    ...
    def actuar(self, gradiente, LF_activa, fuente_activa, t, setpoint_percepcion):
        ...
        # CAMBIO ÚNICO RESPECTO A V143: Eliminar el signo menos
        # V143: ganancia_grad = -np.tanh(gradiente * self.sensibilidad_grad)
        # V145: ganancia_grad = np.tanh(gradiente * self.sensibilidad_grad)
...
class SistemaV145:
    def __init__(self, nombre, seed=SEMILLA_BASE):
        ...
        self.izquierdo = HemisferioV145("L", 30.0, generar_ruido_rosa, seed=seed, sesgo=SESGO_L)
        self.derecho = HemisferioV145("R", 300.0, generar_clicks_poisson, seed=seed+100, sesgo=SESGO_R)
        
        self.sistema_B_izq = HemisferioV145("B_L", 30.0, generar_ruido_rosa, seed=seed+200, sesgo=SESGO_L)
        self.sistema_B_der = HemisferioV145("B_R", 300.0, generar_clicks_poisson, seed=seed+300, sesgo=SESGO_R)
        
        self.modo_entrenamiento = True
        self.motor = AparatoMotorV145()
...
def ejecutar_v145():
    print("=" * 100)
    print("EXPERIMENTO V145 — Corrección quirúrgica de signo")
    print("=" * 100)
    print("  Cambio respecto a V143 (que alternaba con signo invertido):")
    print("    - SOLO eliminar el signo menos en ganancia_grad")
    print("    - TODO lo demás IDÉNTICO a V143")
    print("")
    print("  Parámetros (idénticos a V143):")
    ...
    sistema = SistemaV145("V145", seed=SEMILLA_BASE)
    ...
    print("\n" + "=" * 80)
    print("RESULTADOS V145 — Corrección de signo")
    print("=" * 80)
    ...
    if baseline_ok:
        ...
        print("\n  El problema era el signo invertido en ganancia_grad")
        print("\n  ANIMA-1 listo para V146 (Fatiga progresiva)")
    else:
        print("\n  ⚠️ BASELINE PARCIAL")
        if exito_60:
    ...
    if 'tiempos' in locals() and 'orientaciones' in locals():
        ...
        ax.set_title('Alternancia ±60° (corrección de signo)')
        ax.legend()
    ...
    os.makedirs('v145_logs', exist_ok=True)
    plt.savefig(f'v145_logs/v145_signo_{timestamp}.png', dpi=150)
    print(f"\n  Graficos guardados: v145_logs/v145_signo_{timestamp}.png")

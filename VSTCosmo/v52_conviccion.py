#!/usr/bin/env python3
"""
VSTCosmos - v52: Refuerzo Estructural No Lineal
El sistema NO promedia. ACUMULA con aceleración.
Cuando una entrada es consistentemente nutritiva:
-> su valor estructural crece de forma NO LINEAL
-> aparece especialización y convicción
"""
...
# NUEVO: REFUERZO ESTRUCTURAL NO LINEAL (convicción)
VALOR_ESTRUCTURAL_DECAY = 0.99      # Decaimiento lento
VALOR_ESTRUCTURAL_BASE = 0.3        # Valor inicial
REFUERZO_EXPONENTE = 1.5            # Aceleración (no lineal)
REFUERZO_MAX = 1.0                  # Máximo valor (convicción plena)
...
def calcular_ganancia_estructural(estado_before, estado_after, fatiga):
    ...
    ganancia_neta = ganancia_bruta * max(0.05, factor_fatiga)
...
def actualizar_valor_estructural_no_lineal(valor_estructural, comida, ganancia_estructural):
    """
    REFUERZO NO LINEAL:
    - Decaimiento general (lento)
    - Si hay ganancia, el refuerzo es ACELERADO (exponencial)
    ...
    if ganancia_estructural > 0:
        # REFUERZO NO LINEAL: la ganancia se potencia
        # El exponente > 1 hace que las diferencias se amplifiquen
        refuerzo = ganancia_estructural ** REFUERZO_EXPONENTE
        refuerzo = min(refuerzo, 0.3)  # Límite para evitar explosión
        valor_estructural[comida] += refuerzo
...
def calcular_utilidad_con_conviccion(comida, historial_im, habituacion, exploracion_prob, 
    ...
    # NUEVO: la convicción (valor estructural) domina la decisión
    # El sistema apuesta por lo que cree mejor
...
def elegir_proxima_comida_con_conviccion(opciones, historial_im, habituacion, exploracion_prob, 
                                          fatiga, valor_estructural):
    utilidades = {}
    for comida in opciones:
        utilidad = calcular_utilidad_con_conviccion(comida, historial_im, habituacion, 
...
# ============================================================
# SIMULACIÓN PRINCIPAL CON REFUERZO NO LINEAL
# ============================================================
def simular_vida_con_conviccion(opciones, n_experiencias=120):
    print("=" * 100)
    print("VSTCosmos - v52: Refuerzo Estructural No Lineal (Convicción)")
    print("El sistema NO promedia. ACUMULA con aceleración.")
    print("Entradas consistentemente nutritivas → valor estructural crece exponencialmente")
    print("Aparece especialización y convicción")
    print("=" * 100)
    ...
    for exp in range(n_experiencias):
        ...
        comida_elegida, utilidades = elegir_proxima_comida_con_conviccion(
        ...
        print(f"  💎 Convicción (valor estructural): {valor_actual:.3f}")
        print(f"  😫 Fatiga: {fatiga[comida_elegida]:.3f}")
        ...
        # Calcular costo y ganancia estructural
        costo_struct = calcular_costo_estructural(estado_before, estado_after)
        ganancia_struct = calcular_ganancia_estructural(estado_before, estado_after, fatiga[comida_elegida])
        ...
        # NUEVO: actualizar valor estructural (refuerzo no lineal)
        valor_estructural = actualizar_valor_estructural_no_lineal(valor_estructural, comida_elegida, ganancia_struct)
        ...
        print(f"    Ganancia estructural = {ganancia_struct:.4f}")
        print(f"    K: {K_antes:.4f} → {K:.4f}")
        print(f"    Nueva convicción: {valor_estructural[comida_elegida]:.3f}")
...
def main():
    ...
    print("\n⚠️  REFUERZO ESTRUCTURAL NO LINEAL (CONVICCIÓN)")
    print("   - El sistema NO promedia. ACUMULA con aceleración.")
    print("   - Ganancia estructural → refuerzo = ganancia^1.5")
    print("   - Las diferencias se AMPLIFICAN")
    print("   - Aparece especialización real")
    
    historial, K_final, fatiga_final, valor_final = simular_vida_con_conviccion(opciones, n_experiencias=100)
    
    print("\n" + "=" * 100)
    print("RESULTADOS FINALES: CONVICCIÓN Y ESPECIALIZACIÓN")
    print("=" * 100)
    ...
    print(f"\n💎 CONVICCIÓN (valor estructural final):")
    ...
    print("\n" + "=" * 100)
    print("ANÁLISIS DE ESPECIALIZACIÓN")
    print("=" * 100)
    
    max_valor = max(valor_final.values())
    for comida, valor in valor_final.items():
        if valor >= 0.9 * max_valor:
            print(f"  🌟 {comida}: ESPECIALIZACIÓN DOMINANTE (convicción = {valor:.3f})")
        elif valor >= 0.7 * max_valor:
            print(f"  ✅ {comida}: preferencia fuerte (convicción = {valor:.3f})")
        elif valor >= 0.5 * max_valor:
            print(f"  ➖ {comida}: preferencia moderada (convicción = {valor:.3f})")
        else:
            print(f"  ❌ {comida}: evitado (convicción = {valor:.3f})")
    
    print("\n" + "=" * 100)
    print("EXPERIMENTO COMPLETADO")
    print("El sistema ahora tiene CONVICCIÓN y ESPECIALIZACIÓN real.")
    print("=" * 100)

"""
grafo_random_piloto_correr.py — corre Phantom sobre las 3 condiciones del piloto de control
grafo-random (N=500, semillas 901-903). Mismo patrón EXACTO que `null3_piloto_correr.py`/
`null2_zeldovich_piloto_correr.py`/`null1_bateria_correr.py` (congelados, no se tocan directamente):
por cada carpeta `/Users/alexis/phantom_cs073/piloto_grafo_random/random_s{1,2,3}/` (ya con su
`cosmogenesis_ic.txt` escrito por `grafo_random_piloto_generar.py`):
...
BASE = Path("/Users/alexis/phantom_cs073/piloto_grafo_random")
CARPETAS = [BASE / f"random_s{i}" for i in (1, 2, 3)]
...
def correr_una(carpeta: Path) -> dict:
    ...
    assert ic.exists(), f"falta {ic} -- correr grafo_random_piloto_generar.py primero"

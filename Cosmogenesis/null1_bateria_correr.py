"""
null1_bateria_correr.py — corre Phantom sobre las 8 condiciones NULL-1 de la batería N=2000.

Qué hace: por cada carpeta /Users/alexis/phantom_cs073/bateria_null1_n2000/ic_null1_s{1..8}/ (ya con
su cosmogenesis_ic.txt escrito por null1_bateria_generar.py):
  1. corre `phantomsetup_cosmogenesis_backup cosmog` (el binario "_backup", SIN APR -- el binario
     `phantom`/`phantomsetup` actual fue recompilado el 3-ago con refinamiento adaptativo de
     partículas por defecto, que habría sido un confound frente a la metodología ya validada de
     bateria_n2000/CS073; ver NULL1_piloto_distribucion_radial_CS.md). Esto lee cosmogenesis_ic.txt y
     escribe un cosmog.in con valores por DEFECTO de Phantom (icreate_sinks=0, tmax=10 -- sin
     sumideros, no sirve tal cual).
  2. reescribe el bloque de sumideros + tmax/dtmax de cosmog.in para que coincidan EXACTO con
     /Users/alexis/phantom_cs073/bateria_n2000/ic_real/cosmog.in (icreate_sinks=1, rho_crit_cgs=1000,
     r_crit=0.6, h_acc=0.3, h_soft_sinkgas=0, r_merge_uncond=0, r_merge_cond=0, tmax=0.500,
     dtmax=0.001) -- la MISMA configuración física que usó la batería original de CS073, no
     reinventada. El bloque de reemplazo es texto literal, no un parser de formato Phantom.
  3. corre `phantom_cosmogenesis_backup cosmog.in`, mide el wall time real.

Salvaguarda de tiempo (pedida por Alexis): si el tiempo ACUMULADO de cómputo Phantom (las 8 corridas)
supera TIEMPO_LIMITE_S, el script deja de lanzar corridas nuevas, imprime cuántas corrió y cuántas
faltan, y termina limpio -- no se queda esperando indefinidamente sin reportar.

No toca bateria_n2000/ (sólo se LEE ic_real/cosmog.in como plantilla de parámetros, nunca se escribe
ahí). No usa I_WILL_NOT_PUBLISH_CRAP -- si una corrida aborta por el guardián de conservación de
Phantom, se deja constancia tal cual (exit code + tail del log), no se fuerza.
"""
import re
import subprocess
import sys
...
BASE = Path("/Users/alexis/phantom_cs073/bateria_null1_n2000")
CARPETAS = [BASE / f"ic_null1_s{i}" for i in range(1, 9)]
...
# escribir este script -- no reinventada). Ver RESULTADO_bateria_ignicion_sumideros_N2000_CS.md.
TMAX = "0.500"
DTMAX = "0.001"
...
TIEMPO_LIMITE_S = 35 * 60  # salvaguarda pedida por Alexis: 30-40 min, se usa el punto medio
...
def correr_una(carpeta: Path) -> dict:
    ...
    assert ic.exists(), f"falta {ic} -- correr null1_bateria_generar.py primero"

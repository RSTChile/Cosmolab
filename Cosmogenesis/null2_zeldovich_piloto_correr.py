"""
null2_zeldovich_piloto_correr.py — corre Phantom sobre las 3 condiciones NULL-2-Zel'dovich del
piloto. MISMO patrón que null2_piloto_correr.py (que a su vez copia el método de
null1_bateria_correr.py/real_extra_correr.py, todos congelados, no se tocan directamente): se
reemplaza el bloque de sumideros + tmax/dtmax por el EXACTO de bateria_n2000/ic_real/cosmog.in
(icreate_sinks=1, rho_crit_cgs=1000, r_crit=0.6, h_acc=0.3, tmax=0.500, dtmax=0.001) -- la MISMA
configuración física de toda la jerarquía -- y se corre el binario phantom_cosmogenesis_backup.

No re-corre REAL -- ya está en disco en /Users/alexis/phantom_cs073/piloto_null1/real/ (4 sumideros,
masa 282.0), se usa como referencia sin tocarlo.
"""
import re
import subprocess
import sys
...
BASE = Path("/Users/alexis/phantom_cs073/piloto_null2_zeldovich")
CARPETAS = [BASE / f"null2z_s{i}" for i in (1, 2, 3)]
...
def correr_una(carpeta: Path) -> dict:
    ...
    assert ic.exists(), f"falta {ic} -- correr null2_zeldovich_piloto_generar.py primero"

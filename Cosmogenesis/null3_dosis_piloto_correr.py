"""
null3_dosis_piloto_correr.py — corre Phantom sobre las condiciones NULL-3 del piloto dosis-respuesta
(tol_relativa 0.4 y 0.8, N=500). Mismo patrón EXACTO que `null3_piloto_correr.py` (congelado, no se
toca directamente): por cada carpeta ya con su `cosmogenesis_ic.txt` escrito por
`null3_dosis_piloto_generar.py`:
...
masa 282.0, N=500), se usa como referencia sin tocarlo. El nivel tol=0.2 también ya está en disco
(`piloto_null3/null3_s{1,2,3}`, semillas 601-603, 3-5 sumideros cada uno) -- no se re-corre.
"""
import re
import subprocess
import sys
...
BASE = Path("/Users/alexis/phantom_cs073/piloto_null3_dosis")
CARPETAS = [BASE / "tol0.4_s1", BASE / "tol0.8_s1"]
...
def correr_una(carpeta: Path) -> dict:
    ...
    assert ic.exists(), f"falta {ic} -- correr null3_dosis_piloto_generar.py primero"
...
def main():
    ...
    for carpeta in CARPETAS:
        if tiempo_acumulado > TIEMPO_LIMITE_S:
            print(f"[SALVAGUARDA] {tiempo_acumulado:.0f}s > {TIEMPO_LIMITE_S}s. Deteniendo.", flush=True)
            break

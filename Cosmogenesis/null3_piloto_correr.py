"""
null3_piloto_correr.py — corre Phantom sobre las 3 condiciones NULL-3 del piloto (N=500, semillas
601-603). Mismo patrón EXACTO que `null2_zeldovich_piloto_correr.py`/`null1_bateria_correr.py`
(congelados, no se tocan directamente): por cada carpeta
`/Users/alexis/phantom_cs073/piloto_null3/null3_s{1,2,3}/` (ya con su `cosmogenesis_ic.txt` escrito por
`null3_piloto_generar.py`):
...
BASE = Path("/Users/alexis/phantom_cs073/piloto_null3")
CARPETAS = [BASE / f"null3_s{i}" for i in (1, 2, 3)]
...
def correr_una(carpeta: Path) -> dict:
    ...
    assert ic.exists(), f"falta {ic} -- correr null3_piloto_generar.py primero"

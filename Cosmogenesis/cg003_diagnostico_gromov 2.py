...
def landmarks_dist(adj, N, K, seed=0):
    """BFS desde K nodos-faro -> matriz KxK de distancias (misma componente gigante)."""
    rng = np.random.default_rng(seed)
    # anclar en la componente gigante: BFS desde un nodo, tomar los alcanzables
    seed_node = int(rng.integers(N))
    d0 = _bfs_dist(adj, seed_node, N)
    reach = np.where(d0 >= 0)[0]
    if len(reach) < K + 4:
        return None, 0
...
def diagnos(nombre, adj, N, K, seed=0):
    D, dmax = landmarks_dist(adj, N, K, seed=seed)
    if D is None:
        return dict(nombre=nombre, N=N, diam=np.nan, dmean=np.nan, d95=np.nan)
    dmean, d95 = gromov_delta(D, seed=seed + 1)
    return dict(nombre=nombre, N=N, diam=dmax, dmean=dmean, d95=d95)
...
def main():
    ...
    print(f"{'grafo':>16} {'N':>7} {'diam':>5} {'δ_media':>8} {'δ_q95':>7}")
    ...
    for N in Ns:
        ...
        for nombre, mk in [
            ...
            print(f"{nombre:>16} {r['N']:>7} {r['diam']:>5} {r['dmean']:>8.2f} {r['d95']:>7.2f}", flush=True)
        # nuestro grafo real
        for Dt in (2, 3):
            ...
            print(f"{'  (shuffle)':>16} {rs['N']:>7} {rs['diam']:>5} {rs['dmean']:>8.2f} {rs['d95']:>7.2f}", flush=True)
        print()

"""
cs090_fase7_f703_correr.py — FASE VII F7-03: corre Phantom sobre los cinco brazos de organización de
triángulos que fabricó `cs090_fase7_f703_organizacion.py`.

MISMA FORMA EXACTA que `cs090_fase7_f702_correr.py` (que copia la de `cs090_fase6_o3b_correr.py`, que a
su vez copia la de `cs090_fase5b_correr.py`): se copia el patrón, no se importa ni se modifica ninguno.
El punto de la tarea es que los cinco brazos pasen por el MISMO tubo, sin un solo parámetro distinto:

  1. `phantomsetup_cosmogenesis_backup cosmog` sobre el ASCII ya escrito,
  2. se reescribe el bloque de sumideros + tmax/dtmax de `cosmog.in` al protocolo estándar de toda la
     jerarquía CS073 / Fase V-B / Fase VI / Fase VII: icreate_sinks=1, rho_crit_cgs=1000, r_crit=0.6,
...
(misma salvaguarda que CS073 / Fase V-B / O3-B / F7-02). No declara cierre ni veredicto.
"""
import re
import subprocess
import sys
...
def correr_una(carpeta: Path) -> dict:
    ...
    assert ic.exists(), f"falta {ic} -- correr cs090_fase7_f703_organizacion.py primero"
...
if __name__ == "__main__":
    # uso: python3.9 cs090_fase7_f703_correr.py [patron_glob]   (default: todas las carpetas)
    # CUIDADO (misma lección que F7-02): que el archivo de condición inicial EXISTA no significa que
    # esté COMPLETO -- el generador tarda ~1 min en escribir las 2000 líneas y este runner puede correr
    # en paralelo. Se exige (a) que exista `meta_regla.json`, que el generador escribe DESPUÉS del IC y
    # por lo tanto funciona como marca de "terminado", y (b) que el IC tenga las N+2 líneas esperadas.
    ...
    carpetas = sorted(c for c in BASE.glob(patron)
                      ...
                      and len((c / "cosmogenesis_ic.txt").read_text().splitlines()) == 2002)

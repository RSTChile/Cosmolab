"""
null4_bateria_correr.py — corre Phantom sobre las 3 condiciones NULL-4 de la batería N=2000 (semillas
de reordenamiento 601-603). Mismo patrón EXACTO que `null3_bateria_correr.py`/`null2_bateria_correr.py`/
`null1_bateria_correr.py` (congelados, no se tocan directamente): por cada carpeta
`/Users/alexis/phantom_cs073/bateria_null4_n2000/ic_null4_s{601..603}/` (ya con su
`cosmogenesis_ic.txt` escrito por `null4_bateria_generar.py`):
...
No toca `bateria_n2000/` (sólo se LEE `ic_real/cosmog.in` como plantilla de parámetros, y de hecho ni
siquiera eso -- el bloque de referencia está hardcodeado igual que en null3_bateria_correr.py, para no
depender de leer un archivo ajeno en tiempo de ejecución).
"""
import re
import subprocess
import sys
...
BASE = Path("/Users/alexis/phantom_cs073/bateria_null4_n2000")
SEMILLAS = [601, 602, 603]
CARPETAS = [BASE / f"ic_null4_s{s}" for s in SEMILLAS]
...
def correr_una(carpeta: Path) -> dict:
    ...
    assert ic.exists(), f"falta {ic} -- correr null4_bateria_generar.py primero"
...
def main():
    ...
    for carpeta in CARPETAS:
        if tiempo_phantom_acumulado > TIEMPO_LIMITE_S:
            ...
            print(f"\n[SALVAGUARDA DE TIEMPO] {tiempo_phantom_acumulado:.0f}s acumulados > "
                  f"{TIEMPO_LIMITE_S}s límite. Corridas hechas: {len(resultados)}/3. "
    ...
    print(f"\n[TOTAL] {len(resultados)}/3 corridas completadas. "

"""
cs090_fase7_f702_correr.py — FASE VII F7-02: corre Phantom sobre todos los escalones de la escalera de
clustering que fabricó `cs090_fase7_f702_escalera.py`.

MISMA FORMA EXACTA que `cs090_fase6_o3b_correr.py` (que copia la de `cs090_fase5b_correr.py`): se copia
el patrón, no se importa ni se modifica ninguno de los dos. El punto de esta tarea es que TODOS los
escalones pasen por el MISMO tubo, sin un solo parámetro distinto entre ellos:
  ...
  2. se reescribe el bloque de sumideros + tmax/dtmax de `cosmog.in` al protocolo estándar de toda la
     jerarquía CS073 / Fase V-B / Fase VI: icreate_sinks=1, rho_crit_cgs=1000, r_crit=0.6, h_acc=0.3,
     tmax=0.500, dtmax=0.001,
...
(misma salvaguarda que CS073 / Fase V-B / O3-B). No declara cierre ni veredicto.
"""
import re
import subprocess
import sys
...
def correr_una(carpeta: Path) -> dict:
    ...
    assert ic.exists(), f"falta {ic} -- correr cs090_fase7_f702_escalera.py primero"
...
if __name__ == "__main__":
    # uso: python3.9 cs090_fase7_f702_correr.py [patron_glob]   (default: todas las carpetas)
    ...
    # CUIDADO: que el archivo de condición inicial EXISTA no significa que esté COMPLETO -- el
    # generador tarda ~1 min en escribir las 2000 líneas y este runner puede correr en paralelo. Se
    # exige (a) que exista `meta_regla.json`, que el generador escribe DESPUÉS del IC y por lo tanto
    # funciona como marca de "terminado", y (b) que el IC tenga las N+2 líneas esperadas.

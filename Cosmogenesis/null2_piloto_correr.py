"""
null2_piloto_correr.py — corre Phantom sobre las 3 condiciones NULL-2 del piloto (Paso 2, escalón
NULL-2). MISMO patrón que null1_bateria_correr.py / real_extra_correr.py (congelados, no se tocan
directamente, sólo se copia su método): phantomsetup escribe un cosmog.in por defecto
(icreate_sinks=0), se reemplaza el bloque de sumideros + tmax/dtmax por el EXACTO de
...
física de siempre, no reinventada -- y se corre el binario phantom_cosmogenesis_backup (build previa a
APR, igual que el resto de los pilotos de esta jerarquía, para no introducir un confound de refinamiento
adaptativo frente a bateria_n2000).

No re-corre REAL -- ya está en disco en /Users/alexis/phantom_cs073/piloto_null1/real/ (4 sumideros,
masa 282.0, ver NULL1_piloto_distribucion_radial_CS.md), se usa como referencia sin tocarlo.
"""
import re
import subprocess
import sys
...
BASE = Path("/Users/alexis/phantom_cs073/piloto_null2")
CARPETAS = [BASE / f"null2_s{i}" for i in (1, 2, 3)]
...
TIEMPO_LIMITE_S = 20 * 60  # margen amplio -- se espera <30s de Phantom en total a esta escala (N=500)
...
def correr_una(carpeta: Path) -> dict:
    ...
    assert ic.exists(), f"falta {ic} -- correr null2_piloto_generar.py primero"

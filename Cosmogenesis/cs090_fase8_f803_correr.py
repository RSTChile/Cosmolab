"""
cs090_fase8_f803_correr.py — FASE VIII F8-03: corre Phantom sobre las variantes que fabricó
`cs090_fase8_f803_mismo_pico.py`.

MISMA FORMA EXACTA que `cs090_fase8_f801_correr.py` (que copia la de F7-03, que copia la de F7-02, que
copia la de O3-B, que copia la de Fase V-B): se copia el patrón, no se importa ni se modifica ninguno.
El punto de la tarea es que las dos ramas del par pasen por el MISMO tubo, sin un solo parámetro
distinto:
  ...
  2. se reescribe el bloque de sumideros + tmax/dtmax al protocolo estándar de toda la jerarquía
     CS073 / Fase V-B / Fase VI / Fase VII / Fase VIII: icreate_sinks=1, rho_crit_cgs=1000, r_crit=0.6,
     h_acc=0.3, tmax=0.500, dtmax=0.001,
  3. `phantom_cosmogenesis_backup cosmog.in`, midiendo el wall time real.

El criterio de sumidero es el de DENSIDAD de Phantom (rho_crit_cgs=1000 = 49.453× la densidad media,
F8-05), no un FoF laxo: el observable principal quedó declarado antes de correr.
...
def correr_una(carpeta: Path) -> dict:
    ...
    assert ic.exists(), f"falta {ic} -- correr cs090_fase8_f803_mismo_pico.py primero"
...
if __name__ == "__main__":
    # uso: python3.9 cs090_fase8_f803_correr.py [patron_glob]   (default: todas las carpetas)
    # CUIDADO (misma lección que F7-02/F7-03/F8-01): que el archivo de condición inicial EXISTA no
    # significa que esté COMPLETO -- el generador tarda ~1 min en escribir las 2000 líneas y este runner
    # puede correr en paralelo. Se exige (a) que exista `meta_regla.json`, que el generador escribe
    # DESPUÉS del IC y por lo tanto funciona como marca de "terminado", y (b) que el IC tenga N+2 líneas.

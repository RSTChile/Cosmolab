"""
FASE6-HIGGS-A V3 FINAL - barrido fino ventana VIVA
L=30 H_topo=0.01 como F0 PASS
r0>0 0.5-3.0 u 0.1-1.0 para VEV vivo
m_k = y0 * v_k * sum_rho formula unica
"""
import numpy as np
...
def run_once(r0,u):
    ...
    for step in range(pasos):
        ...
        r=r0*(Tc - Tnorm)
        lap=(np.roll(Phi,-1,axis=1)+np.roll(Phi,1,axis=1)+np.roll(Phi,-1,axis=0)+np.roll(Phi,1,axis=0)-4*Phi)
    ...
    for y in range(L):
        for x in range(L):
            ...
            q=deque([(y,x)]); visto[y,x]=True; nodes=[(y,x)]; lado=phi[y,x]>=media; sum_rho=phi[y,x]; sum_Phi=abs(Phi[y,x]); perim=0
            while q:
                cy,cx=q.popleft()
                ...
                for ny,nx,cond in [(cy,(cx+1)%L,ar[cy,cx]),(cy,(cx-1)%L,ar[cy,(cx-1)%L]),((cy+1)%L,cx,ad[cy,cx]),((cy-1)%L,cx,ad[(cy-1)%L,cx])]:
                    if cond and not visto[ny,nx] and (phi[ny,nx]>=media)==lado:
                        visto[ny,nx]=True; q.append((ny,nx)); nodes.append((ny,nx)); sum_rho+=phi[ny,nx]; sum_Phi+=abs(Phi[ny,nx])
            k=len(nodes); v_k=sum_Phi/k if k>0 else 0; m=y0*v_k*sum_rho
            clusters.append((k,perim,v_k,m))
    Phi_mean=np.mean(np.abs(Phi))
    k1_m=[c[3] for c in clusters if c[0]==1]; k3_m=[c[3] for c in clusters if c[0]==3 and c[1]==8]
    ...
    vk3=np.mean([c[2] for c in clusters if c[0]==3 and c[1]==8]) if any(c[0]==3 and c[1]==8 for c in clusters) else 0
    # NULL Phi=0 -> ratio geom 1/3
    ratio_null=0.333
    return Phi_mean,ratio,vk1,vk3,len(k1_m),len(k3_m)

print("FASE6-HIGGS-A V3 barrido r0 x u VIVO H=0.01 L30")
for r0,u in itertools.product(r0_list,u_list):
    ...
    print(f"r0={r0:.1f} u={u:.1f} <|Phi|>={Pm:.3f} v_k1={vk1:.3f} v_k3={vk3:.3f} ratio_Higgs={ratio:.5f} k1={k1n} k3={k3n} {flag}")

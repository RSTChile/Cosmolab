#!/usr/bin/env python3
"""
SUITE ÉPOCAS MASA v6 — mass_obs acoplada al linaje (pre-registrado)

Pre-registro: PROTOCOLO_V6_MASS_LINAJE_PREREGISTRO.md (ANTES de correr).

Hereda v5 (juez E4 = linaje). Cambio:
  mass_obs ya no ∝ E_mutual; mass_obs ∝ dens × n_groups × n_long_co × co_member × gyr
  solo en grav_mode=real. OFF/SHUFFLE/INVERT → mass_obs=0.

Contrato Alexis: E0–E3 sin masa; masa solo E4 REAL. Anti-Shannon.
"""
...
OUT = Path(__file__).resolve().parents[2] / "results" / "suite_epocas_masa_v6"
...
RATE_PASS = 0.55
PROTOCOL_ID = "V6_MASS_LINAJE_2026-07-23"
...
def simulate(p: P) -> dict:
    ...
    e_bind_acc = []
    e_mutual_acc = []
    mass_obs_max = 0.0  # v6: se fija al final desde linaje (no E_mutual)
    ...
    fusion_events = 0
    e4_steps = 0
    # últimos valores E4 para mass_obs v6
    dens_last = 1.0
    n_groups_last = 0
    ...
    for step in range(p.pasos):
        ...
        if e4_window and len(stable_atoms) >= 2:
            ...
            # v6: mass_obs se calcula al FINAL desde linaje (no E_mutual).
            # En hist dejamos 0 durante el loop; mass_obs_max se rellena al cierre.
    ...
    # --- mass_obs v6 (pre-registrado): linaje × dens × grupos ---
    if p.grav_mode == "real" and p.G_GRAV > 0 and e4_steps > 0:
        gyr_factor = 1.0
        if (
            r_gyr_start
            and r_gyr_last == r_gyr_last
            and r_gyr_start > 0
        ):
            gyr_factor = max(r_gyr_start / (r_gyr_last + 1e-6), 1.0)
        mass_obs_max = float(
            dens_last
            * max(n_groups_last, 1)
            * (n_long_co + 1e-6)
...
def main():
    print(f"=== SUITE ÉPOCAS MASA v6 — mass∝linaje ({PROTOCOL_ID}) ===\n")
    t0 = time.time()
    results = {}
    ...
    synthesis = {
        ...
        "design": {
            "juez_primario": "linaje; mass_obs acoplada a linaje (v6)",
            "diagnostico": "mutual_bind R/S (stack v4) — no gatea PASS",
            "pre_registro": "PROTOCOLO_V6_MASS_LINAJE_PREREGISTRO.md",
            "mass_obs": "v6: dens×n_groups×n_long_co×co_member×gyr solo REAL",
        },
    }
    results["synthesis"] = synthesis

    path = OUT / "suite_epocas_masa_v6_result.json"
    out = {
        "synthesis": synthesis,
    ...
    (OUT / "RESUMEN_SUITE_EPOCAS_MASA_v6.md").write_text("".join(md), encoding="utf-8")
    print(f"MD  → {OUT / 'RESUMEN_SUITE_EPOCAS_MASA_v6.md'}")
    print("\n=== GLOBAL ===", verdict)

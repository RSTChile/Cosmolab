"""
null5_bateria_correr.py -- corre Phantom sobre las 2 condiciones NULL-5 (semillas de permutación de fila
801-802). Mismo patrón EXACTO que `null4_bateria_correr.py` (no se toca directamente): por cada carpeta
`/Users/alexis/phantom_cs073/bateria_null5_n2000/ic_null5_s{801,802}/` (ya con su `cosmogenesis_ic.txt`
escrito por `null5_bateria_generar.py`):
...
No toca `bateria_n2000/` ni ningún script congelado -- sólo lectura/importación.
"""
import re
import subprocess
import sys
...
BASE = Path("/Users/alexis/phantom_cs073/bateria_null5_n2000")
SEMILLAS = [801, 802]
CARPETAS = [BASE / f"ic_null5_s{s}" for s in SEMILLAS]
...
def correr_una(carpeta: Path) -> dict:
    ...
    assert ic.exists(), f"falta {ic} -- correr null5_bateria_generar.py primero"
...
def main():
    ...
    for carpeta in CARPETAS:
        if tiempo_phantom_acumulado > TIEMPO_LIMITE_S:
            ...
            print(f"\n[SALVAGUARDA DE TIEMPO] {tiempo_phantom_acumulado:.0f}s acumulados > "
                  f"{TIEMPO_LIMITE_S}s límite. Corridas hechas: {len(resultados)}/2. "
    ...
    print(f"\n[TOTAL] {len(resultados)}/2 corridas completadas. "

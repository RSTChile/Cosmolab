"""
cs090_fase8_f802_correr.py — FASE VIII F8-02: corre Phantom sobre los cinco niveles de pico local que
fabricó `cs090_fase8_f802_pico.py`.
...
El punto de la tarea es que los cinco niveles pasen por el MISMO tubo, sin un solo parámetro distinto:
  ...
  2. se reescribe el bloque de sumideros + tmax/dtmax al protocolo estándar de toda la jerarquía
     CS073 / Fase V-B / Fase VI / Fase VII / F8-01: icreate_sinks=1, rho_crit_cgs=1000, r_crit=0.6,
     h_acc=0.3, tmax=0.500, dtmax=0.001,
  3. `phantom_cosmogenesis_backup cosmog.in`, midiendo el wall time real.

`rho_crit_cgs = 1000` es el criterio de densidad REAL de Phantom (49.453 × la densidad media de estas
cajas, medido en `FASE8_F805_f703_solver_independiente_CS.md`) y es el que define el observable
principal declarado de antemano en esta tarea: **fracción de masa en sumideros**. No se usa FoF laxo.
...
USO
---
    python3.9 cs090_fase8_f802_correr.py                 # todas las carpetas
    python3.9 cs090_fase8_f802_correr.py '*_f802_L1'     # sólo un nivel (para repartir en shards)
"""
import re
import subprocess
import sys
...
def correr_una(carpeta: Path) -> dict:
    ...
    assert ic.exists(), f"falta {ic} -- correr cs090_fase8_f802_pico.py primero"
...
if __name__ == "__main__":
    # misma guarda que F8-01/F7-03: que el IC EXISTA no significa que esté COMPLETO. Se exige que
    # exista `meta_regla.json` (el generador lo escribe DESPUÉS del IC) y que el IC tenga N+2 líneas.

"""
null3_bateria_correr.py — corre Phantom sobre las 8 condiciones NULL-3 de la batería N=2000 (semillas
501-508). Mismo patrón EXACTO que `null2_bateria_correr.py`/`null1_bateria_correr.py` (congelados, no
se tocan directamente): por cada carpeta
`/Users/alexis/phantom_cs073/bateria_null3_n2000/ic_null3_s{501..508}/` (ya con su
`cosmogenesis_ic.txt` escrito por `null3_bateria_generar.py`):
...
Salvaguarda de tiempo (mismo criterio que null2_bateria_correr.py): si el tiempo ACUMULADO de cómputo
Phantom supera TIEMPO_LIMITE_S, el script deja de lanzar corridas nuevas y termina limpio.

No toca `bateria_n2000/` (sólo se LEE `ic_real/cosmog.in` como plantilla de parámetros).
"""
import re
import subprocess
import sys
...
BASE = Path("/Users/alexis/phantom_cs073/bateria_null3_n2000")
SEMILLAS = list(range(501, 509))
CARPETAS = [BASE / f"ic_null3_s{s}" for s in SEMILLAS]
...
def correr_una(carpeta: Path) -> dict:
    ...
    assert ic.exists(), f"falta {ic} -- correr null3_bateria_generar.py primero"

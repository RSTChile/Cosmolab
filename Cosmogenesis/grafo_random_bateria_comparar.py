"""
grafo_random_bateria_comparar.py — REAL (n=6) vs las 8 corridas del control grafo-random (N=2000,
semillas 701-708) recién corridas, y grafo-random vs NULL-1/NULL-2/NULL-3 (8 vs 8, ya en disco) --
Fase II CS073, control adicional (Erdős-Rényi independiente de REAL).

Reimplementación línea a línea de `null3_bateria_comparar.py` (congelado, no se toca) -- misma
matemática exacta, mismas columnas del `.sink`, mismo test de permutación exacto (H1 pre-registrada de
una cola REAL>control, sobre TODAS las C(n_REAL+n_control, n_REAL) asignaciones posibles bajo H0).

No corre ninguna simulación. No toca ninguna carpeta de batería anterior (sólo lectura). No declara
cierre de CS073 ni de la jerarquía -- reporta números, la lectura es de Alexis.
"""
...
BASE_RANDOM = Path("/Users/alexis/phantom_cs073/bateria_grafo_random_n2000")
RANDOM_SEEDS = list(range(701, 709))
RANDOM_CARPETAS = [BASE_RANDOM / f"ic_random_s{s}" for s in RANDOM_SEEDS]
...
def main():
    print("=" * 92)
    print("Control grafo-random (Erdős-Rényi, N=2000, 8 semillas 701-708) vs REAL (n=6), NULL-1 (n=8), "
          "NULL-2 (n=8), NULL-3 (n=8)")
    ...
    random_ctrl = [masa_y_n_sumideros(c) for c in RANDOM_CARPETAS]
    ...
    masas_random = _reportar_grupo(
        "CONTROL grafo-random (Erdős-Rényi G(n,m), INDEPENDIENTE de REAL, mismo n/m + layout_resortes)",
        random_ctrl)

    r_a = test_permutacion_2_grupos(masas_real, masas_random, nombre_a="REAL", nombre_b="RANDOM")
    _reportar_comparacion("(a) REAL (n=6) vs CONTROL grafo-random (n=8) -- test de permutación EXACTO",
                           r_a)
    if masas_random.std(ddof=1) > 0:
        z = (masas_real.mean() - masas_random.mean()) / masas_random.std(ddof=1)
        print(f"    z-score ((media_REAL - media_RANDOM) / DE_RANDOM) = {z:.3f}")
    else:
        print("    z-score: INDEFINIDO (DE_RANDOM = 0, probablemente todas las corridas dieron masa 0).")

    r_b = test_permutacion_2_grupos(masas_null3, masas_random, nombre_a="NULL-3", nombre_b="RANDOM")
    _reportar_comparacion(
        "(b) NULL-3 (n=8, grafo REAL rebarajado) vs CONTROL grafo-random (n=8, grafo INDEPENDIENTE) -- "
        "¿importa la identidad del grafo, o basta con pasar por layout_resortes?", r_b)

    r_c = test_permutacion_2_grupos(masas_random, masas_null1, nombre_a="RANDOM", nombre_b="NULL-1")
    _reportar_comparacion(
        "(c) CONTROL grafo-random (n=8) vs NULL-1 (n=8, sin grafo/sin layout_resortes) -- ¿el mero "
        "hecho de pasar por layout_resortes ya produce ALGO más que NULL-1?", r_c)

    r_d = test_permutacion_2_grupos(masas_random, masas_null2, nombre_a="RANDOM", nombre_b="NULL-2")
    _reportar_comparacion(
        "(d) CONTROL grafo-random (n=8) vs NULL-2 (n=8, sin grafo/sin layout_resortes) -- mismo "
        "contraste que (c) contra el otro brazo sin layout", r_d)
